...
import { callTool, getRuntimeBaseUrl, tools } from "./runtime-client.js"

const port = Number(process.env.APEX_MCP_HTTP_PORT ?? 8713)
...
createServer(async (request, response) => {
  ...
  if (request.method === "GET" && url.pathname === "/health") {
    try {
      await callTool("get_runtime_status")
      sendJson(response, 200, { status: "ok", server: "apex-p6", runtime: getRuntimeBaseUrl() })
    } catch (error) {
  ...
  if (message.method === "initialize") {
    sendJson(response, 200, {
      ...
      result: {
        ...
        serverInfo: { name: "apex-p6", version: "0.1.0" },
      },
    })
    return
  }
  ...
  if (message.method === "tools/call") {
    ...
    try {
      const result = await callTool(String(params.name ?? ""))
      sendJson(response, 200, {
        jsonrpc: "2.0",
        id: message.id ?? null,
        result: {
  ...
  console.error(`apex-p6 HTTP transport listening on ${port}`)
})