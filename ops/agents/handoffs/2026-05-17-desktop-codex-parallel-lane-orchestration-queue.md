...
| NETA Study Material | Align external source archive policy after approved Resources cleanup/archive findings; topic-spine comparative audit remains parked behind this policy wording scout | Band A/B | Desktop Codex or sidecar executor | One future closeout only if external archive policy scout is dispatched | Shared code, package files, product UI, repo-wide status, credentials, hosted services, macros, source PDF/workbook contents | Resources docs/manifest approval, Box archive filename/count reconciliation, proposed archive policy wording | VS Code Codex review required before any source-domain edit or cleanup commit | READY_FOR_JASON_DECISION for external archive policy scout dispatch | Dispatch `Desktop Codex NETA External Source Archive Policy Alignment Scout` | Required for source-domain edits |
...
Desktop Codex Relay proof, first NETA scout proof, revised NETA source-map/artifact-backlog proof, and NETA Resources cleanup/archive findings are accepted. The next approved non-PM move is a read-only `Desktop Codex NETA External Source Archive Policy Alignment Scout`, not source-domain editing, content generation, restore work, or cleanup commit. The previously recommended `NETA Topic Spine Comparative Audit - Electrical Fundamentals` remains parked until the external archive policy wording scout returns clean or Jason explicitly reprioritizes. TCC remains `HOLD_NOT_ADMITTED` until NETA proves the same relay-load reduction and evidence-compression pattern or Jason explicitly reprioritizes TCC.
