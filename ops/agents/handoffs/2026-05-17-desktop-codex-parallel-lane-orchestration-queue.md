...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| PM Temp Power | Keep Project Miner PM lane moving from first governed approval-row execution toward import mutation admission and field-ready workflow | Band C only where packet-approved | VS Code Codex primary; VS Code Copilot authenticated Render executor for hosted Render lanes; Desktop Codex support only | Only packet-approved PM support notes, Render completion closeouts, approval-row closeouts, or handoffs | PM business-state mutation beyond admitted approval row, schema, import mutation, schedule/status writes, new service/public storage unless separately admitted | PM lane packet validations and hosted smokes owned by VS Code Codex; PM Lane 276 Render proof accepted from VS Code Copilot; PM Lane 277 first approval-row proof accepted from VS Code Codex | Standing PM blocker authority is recorded for bounded predetermined PM packets; VS Code review still required | Active priority | PM Lane 277 created the first approval row: approval status is `approved_for_import_packet`, row count `1`, record `pm-import-approval-03a1aea39afde71b44516f44`, and `import_authority: not_admitted`. Next blocker: `STOPPED_AWAITING_PROJECT_IMPORT_MUTATION_PACKET_AFTER_APPROVAL_ROW` | Always required |
