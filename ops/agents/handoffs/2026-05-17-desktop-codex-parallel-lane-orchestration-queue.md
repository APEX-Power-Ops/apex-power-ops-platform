...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| PM Temp Power | Keep Project Miner PM lane moving from approved import route deployment toward live import proof and field-ready workflow | Band C only where packet-approved | VS Code Codex primary; VS Code Copilot authenticated Render executor for hosted Render lanes; Desktop Codex support only | Only packet-approved PM support notes, Render completion closeouts, approval-row/import-route closeouts, or handoffs | PM business-state mutation beyond admitted approval/import route, schema, schedule/status writes, new service/public storage unless separately admitted | PM lane packet validations and hosted smokes owned by VS Code Codex; PM Lane 276 Render proof accepted from VS Code Copilot; PM Lane 277 first approval-row proof accepted from VS Code Codex; PM Lane 278 local route validation passed | Standing PM blocker authority is recorded for bounded predetermined PM packets; VS Code review still required | Active priority | PM Lane 278 implemented and pushed `POST /api/v1/mutations/project-imports` plus `GET /api/v1/reads/project-import-status` at commit `4d5af24f`. Hosted still returns `404` for the new route. Next blocker: `STOPPED_AWAITING_RENDER_DEPLOY_OF_PROJECT_IMPORT_ROUTE` | Always required |
