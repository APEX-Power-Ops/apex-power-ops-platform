...
| NETA Study Material | Prepare Electrical Fundamentals topic-spine comparative audit decision after revised source-map closeout | Band A/B | Desktop Codex or sidecar executor | One future closeout only if comparative audit packet is admitted | Shared code, package files, product UI, repo-wide status, credentials, hosted services, macros | v1/v2.3/v2.4 comparison map, topic-spine model, preserve/improve/discard list, citation/KSA/schema gaps | VS Code Codex review only if shared repo/product surfaces are touched | READY_FOR_JASON_DECISION for comparative audit | Decide whether to admit NETA Topic Spine Comparative Audit | Conditional |
...
Desktop Codex Relay proof, first NETA scout proof, and revised NETA source-map/artifact-backlog proof are accepted. The next recommended non-PM move is a read-only `NETA Topic Spine Comparative Audit - Electrical Fundamentals`, not broad content generation or source-domain editing. TCC remains `HOLD_NOT_ADMITTED` until that comparative audit proves the same relay-load reduction and evidence-compression pattern or Jason explicitly reprioritizes TCC.
