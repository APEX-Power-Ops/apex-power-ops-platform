...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| PM Temp Power | Keep Project Miner PM lane moving from approved import proof toward field authorization, lead assignment, and field-ready workflow | Band C only where packet-approved | VS Code Codex primary; VS Code Copilot authenticated Render executor for hosted Render lanes; Desktop Codex support only | Only packet-approved PM support notes, Render completion closeouts, approval-row/import-route/import closeouts, or handoffs | PM business-state mutation beyond admitted approval/import route, schema, schedule/status writes, new service/public storage unless separately admitted | PM lane packet validations and hosted smokes owned by VS Code Codex; PM Lane 276 Render proof accepted from VS Code Copilot; PM Lane 277 first approval-row proof accepted from VS Code Codex; PM Lane 278 import proof accepted from VS Code Codex | Standing PM blocker authority is recorded for bounded predetermined PM packets; VS Code review still required | Active priority | PM Lane 278 imported the approved Temp Power candidate: status `accepted`, replay `idempotent_hit`, import-status `imported`, rows `1/7/15/184`, source traces `199`, warning reviews `1`, and downstream assignments/schedule/status/customer/production/finance writes remain blocked. Next blocker: `STOPPED_AWAITING_FIELD_AUTHORIZATION_AND_LEAD_ASSIGNMENT_PACKET_AFTER_PROJECT_IMPORT` | Always required |
