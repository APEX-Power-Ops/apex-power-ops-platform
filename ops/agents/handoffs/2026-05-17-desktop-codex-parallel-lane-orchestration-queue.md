...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| PM Temp Power | Keep Project Miner PM lane moving toward first governed approval-row execution and field-ready workflow | Band C only where packet-approved | VS Code Codex primary; VS Code Copilot authenticated Render executor for PM Lane 276; Desktop Codex support only | Only packet-approved PM support notes, Render completion closeout, or handoffs | PM business-state mutation, Supabase writes, schema, approval POST, import mutation, schedule/status writes, new service/public storage unless separately admitted | PM lane packet validations and hosted smokes owned by VS Code Codex; PM Lane 276 Render proof accepted from VS Code Copilot Internal browser | Jason approval for business workflow and live writes | Active priority | Render hosted-read blocker cleared by PM Lane 276 `RENDER_SNAPSHOT_ENV_DEPLOY_PASS_NO_APPROVAL_POST`; hosted reads now return `pm-import-candidate-miner-temp-power` with 7 workpackages, 15 tasks, 184 apparatus candidates, and `mutation_authority: not_admitted`. Next blocker: `STOPPED_AWAITING_EXPLICIT_LIVE_APPROVAL_POST_PACKET_NO_IMPORT` | Always required |
