# Historical Parent-Root PM-Schema-UI-002D Draft Publication Handoff
## Date: 2026-04-22
...
3. use this handoff only when historical provenance is needed for the earlier parent-root `pm-schema-ui-002d` draft-publication decision.
...
Measured from the parent git root at `C:/APEX Platform` on 2026-04-22 after the `pm-schema-020c` draft publication:

1. remaining untracked top-level distribution is `ops` 420, `knowledge` 974, `archive` 2516, plus 2 excluded generated app artifacts
2. the remaining `ops/agents` backlog is still split into `handoffs` 333 and `packets/draft` 87
3. `git add -n --` on the bounded `pm-schema-ui-002d` packet stages exactly this file cleanly:
   - `2026-04-18-pm-schema-ui-002d-baseline-overlay-reissue-and-read-bridge-extension.json`
4. `pm-schema-ui-002d` is now fully dependency-safe after `pm-schema-ui-002b`, `pm-schema-ui-002c`, and the published `pm-schema-020a` through `pm-schema-020c` baseline substrate chain
...
3. run `Parent-root pm-schema-ui 002d draft packet staged diff`

Direct parent-root path if tasks are not used:

```powershell
Set-Location 'C:/APEX Platform'
git add -n -- apex-power-ops-platform/ops/agents/packets/draft/2026-04-18-pm-schema-ui-002d-baseline-overlay-reissue-and-read-bridge-extension.json
git add -- apex-power-ops-platform/ops/agents/packets/draft/2026-04-18-pm-schema-ui-002d-baseline-overlay-reissue-and-read-bridge-extension.json
git diff --cached -- apex-power-ops-platform/ops/agents/packets/draft/2026-04-18-pm-schema-ui-002d-baseline-overlay-reissue-and-read-bridge-extension.json
```
...
1. do not widen this packet into `pm-schema-020d`, later host/UI packets, handoffs, `knowledge`, or `archive`
2. do not imply real XER baseline ingestion work is included in this UI re-issue packet
...
1. the bounded `pm-schema-020d` baseline XER parser and loader wiring singleton
2. then re-evaluate any later baseline-ingestion and driver packets from the `020` family