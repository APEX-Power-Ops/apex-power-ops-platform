...
Status: **Stages A + B (ETU+TMT+EMT) + dual-axis (backend+FRONTEND) + NETA Test @ fix + Stage C ETU curve SHIPPED + verified live.** Remaining: TMT/EMT live curves + the **gated** `route_delay_curve` engine patch.
...
- **Stage C** — ✅ **ETU curve SHIPPED** (`55bafdcd`, nominal illustration). Remaining: **TMT/EMT live curves** (`/tmt/plot-tcc` · `/emt/plot-tcc`, same nominal-illustration pattern — buildable, bounded) + the **GATED** `route_delay_curve` engine work (checkpoint). The engine half could be lighter than feared: the **finding above** shows `/plot-tcc` already computes correct delay times via separate `*_test_multiple` params, so reconciling Screen 2's delay rows onto that surface may promote them "verify"→"DB" without a kernel change. Also unlocks: selectable delay test points, EMT computed test currents, TMT thermal LT band.
