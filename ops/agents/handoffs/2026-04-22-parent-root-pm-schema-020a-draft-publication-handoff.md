...
Use this packet to introduce the bounded `pm-schema-020a` baseline field authority promotion singleton:
...
1. it introduces only the first authority-only baseline-source promotion step and does not widen into `020b`, `020c`, or later runtime/UI follow-ons
...
3. only then re-evaluate the next dependency-safe `ui-002` follow-on such as `pm-schema-ui-002d`