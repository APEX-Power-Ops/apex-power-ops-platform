# PM Lane 095 - Local Import Exception Decision Register Disclosure Controls Handoff

## Purpose

PM Lane 095 reduces the `/pm-review/import-intake` scan burden by wrapping only the existing `Local Import Exception Decision Register` panel in a browser-native, default-open disclosure control.
...
- Branch: `clean-main`
- Source floor: `221d11b2d655a0623d9b96085b8fa3e634e6cdce`
- Prior lane: PM Lane 094, Local PM Operating Queue Disclosure Controls

## Implemented Scope

- Converted only `#import-exception-register` / `Local Import Exception Decision Register` from a plain card section to a default-open native `<details>` disclosure.
- Preserved the `import-exception-register` id, `Local import exception decision register` aria label, and placement inside `Source and Exception Detail`.
- Preserved the six derived register items, item order, status pills, summary counts, dynamic behavior, export behavior, no-storage behavior, read seams, and no-authority wording.
- Added focused smoke coverage for default-open state, collapse/reopen behavior, hidden/visible register items, unchanged item count, and no `collapse|disclosure|import-exception-register` localStorage key.
...
- `ops/agents/packets/draft/2026-05-16-pm-lane-095-local-import-exception-register-disclosure-controls.json`
- `ops/agents/handoffs/2026-05-16-pm-lane-095-local-import-exception-register-disclosure-controls-handoff.md`

## Not Allowed

- No backend endpoint change.
- No new route or route target change.
- No quick-jump, route-link, output action, output status, output selector, project packet, workflow gates, exception review detail, approval prep, snapshot, operating queue, workflow-map, or open-items behavior change.
- No exception register item label, order, count, status pill, summary count behavior, dynamic behavior, export behavior, export filename, or export content change.
...
`Project Miner Workflow Gates Disclosure Controls`

The recommended PM Lane 096 scope is to wrap only `#workflow-gates` / `Workflow Gates` in the same default-open disclosure pattern while preserving id, aria label, placement after the exception register, six gate items, item order, status pills, detail text, read-only label, quick-jump hrefs, export references, no-storage behavior, read seams, and no-authority wording.
...
& "C:/APEX Platform/apex-power-ops-platform/.venv/Scripts/python.exe" -c "import json; json.load(open(r'C:/APEX Platform/apex-power-ops-platform/ops/agents/packets/draft/2026-05-16-pm-lane-095-local-import-exception-register-disclosure-controls.json', encoding='utf-8')); print('packet-json-ok')"
git diff --check
...
- Stage only the Lane 095 files listed above.
- Commit with message `Add PM exception register disclosure controls`.
- Push `clean-main`.
