# Historical Parent-Root PM-Schema-020B Draft Publication Handoff
## Date: 2026-04-22
...
3. use this handoff only when historical provenance is needed for the earlier parent-root `pm-schema-020b` draft-publication decision.
...
Measured from the parent git root at `C:/APEX Platform` on 2026-04-22 after the `pm-schema-020a` draft publication:

1. remaining untracked top-level distribution is `ops` 422, `knowledge` 974, `archive` 2516, plus 2 excluded generated app artifacts
2. the remaining `ops/agents` backlog is still split into `handoffs` 333 and `packets/draft` 89
3. `git add -n --` on the bounded `pm-schema-020b` packet stages exactly this file cleanly:
   - `2026-04-17-pm-schema-020b-xer-baseline-mapping-authorization.json`
4. `020b` is authority-only and maps baseline-bearing XER sources before the later persisted schedule baseline work in `020c`
...
3. run `Parent-root pm-schema-020b draft packet staged diff`

Direct parent-root path if tasks are not used:

```powershell
Set-Location 'C:/APEX Platform'
git add -n -- apex-power-ops-platform/ops/agents/packets/draft/2026-04-17-pm-schema-020b-xer-baseline-mapping-authorization.json
git add -- apex-power-ops-platform/ops/agents/packets/draft/2026-04-17-pm-schema-020b-xer-baseline-mapping-authorization.json
git diff --cached -- apex-power-ops-platform/ops/agents/packets/draft/2026-04-17-pm-schema-020b-xer-baseline-mapping-authorization.json
```
...
1. do not widen this packet into `020c`, `020d`, later `ui-002` packets, handoffs, `knowledge`, or `archive`
2. do not imply the persisted baseline lane is authorized before `020c` lands
...
1. the bounded `pm-schema-020c` persisted schedule baseline DDL and loader lane
2. then re-evaluate the next dependency-safe follow-on, including `pm-schema-020d` and `pm-schema-ui-002d`