# PM Lane 100 - Local Approval Decision Draft Disclosure Controls Handoff

## Purpose

PM Lane 100 reduces the `/pm-review/import-intake` scan burden by wrapping only the existing `Local approval decision draft` panel in a browser-native, default-open disclosure control.
...
- Branch: `clean-main`
- Source floor: `d8306b0ace5867f0570eab2d672f0fce1cf99f5d`
- Prior lane: PM Lane 099, Local Review Checklist Disclosure Controls

## Implemented Scope

- Converted only `Local approval decision draft` from a plain panel to a default-open native `<details>` disclosure.
- Preserved the `Local approval decision draft` aria label and placement inside `Approval Prep Detail` after Local Review Checklist.
- Preserved the decision selector, review notes textarea, local-only attestation, clear button, existing candidate-scoped storage key, export inclusion, dynamic derived-state behavior, no-storage behavior, read seams, and no-authority wording.
- Added focused smoke coverage for default-open state, collapse/reopen behavior, hidden/visible draft controls, three draft-control labels, no `collapse|disclosure|approval-draft|approval-decision-draft` localStorage key, disabled/enabled clear button behavior, and unchanged draft selection/notes/attestation behavior.
...
- `ops/agents/packets/draft/2026-05-16-pm-lane-100-local-approval-decision-draft-disclosure-controls.json`
- `ops/agents/handoffs/2026-05-16-pm-lane-100-local-approval-decision-draft-disclosure-controls-handoff.md`

## Not Allowed

- No backend endpoint change.
- No new route or route target change.
- No quick-jump, route-link, output action, output status, output selector, project packet, source freshness, import exception register, workflow gates, exception review detail, admission and approval contract, local review checklist, local executor closeout intake, snapshot, operating queue, workflow-map, or open-items behavior change.
- No decision selector option, review notes textarea behavior, local-only attestation behavior, clear button behavior, candidate-scoped approval draft storage key, approval draft export inclusion, or dynamic derived-state behavior change.
...
`Project Miner Local Executor Closeout Intake Disclosure Controls`

The recommended PM Lane 101 scope is to wrap only the existing `Local executor closeout intake` panel in the same default-open disclosure pattern while preserving placement inside `Executor Closeout Detail`, all eight closeout checklist items, checkbox behavior, clear button, candidate-scoped storage key, export inclusion, dynamic derived state, labels, no-authority wording, and read seams.
...
& "C:/APEX Platform/apex-power-ops-platform/.venv/Scripts/python.exe" -c "import json; json.load(open(r'C:/APEX Platform/apex-power-ops-platform/ops/agents/packets/draft/2026-05-16-pm-lane-100-local-approval-decision-draft-disclosure-controls.json', encoding='utf-8')); print('packet-json-ok')"
git diff --check
...
- Stage only the Lane 100 files listed above.
- Commit with message `Add PM approval draft disclosure controls`.
- Push `clean-main`.
