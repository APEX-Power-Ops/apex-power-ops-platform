# PM Lane 122 - Local Workflow Gates Body Controls Handoff

## Purpose

PM Lane 122 reduces the `/pm-review/import-intake` scan burden by wrapping only the existing `Workflow gates` body content in a labeled body-controls container under its already-existing browser-native, default-open disclosure.

This is local workflow-gate scanning and AI-orchestration ergonomics only. It does not accept, approve, persist, deploy, import, assign, schedule, change status, create issues, create tasks, create durable field records, write production tracking rows, claim hosted parity, or mutate production state.

## Source Floor

- Repo: `C:/APEX Platform/apex-power-ops-platform`
- Branch: `clean-main`
- Source floor: `5bf0abd9db57cc55736296a080ae83afdba83dec`
- Prior lane: PM Lane 121, Local Import Exception Decision Register Body Controls
- Parallel external lane available: Desktop Codex hosted parity dispatch through PM Lane 076

## Implemented Scope

- Added only a labeled `Workflow gates controls` body container under the existing `Workflow gates` disclosure summary.
- Preserved the `#workflow-gates` anchor, existing disclosure, aria label, heading, read-only pill, and placement inside `Source and Exception Detail` after the exception register.
- Preserved the six gate cards, card order, detail text, status pills, quick-jump target, read seams, export references, and no-storage behavior.
- Added focused smoke coverage for default-open state, visible body controls, collapse/reopen behavior, hidden/visible workflow-gates controls, six gate articles, no `collapse|disclosure|workflow-gates` localStorage key, and unchanged gate labels.
...
& "C:/APEX Platform/apex-power-ops-platform/.venv/Scripts/python.exe" -c "import json; json.load(open(r'C:/APEX Platform/apex-power-ops-platform/ops/agents/packets/draft/2026-05-16-pm-lane-122-local-workflow-gates-body-controls.json', encoding='utf-8')); print('packet-json-ok')"
git diff --check
...
`PM Lane 123 - Local Exception Review And PM Decision Detail Body Controls`

That next lane should wrap only the existing `Exception review and PM decision detail` body content under its already-existing default-open disclosure, preserving the detail anchor if present, exception/decision cards, warning/decision copy, no persisted collapse state, no authority widening, and the same focused smoke pattern.
