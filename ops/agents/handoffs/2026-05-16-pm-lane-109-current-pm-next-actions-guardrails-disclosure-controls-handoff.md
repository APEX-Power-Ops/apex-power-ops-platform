# PM Lane 109 - Current PM Next Actions and Guardrails Disclosure Controls Handoff

## Purpose

PM Lane 109 reduces the `/pm-review/import-intake` scan burden by wrapping only the existing `Current PM next actions and guardrails` footer in a browser-native, default-open disclosure control.
...
- Branch: `clean-main`
- Source floor: `8e49c402af743032760985cd95876af98dd2c9b5`
- Prior lane: PM Lane 108, Approval Persistence Readiness Disclosure Controls

## Implemented Scope

- Converted only `Current PM next actions and guardrails` from a plain footer panel to a default-open native `<details>` disclosure.
- Preserved the `#guardrails` anchor, aria label, and placement after `Approval Persistence Readiness`.
- Preserved the `Current PM Next Actions` card, `Not Allowed Now` card, list text, list order, not-allowed fallback rendering, route/quick-jump links, read seams, and no-authority wording.
- Added focused smoke coverage for default-open state, quick-jump hash behavior, collapse/reopen behavior, hidden/visible guardrails controls, both guardrail cards, no `collapse|disclosure|guardrails` localStorage key, and unchanged no-authority wording.
...
A read-only PM Lane 110 sidecar scout ran in parallel and made no edits, staged nothing, committed nothing, ran no validation, and did not touch Supabase, Render, Vercel, Olares, live services, macros, source-domain files, project source files, or workbook files.

Recommended next lane:

`Project Miner Local PM Intake Command Center Disclosure Controls`

The recommended PM Lane 110 scope is to wrap only the existing `Local PM Intake Command Center` panel in the same default-open disclosure pattern while preserving the `#pm-command-center` anchor, aria label, placement under Intake Triage Panels, heading, browser-local pill, four derived link cards, hrefs, dynamic text, status pills, no persisted collapse state, and no-authority wording.
...
& "C:/APEX Platform/apex-power-ops-platform/.venv/Scripts/python.exe" -c "import json; json.load(open(r'C:/APEX Platform/apex-power-ops-platform/ops/agents/packets/draft/2026-05-16-pm-lane-109-current-pm-next-actions-guardrails-disclosure-controls.json', encoding='utf-8')); print('packet-json-ok')"
git diff --check
...
- Stage only the Lane 109 files listed above.
- Commit with message `Add PM guardrails disclosure controls`.
- Push `clean-main`.
