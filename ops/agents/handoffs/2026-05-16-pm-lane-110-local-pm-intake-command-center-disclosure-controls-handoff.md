# PM Lane 110 - Local PM Intake Command Center Disclosure Controls Handoff

## Purpose

PM Lane 110 reduces the `/pm-review/import-intake` scan burden by wrapping only the existing `Local PM intake command center` panel in a browser-native, default-open disclosure control.
...
- Branch: `clean-main`
- Source floor: `61ef675cb10136d7b59436c2987032767e399be1`
- Prior lane: PM Lane 109, Current PM Next Actions and Guardrails Disclosure Controls

## Implemented Scope

- Converted only `Local PM intake command center` from a plain panel to a default-open native `<details>` disclosure.
- Preserved the `#pm-command-center` anchor, aria label, heading, browser-local pill, and placement under `Intake Triage Panels`.
- Preserved the explanatory no-authority wording, four derived command-center cards, card order, hrefs, dynamic text, status pills, quick-jump target, read seams, and no-storage behavior.
- Added focused smoke coverage for default-open state, collapse/reopen behavior, hidden/visible command-center controls, four command-center cards, no `collapse|disclosure|command-center` localStorage key, unchanged hrefs, and unchanged before/after dynamic command-center assertions.
...
- `ops/agents/packets/draft/2026-05-16-pm-lane-110-local-pm-intake-command-center-disclosure-controls.json`
- `ops/agents/handoffs/2026-05-16-pm-lane-110-local-pm-intake-command-center-disclosure-controls-handoff.md`
...
- No command-center anchor, card label, card detail, card href, card count, status pill, dynamic text, or export inclusion change.
...
A read-only PM Lane 111 sidecar scout ran in parallel and made no edits, staged nothing, committed nothing, ran no validation, and did not touch Supabase, Render, Vercel, Olares, live services, macros, source-domain files, project source files, or workbook files.

Recommended next lane:

`Project Miner Local PM Intake Meeting Readout Disclosure Controls`

The recommended PM Lane 111 scope is to wrap only the existing `Local PM Intake Meeting Readout` panel in the same default-open disclosure pattern while preserving the `#pm-meeting-readout` anchor, aria label, placement after `#pm-command-center` and before `#pm-constraint-radar`, heading, browser-local pill, no-authority paragraph, four derived readout cards, hrefs, order, dynamic text, status pills, no persisted collapse state, and read seams.
...
& "C:/APEX Platform/apex-power-ops-platform/.venv/Scripts/python.exe" -c "import json; json.load(open(r'C:/APEX Platform/apex-power-ops-platform/ops/agents/packets/draft/2026-05-16-pm-lane-110-local-pm-intake-command-center-disclosure-controls.json', encoding='utf-8')); print('packet-json-ok')"
git diff --check
...
- `git diff --cached --check` passed after staging only the Lane 110 files.

## Closeout Requirements

- Stage only the Lane 110 files listed above.
- Commit with message `Add PM command center disclosure controls`.
- Push `clean-main`.
