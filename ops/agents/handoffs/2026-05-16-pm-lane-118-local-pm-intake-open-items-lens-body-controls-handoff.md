# PM Lane 118 - Local PM Intake Open Items Lens Body Controls Handoff

## Purpose

PM Lane 118 reduces the `/pm-review/import-intake` scan burden by wrapping only the existing `Local PM intake open items lens` body content in a labeled body-controls container under its already-existing browser-native, default-open disclosure.

This is local PM attention/blocker scanning and AI-orchestration ergonomics only. It does not accept, approve, persist, deploy, import, assign, schedule, change status, create issues, create tasks, create durable field records, write production tracking rows, claim hosted parity, or mutate production state.

## Source Floor

- Repo: `C:/APEX Platform/apex-power-ops-platform`
- Branch: `clean-main`
- Source floor: `b4f31d1cddf737ef99a1b67349d82a3cf328b307`
- Prior lane: PM Lane 117, Local PM Intake Workflow Map Body Controls

## Implemented Scope

- Added only a labeled `Open items lens controls` body container under the existing `Local PM intake open items lens` disclosure summary.
- Preserved the `#pm-open-items` anchor, existing disclosure, aria label, heading, browser-local pill, and placement inside `Workflow Review Panels` after `#pm-workflow-map`.
- Preserved the explanatory no-authority wording, six derived open-items cards, card order, hrefs, dynamic text, status pills, quick-jump target, read seams, and no-storage behavior.
- Added focused smoke coverage for default-open state, visible body controls, collapse/reopen behavior, hidden/visible open-items controls, six open-items links, no `collapse|disclosure|open-items` localStorage key, unchanged hrefs, and unchanged before/after dynamic open-items assertions.
...
A read-only PM Lane 119 sidecar scout ran in parallel and made no edits, staged nothing, committed nothing, ran no macros, created no artifacts, and did not touch Supabase, Render, Vercel, Olares, live services, source-domain files, project source files, or workbook files.

Recommended next lane:

`Project Miner Local PM Intake Snapshot Body Controls`

The recommended PM Lane 119 scope is to add only a body wrapper under the existing `Local PM Intake Snapshot` disclosure summary, preserving the `#pm-intake-snapshot` anchor, aria label, heading, browser-local pill, existing copy, six snapshot items, counts, status pills, no persisted collapse state, and read-only seams.
...
& "C:/APEX Platform/apex-power-ops-platform/.venv/Scripts/python.exe" -c "import json; json.load(open(r'C:/APEX Platform/apex-power-ops-platform/ops/agents/packets/draft/2026-05-16-pm-lane-118-local-pm-intake-open-items-lens-body-controls.json', encoding='utf-8')); print('packet-json-ok')"
git diff --check
...
- `git diff --cached --check` passed after staging only the Lane 118 files.

## Closeout Requirements

- Stage only the Lane 118 files listed above.
- Commit with message `Add PM open items body controls`.
- Push `clean-main`.
