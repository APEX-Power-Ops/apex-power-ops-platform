...
Sub PopulateAllTasks()
    ...
    For Each ws In ThisWorkbook.Worksheets
        If IsScopeSheetByContent(ws) And ws.Name <> Global_Constants.SHEET_SCOPE_TEMPLATE Then
            ...
            For currentRow = 6 To lastRow  ' Start from row 6 (first data row)
                ...
                ' Only copy rows with Task_ID (skip headers/blank rows)
                If Len(taskId) > 0 And taskId <> "TOTALS" Then
                    ' Only copy child rows (3+ parts in Task_ID)
                    If Global_Constants.CountDots(taskId) >= 2 Then
                        With wsAllTasks
    ...
    MsgBox "All_Tasks populated with " & (targetRow - Global_Constants.AT_FIRST_DATA_ROW) & " rows!", vbInformation
End Sub
