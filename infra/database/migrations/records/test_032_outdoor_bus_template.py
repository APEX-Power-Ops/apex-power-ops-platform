"""TDD - records Chip 2c: Outdoor Bus Structure datasheet (NETA 7.21).

Slice-3 coverage backlog. Leaf 'outdoor_bus_structure' already exists (Chip 2-shell). NETA 7.21 =
6 VM + 4 electrical = 10 items. R-A = neta_table (IR->100.1, dielectric->100.19) + mfr (PF/DF, bolted).
Capture = field + cover_attach.

RED until gen_outdoor_bus_template.py emits 032_outdoor_bus_template.sql. Run PER-CHIP.
"""
import json
import os
import subprocess
...
def conn():
    _psql("032_outdoor_bus_template_down.sql")
    _psql("032_outdoor_bus_template.sql")
...
def test_template_bound(conn):
    ...
    assert sec == SEC and leaf == "outdoor_bus_structure", f"{sec}/{leaf}"
...
def test_coverage_invariant(conn):
    ...
    assert len(required) == 10, f"expected 10, got {len(required)}"
...
def test_ir_and_dielectric_neta_tables(conn):
    ...
    assert tv.get("tolerance_source", {}).get("table") == "100.19"
