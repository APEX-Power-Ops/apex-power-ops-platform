...
`test_008_backfill.py` (8 tests); the Chip 2b datasheet by
`test_010_lv_cb_template.py` (17 tests — incl. the NETA 7.6.1.2 coverage invariant) —
all on local `records_dev`.
...
> (`008`→`neta_procedures`, `010`→`asset_classes`), an in-file-order combined run tears
> tables down out of dependency order and corrupts the dev DB. If that happens, reset in
> reverse order — `010_down → 008_down → 005_down` — then re-apply `005→006→008→009→010`.
...
- **Data seed** — the NETA reference layer (procedures / test-items / acceptance
  ...
  procedure xref graph** in **Chip 2-backfill** (008/009). The **first datasheet
  `field_schema`** (LV circuit breaker, `ats_lv_cb_v1`) lands in **Chip 2b** (010) —
  referencing + filtering the leaf's linked NETA items with a coverage invariant; see
  `reference/records/04-LV-CB-DATASHEET-SPEC.md`. Remaining family datasheets follow as
  Chip 2b repeats.
