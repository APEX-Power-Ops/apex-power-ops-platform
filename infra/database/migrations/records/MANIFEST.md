...
| 6 | `006_neta_reference_seed.sql` | **Chip 2a** — accurate seed from the NETA master equipment table (ANSI/NETA ATS-2025 / MTS-2023): **72 procedures · 3,920 test items · 43 acceptance tables**. GENERATED by `gen_neta_seed.py`; idempotent (`ON CONFLICT`), deterministic UUID5 ids | Requires 005 |
| 7 | `007_asset_class_shell.sql` | **Chip 2-shell** — the `asset_classes` family taxonomy (2-level): **27 NETA-category parents** (19 active + 8 future) + **36 practical leaf classes**. Parents carry `neta_category` (the anchor into `neta_procedures`); leaves carry templates/assets later. GENERATED by `gen_shell_seed.py`; idempotent, deterministic UUID5 | Requires 002 (+ 005 for the anchor) |

Each file has a matching `_down.sql` that reverses it in dependency order. Chip 2a
is validated by `test_005_neta_reference.py` (7 tests); the Chip 2-shell family
taxonomy by `test_007_asset_class_shell.py` (11 tests) — both on local `records_dev`.
