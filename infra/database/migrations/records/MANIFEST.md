...
taxonomy by `test_007_asset_class_shell.py` (13 tests) — both on local `records_dev`.
...
- **Data seed** — the NETA reference layer (procedures / test-items / acceptance
  ...
  2-level NETA-anchored shell) in **Chip 2-shell** (007). Datasheet `field_schema`
  templates = **Chip 2b** (they reference + filter the NETA layer onto the leaf
  classes; the NETA-vs-common-datasheet divergence is then a query, not a copy).
