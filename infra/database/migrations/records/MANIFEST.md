...
is validated by `test_005_neta_reference.py` (7 tests, local `records_dev`).
...
- **Data seed** — the NETA reference layer (procedures / test-items / acceptance
  tables) is seeded in **Chip 2a** (005/006). Datasheet `field_schema` templates +
  the asset-class catalog = **Chip 2b** (they reference + filter the NETA layer; the
  NETA-vs-common-datasheet divergence is then a query, not a copy).
