"""TDD - records Chip 2c: Circuit Switcher datasheet (NETA 7.7).

Slice-3 coverage backlog. Leaf 'circuit_switcher' already exists (Chip 2-shell). NETA 7.7 = 13 VM +
11 electrical = 24 items. R-A = neta_table (IR->100.1) + mfr (contact/connection/coil/dielectric/PF).
Capture = field + cover_attach.

RED until gen_circuit_switcher_template.py emits 035_circuit_switcher_template.sql. Run PER-CHIP.
"""
import json
import os
import subprocess
...
def conn():
    _psql("035_circuit_switcher_template_down.sql")
    _psql("035_circuit_switcher_template.sql")
...
def test_template_bound(conn):
    ...
    assert sec == SEC and leaf == "circuit_switcher", f"{sec}/{leaf}"
...
def test_coverage_invariant(conn):
    ...
    assert len(required) == 24, f"expected 24, got {len(required)}"


def test_structure_and_capture(conn):
    _, fs, _ = _schema(conn)
    keys = [s["key"] for s in fs["sections"]]
    for k in ("identification", "visual_mechanical", "insulation_resistance", "contact_resistance",
              "coil_pickup", "power_factor", "attachments"):
