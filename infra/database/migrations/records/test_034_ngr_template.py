"""TDD - records Chip 2c: Neutral-Grounding Resistor datasheet (NETA 7.20.4 Resistors).

Slice-3 coverage backlog. Leaf 'ngr' already exists (Chip 2-shell). NETA 7.20.4 = 11 VM + 4
electrical = 15 items (A.9 / B.4 = instrument-transformer cross-refs to 7.10). R-A = neta_table
(IR->100.1) + mfr (resistance value, bolted). Capture = field + cover_attach.

RED until gen_ngr_template.py emits 034_ngr_template.sql. Run PER-CHIP.
"""
import json
import os
import subprocess
...
def conn():
    _psql("034_ngr_template_down.sql")
    _psql("034_ngr_template.sql")
...
def test_template_bound(conn):
    ...
    assert sec == SEC and leaf == "ngr", f"{sec}/{leaf}"
...
def test_coverage_invariant(conn):
    ...
    assert len(required) == 15, f"expected 15, got {len(required)}"


def test_structure_and_capture(conn):
    _, fs, _ = _schema(conn)
    keys = [s["key"] for s in fs["sections"]]
    for k in ("identification", "visual_mechanical", "insulation_resistance", "resistance_value",
              "it_xref", "attachments"):
