"""TDD - records Chip 2c: Load Tap-Changer datasheet (NETA 7.12.3).

Slice-3 coverage backlog. Leaf 'load_tap_changer' already exists (Chip 2-shell). NETA 7.12.3 = 14 VM
+ 13 electrical = 27 items (incl. vacuum-interrupter MAC + bottle dielectric). R-A = neta_table
(IR->100.5 via 7.2.2) + mfr (winding-R / PF/DF / turns-ratio / reactance / vacuum / excitation) +
standard-basis liquid. Capture = field + cover_attach.

RED until gen_load_tap_changer_template.py emits 037_load_tap_changer_template.sql. Run PER-CHIP.
"""
import json
import os
import subprocess
...
def conn():
    _psql("037_load_tap_changer_template_down.sql")
    _psql("037_load_tap_changer_template.sql")
...
def test_template_bound(conn):
    ...
    assert sec == SEC and leaf == "load_tap_changer", f"{sec}/{leaf}"
...
def test_coverage_invariant(conn):
    ...
    assert len(required) == 27, f"expected 27, got {len(required)}"


def test_vacuum_section_covers_mac_and_dielectric(conn):
    """The vacuum_integrity section satisfies both B.10 (MAC) and B.11 (bottle dielectric)."""
    _, fs, _ = _schema(conn)
    vi = next(s for s in fs["sections"] if s["key"] == "vacuum_integrity")
    assert {f"{SEC}.B.10", f"{SEC}.B.11"} <= set(vi.get("neta_covers", []))
...
def test_structure_and_capture(conn):
    ...
    for k in ("identification", "visual_mechanical", "insulation_resistance", "turns_ratio",
              "winding_resistance", "vacuum_integrity", "excitation", "attachments"):
