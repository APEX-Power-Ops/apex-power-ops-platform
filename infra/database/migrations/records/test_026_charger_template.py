"""TDD - records Chip 2c: Battery Charger datasheet (NETA 7.18.2).

Slice-3 coverage backlog; companion to the battery sheet (025). Leaf 'battery_charger' already exists.
NETA 7.18.2 = 8 VM + 9 electrical = 17 items (float/equalize, hv-shutdown, load-sharing, ripple,
full-load/current-limit; meter calibration cross-refs 7.11). Capture = field + cover_attach.

RED until gen_charger_template.py emits 026_charger_template.sql. Run PER-CHIP.
"""
import json
import os
import subprocess
...
def conn():
    _psql("026_charger_template_down.sql")
    _psql("026_charger_template.sql")
...
def test_template_bound(conn):
    ...
    assert sec == SEC and leaf == "battery_charger", f"{sec}/{leaf}"
...
def test_structure_and_capture(conn):
    ...
    for k in ("identification", "visual_mechanical", "charger_settings", "output_measurements", "attachments"):
