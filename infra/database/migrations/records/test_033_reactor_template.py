"""TDD - records Chip 2c: Dry-Type Reactor datasheet (NETA 7.20.3.1).

Slice-3 coverage backlog. Leaf 'reactor' already exists (Chip 2-shell). NETA 7.20.3.1 = 8 VM + 5
electrical = 13 items. NETA lists PF/DF in BOTH the VM (A.5) and electrical (B.5) lists - one
power_factor section covers both. R-A = neta_table (IR->100.1) + mfr (winding-R/PF-DF/dielectric/
bolted). Capture = field + cover_attach.

RED until gen_reactor_template.py emits 033_reactor_template.sql. Run PER-CHIP.
"""
import json
import os
import subprocess
...
def conn():
    _psql("033_reactor_template_down.sql")
    _psql("033_reactor_template.sql")
...
def test_template_bound(conn):
    ...
    assert sec == SEC and leaf == "reactor", f"{sec}/{leaf}"
...
def test_pf_section_covers_both_vm_and_electrical(conn):
    """The single power_factor section must satisfy NETA's dup-listed A.5 + B.5."""
    _, fs, _ = _schema(conn)
    pf = next(s for s in fs["sections"] if s["key"] == "power_factor")
    assert {f"{SEC}.A.5", f"{SEC}.B.5"} <= set(pf.get("neta_covers", []))


def test_structure_and_capture(conn):
    _, fs, _ = _schema(conn)
    keys = [s["key"] for s in fs["sections"]]
    for k in ("identification", "visual_mechanical", "insulation_resistance", "winding_resistance",
              "power_factor", "attachments"):
