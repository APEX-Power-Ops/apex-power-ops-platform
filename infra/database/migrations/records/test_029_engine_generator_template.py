"""TDD - records Chip 2c: Engine Generator datasheet (NETA 7.22.1).

Slice-3 coverage backlog. Leaf 'engine_generator' already exists (Chip 2-shell). NETA 7.22.1
(Emergency Systems) = 4 VM + 7 electrical = 11 items. R-A = mfr (IR/DA per IEEE 43 - NOT a NETA
table; performance per NFPA 110). Capture = field + cover_attach.

RED until gen_engine_generator_template.py emits 029_engine_generator_template.sql. Run PER-CHIP.
"""
import json
import os
import subprocess
...
def conn():
    _psql("029_engine_generator_template_down.sql")
    _psql("029_engine_generator_template.sql")
...
def test_template_bound(conn):
    ...
    assert sec == SEC and leaf == "engine_generator", f"{sec}/{leaf}"
...
def test_structure_and_capture(conn):
    ...
    for k in ("identification", "visual_mechanical", "insulation_resistance", "engine_protection",
...
def test_ir_acceptance_is_ieee43(conn):
    """Rotating-machine IR resolves to IEEE 43 (mfr engine) - never tcc/neta_table."""
    _, fs, _ = _schema(conn)
    ir = next(s for s in fs["sections"] if s["key"] == "insulation_resistance")
    r60 = next(c for c in ir["fields"] if c["tag"] == "ir_60s_mohm")
    assert r60.get("acceptance", {}).get("basis") == "ieee_43"
    assert r60.get("tolerance_source", {}).get("engine") == "mfr"
