"""TDD - records Chip 2c: Storage Battery datasheet (NETA 7.18.1.3, VRLA).

Slice-3 coverage backlog. Leaf 'battery' already exists. Built for VRLA (7.18.1.3 - the dominant modern
stationary battery): 10 VM + 8 electrical = 18 items. Flooded-LA (7.18.1.1) / vented-NiCd (7.18.1.2)
folds are a clean follow-on (same recipe + a battery_type selector). The per-cell battery (cell voltage,
intercell resistance, internal ohmic) is a grow table. Capture = field + cover_attach.

RED until gen_battery_template.py emits 025_battery_template.sql. Run PER-CHIP.
"""
import json
import os
import subprocess
...
def conn():
    _psql("025_battery_template_down.sql")
    _psql("025_battery_template.sql")
...
def test_template_bound(conn):
    ...
    assert sec == SEC and leaf == "battery", f"{sec}/{leaf}"
...
def test_coverage_invariant(conn):
    ...
    assert len(required) == 18, f"expected 18, got {len(required)}"


def test_structure_and_capture(conn):
    _, fs, _ = _schema(conn)
    keys = [s["key"] for s in fs["sections"]]
    for k in ("identification", "installation_safety", "visual_mechanical", "cell_data",
              "load_test", "ground_voltage", "attachments"):
