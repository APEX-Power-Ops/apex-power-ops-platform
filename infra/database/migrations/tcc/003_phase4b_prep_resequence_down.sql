-- =============================================================================
-- Decision-012 - Phase 4b PREP DOWN: restore prior shared sequence ownership.
-- Reversible sequence metadata only. Valid before the terminal Phase 4b drop.
-- =============================================================================
...
BEGIN
  ...
  IF seq_oid IS NULL THEN
    RAISE EXCEPTION 'ABORT: expected sequence tcc.etu_sensor_maint_id_seq was not found.';
  END IF;

  IF to_regclass('public.tcc_etu_sensor_maint_id_seq') IS NOT NULL THEN
    RAISE EXCEPTION 'ABORT: target sequence public.tcc_etu_sensor_maint_id_seq already exists.';
  END IF;
  ...
  IF to_regclass('public.tcc_etu_sensor_maint_pre_rebuild') IS NULL THEN
    RAISE EXCEPTION 'ABORT: public.tcc_etu_sensor_maint_pre_rebuild is required for DOWN ownership restore.';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'ABORT: tcc.etu_sensor_maint.id default does not depend on tcc.etu_sensor_maint_id_seq.';
  END IF;
END $$;

ALTER SEQUENCE tcc.etu_sensor_maint_id_seq OWNED BY NONE;
ALTER SEQUENCE tcc.etu_sensor_maint_id_seq RENAME TO tcc_etu_sensor_maint_id_seq;
ALTER SEQUENCE tcc.tcc_etu_sensor_maint_id_seq SET SCHEMA public;
ALTER SEQUENCE public.tcc_etu_sensor_maint_id_seq OWNED BY public.tcc_etu_sensor_maint_pre_rebuild.id;
...
BEGIN
  ...
  IF seq_oid IS NULL THEN
    RAISE EXCEPTION 'ABORT: expected sequence public.tcc_etu_sensor_maint_id_seq was not found after DOWN.';
  END IF;

  IF to_regclass('tcc.etu_sensor_maint_id_seq') IS NOT NULL THEN
    RAISE EXCEPTION 'ABORT: tcc.etu_sensor_maint_id_seq still exists after DOWN.';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'ABORT: tcc.etu_sensor_maint.id default lost its sequence dependency after DOWN.';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'ABORT: public.tcc_etu_sensor_maint_pre_rebuild.id default lost its sequence dependency after DOWN.';
  END IF;
END $$;
