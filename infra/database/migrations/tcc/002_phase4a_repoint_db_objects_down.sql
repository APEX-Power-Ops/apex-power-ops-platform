-- =============================================================================
-- Decision-012 - Phase 4a DOWN: restore captured original DB object bodies.
-- Captured from live with pg_get_functiondef/pg_get_viewdef before applying UP.
-- =============================================================================
...
BEGIN
  ...
  SELECT
    ...
    INTO ovr
    FROM public.tcc_etu_stpu_overrides o
...
CREATE OR REPLACE VIEW public.vw_trip_unit_cascade AS
 WITH trip_type_v2 AS (
         SELECT tt_1.id AS trip_type_id,
            tt_1.name AS trip_type_name,
            mv2.id AS manufacturer_id_v2
           FROM tcc.trip_types tt_1
             JOIN tcc_manufacturers_pre_rebuild m1 ON m1.id = tt_1.manufacturer_id
             JOIN tcc.manufacturers mv2 ON mv2.mfr_name::text = m1.name::text
        )
 SELECT m.id AS manufacturer_id,
   ...
   FROM tcc.manufacturers m
     ...
     LEFT JOIN trip_type_v2 tt ON tt.manufacturer_id_v2 = m.id AND tt.trip_type_name::text = ts.type::text;
