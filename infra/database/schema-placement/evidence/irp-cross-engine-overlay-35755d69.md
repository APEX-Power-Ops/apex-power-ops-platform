...
## ROUND 5 — 2nd PLAN re-audit + narrow re-audit (2026-07-12)

**Operator 2nd cross-engine plan audit of rev3 (`aa3af441`): plan NOT build-ready — 1 HIGH + 4 MED** (all NEW correctness/coverage gaps, not regressions; the original ten corrections verified materially present; unconditional-loader layering re-affirmed). All folded → **rev4 `9e972eaf`**:
- **F1 (High):** schema-invalid signed overlays could crash the loader (the loop continued past an OV008 into dict-shaped access — a signed JSON array or malformed `assignments` would raise `AttributeError`). FIX: `isinstance(doc, dict)` guard + `if schema_diags: continue`; `signed_non_object_overlay_OV008` + `signed_malformed_assignments_OV008` tests.
- **F2 (Med):** "missing in_data_api → SP027" was impossible (`_gate_required_dims` includes in_data_api for delete, so OV015 short-circuits first). RATIFIED coherent split: **OV015** (missing) / **SP027** (observed-true) / **OV022** (observed-false, inadequate window). Matrix + Task-5 unit + Task-8 e2e updated.
- **F3 (Med):** the principal `_e2e_red_then_green` was a passing placeholder (`... return True`) — GREEN before implementation. FIX: real `cd.main` RED→GREEN harden test + `_e2e_ov021_via_main` (real); the delete/SP018/retain e2e cases are explicit implementer guidance (not stubs).
- **F4 (Med):** the coded unresolved-`$ref` path was not actually exercised. FIX: real `validate_overlay(<unseeded-$ref validator>)` → asserts exactly `["OV008"]` (no network, no uncaught `Unresolvable`).
- **F5 (Med):** `producing_repo_sha` applicability too permissive. FIX: three categories — required (platform_config / static_repo) · forbidden+reason (advisor / runtime_logs / operator_declaration) · conditional (external_clients) — with positive + negative tests.

**Narrow re-audit (operator instruction) over the five areas — cross-engine.** Claude self-review clean (traced the real e2e green path: in_data_api + static_repo overlays, `sha:`-scheme consumer refs to keep SP014 from treating them as paths, `consumer_disposition: unresolved` to skip SP022). Codex (`codex exec review --base main`, `-m gpt-5.5`, sandbox-bypassed) = **1 P2 (doc-coherence):** the `check_delete_floor_coherence` docstring + the `load_and_merge` in_data_api comment + the round-4 fold row still said a *missing* overlay "defers to SP027," contradicting the rev-4 OV015-for-missing split. FIX → **rev4.1** (this commit): docstring/comment/record aligned to **OV015 (missing) / SP027 (observed-true) / OV022 (observed-false)**. The CODE was already correct — a pure prose-vs-contract mismatch that would have misled the implementer.

**Cross-engine delta:** Codex caught the residual "missing→SP027" prose the rev-4 edits left in the docstring/comment/record; Claude's grounding confirmed all five folds + the real e2e green-path trace. Plan convergence: 9 → 2 → 5 → 1(doc) → 0. **Verdict: BUILD-READY.** Implementation, evidence collection, DB access, production, A1–A3, and the apply runner remain HELD.

---

## ROUND 6 — 3rd PLAN re-audit + narrow diff review (2026-07-12)

**Operator 3rd cross-engine plan audit of rev4.1 (`5ea67cb4`): plan NOT build-ready — 1 HIGH + 1 MED + 1 LOW** (verified closed: invalid-overlay short-circuit, the coherent OV015/SP027/OV022 routing, the primary RED→GREEN CLI test, unseeded-`$ref`→OV008, three producing-sha categories, branch clean, holds correct). All folded → **rev5 `<this commit>`**:
- **F1 (High):** the read-once contract was incomplete — `disposition.schema.json` was read three times (`_validator()` for raw, `load_overlay_contract()` for overlays/hashes, `_validator()` again for the effective view), so a concurrent schema edit could make raw validation, OV020 binding, and effective validation see different bytes. FIX: `OverlayContract` now holds **both** validators (`disposition_validator` + `overlay_validator`) built from the same in-hand bytes; the contract is loaded **once** immediately after SP026; `main()` uses `contract.disposition_validator` for raw + effective and `contract.overlay_validator` for overlays. The overlay-enabled CLI path **never calls `_validator()`**.
- **F2 (Med):** the delete-routing + retain e2e cases were still prose. FIX: six named, registered, real `cd.main` tests — `_e2e_delete_missing_in_data_api_OV015`, `_e2e_delete_true_in_data_api_SP027`, `_e2e_delete_false_noncovering_OV022`, `_e2e_retain_no_overlay_OV018`, `_e2e_retain_green`, `_e2e_unaccepted_manifest_SP018_no_receipt` (delete cases reuse `tcd._recovery_artifact()` + `tcd._ROOT` for SP014 + the full SP027/SP022 consumer set; each varies only the `in_data_api` overlay). Plus a `_capture_main` `SystemExit` guard so the tests report clean per-test RED before implementation.
- **F3 (Low):** the self-review placeholder scan was stale (still called `_e2e_red_then_green` a placeholder). FIX: corrected — no placeholders remain.

**Narrow diff review (operator instruction) over just two properties:**
1. **No `_validator()` in the overlay-enabled CLI path** — PROVEN: `main()` loads the contract once and passes `contract.disposition_validator` to both `validate_documents(raw_docs, …)` and `run(effective, …)`; a grep of the CLI wiring shows no `_validator()` call (it is retained only for the standalone `run()`-level tests).
2. **All six new e2e tests start RED before implementation** — PROVEN by construction: (a) `tests/test_overlay_loader.py` imports `disposition_overlay`, which does not exist until Task 1 → `ModuleNotFoundError` → suite RED; (b) even with the module, `cd.main` lacks `--overlay`/`--max-consumer-evidence-age-hours` until Task 8 → argparse `sys.exit(2)` → `_capture_main` returns `rc=2` → each test asserts `rc==1`/`rc==0` → FAIL. Both guarantee RED before the corresponding task lands.

Plan convergence: **9 → 2 → 5 → 3 → 0** over four plan-audit rounds. **Verdict: BUILD-READY.** Per the operator's ruling, a clean narrow diff review authorizes Subagent-Driven Development without another broad plan audit. Implementation, evidence collection, DB access, production, A1–A3, and the apply runner remain HELD.

---

## ROUND 7 — WHOLE-BRANCH BUILD IRP (SDD complete; cross-engine) — 2026-07-12

The 9-task Subagent-Driven build of the rev-5 plan is complete on `schema-placement/signed-overlay` (27 code/CI commits ahead of main `7c9a97ca`, tip `6696c63b`). Every task got an adversarial per-task spec+quality review (opus on the high-risk tasks) with RED→GREEN fixes; this is the final whole-branch cross-engine pass before the operator-gated squash PR.

**Mandatory verification (Task 9):** all 8 governed suites green (`test_overlay_schema/test_overlay_loader/test_check_disposition/test_collect_disposition/test_disposition_schema/test_disposition_trust/test_verify_census/test_disposition_provenance`); committed-census CI gate = "no census artifacts added on this branch — nothing to verify" (the packet touches no census/schema/collector/trust-anchor); every `OV001–OV022` has ≥3 pinned test refs.

**Claude grounded whole-branch review (opus, live host at 6696c63b): READY TO MERGE.** No Critical/Important. Fail-closed ordering verified end-to-end in `main()` (SP028 → read-bytes-once → SP026 → contract-once → raw SP001 → OV021 → load_and_merge → run(effective) → receipt; every RED path returns before the receipt). All security invariants hold (acyclic DAG; read-once contract, `_validator()` never called on the overlay path; census deepcopy-immutability; signed-bytes integrity; §2A evidence-readiness≠authorization). Delete 3-way split (OV015/SP027/OV022) coherent, no gap/no double-fire. Integration-honest: the provenance-conditional SP009 marker set == the relations semantic_check windows; per-overlay rejects short-circuit before the effective-view build; no diagnostic emitted-then-dropped; receipt green-only (SP018→no receipt).

**Codex cross-engine (`codex exec review --base main -m gpt-5.5`, sandbox-bypassed; exit 0):** "The changed overlay loader, CLI integration, schema contract, and governed tests appear internally consistent. The … offline suites … pass against the patch." Codex independently traced `check_target` (OV019/OV012 IFF, three producing_repo_sha categories), `check_delete_floor_coherence` (OV022 covering predicate + defer), and the `check_disposition` SP027 destructive-evidence floor + external_clients waiver — and raised **no findings**.

**Cross-engine delta:** none — both engines converge on READY with zero Critical/Important. Codex's independent trace of the SP027↔OV022 delete-floor seam corroborates the opus review's coherence finding (the two engines examined the same seam from different entry points and agreed). No engine surfaced a defect the other missed.

**Carried items — all CLOSED:** (1) Task-4 `derive_windows` KeyError → structurally impossible (`cluster_src_oids` built under `if oid in eff_index`); (2) SP007 over the effective view → `semantic_check` flags an absent `source_object` on the deep-copied view; (3) OV022 delete e2e → six named `cd.main` tests; (4) date-time FormatChecker → `rfc3339-validator==0.1.4` pinned + a regression tripwire (`_validate_overlay_calendar_invalid_datetime_OV008`); (5) audit-#9 contributor-map leak → `contrib` local, effective view passes `run()`'s `additionalProperties:false` SP001.

**Deferrable cosmetic Minors (none block merge):** no `main()`-level OV016 recency-flag e2e (loader-level coverage exists); OV010 `except` omits `AttributeError` (unreachable — captured_at is schema-valid before that point); `check_delete_floor_coherence` unused `effective` param (spec-mandated pipeline-symmetry signature); `validate_overlay` catches `Unresolvable` but not `CannotDetermineSpecification` (unreachable at doc-validation; caught at contract build). PLAN-DOC drift (not code): the plan's `_harden_docs` example used `consumer_disposition:"unresolved"`, schema-invalid for accepted-harden; the build correctly adapted the TEST fixture to `"no_consumer"`+overlays (a stricter path).

**VERDICT: BUILD COMPLETE — READY TO MERGE (operator-gated).** Merge to main, evidence collection, DB access, A1–A3, overlay-apply, cluster migrations, and the apply runner all remain HELD. Merging the overlay tooling PRECEDES any evidence collection (spec §10).
