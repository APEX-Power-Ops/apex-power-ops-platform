...
def build_report(conn, run_id, generated_at=None, snapshot_id=None):
    ...
    Returns:
        {
          "provenance": {
            ...
            "host": str or None,
            "governed_source_db": str,     -- authoritative D4/D5 source (tcc_fidelity_governed)
            "tcc_snapshot_db": str or None, -- Phase-1 host TCC snapshot db_name (informational)
            "role": str or None,
    ...
    provenance = {
        ...
        "host": snap_host,
        "governed_source_db": GOVERNED_DB,
        "tcc_snapshot_db": snap_db,
        "role": snap_role,
...
def _provenance_header(report):
    ...
    lines = [
        ...
        "-- governed_source_db:  %s" % p.get("governed_source_db", "?"),
        "-- tcc_snapshot_db:     %s" % p.get("tcc_snapshot_db", "?"),
