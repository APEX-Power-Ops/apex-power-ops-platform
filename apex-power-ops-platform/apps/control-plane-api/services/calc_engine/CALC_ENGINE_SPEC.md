...
**Purpose:** Historical VS Code Claude handoff document for the original `services/calc_engine/` build-out before calc-domain extraction into `packages/calc-engine`
...
**Authority note:** This document is a backend-local lineage handoff, not the governing source of TCC architecture truth. Validate scope and claims against the current platform authority stack and the paired source-domain materials under `C:/APEX Platform/source-domains/neta-ett-study-material/` before treating any section here as active direction.
