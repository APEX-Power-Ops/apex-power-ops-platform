...
|------|-------|---------------|--------------------|-----------------------------|----------------------------|
| Olares developer residency and operator hardening | Active governing migration lane | The governing direction is now explicit: all Apex Ops work should converge on Olares-resident governance, execution, validation, and operator context; the laptop remains client-only and the current Windows parent-root publication boundary is transitional rather than permanent. | Select and execute the next bounded dependency that retires split-residency practice without changing GitHub canonical status by default. | Do not reintroduce laptop-first durable state, conflicting workspace methods, or silent publication-boundary exceptions without a separate packet. | `Olares host bootstrap status` or `bash apex-power-ops-platform/tools/ai/run-olares-host-bootstrap-status.sh` |
...
2. transitional parent-root git publication from `C:/APEX Platform`,
