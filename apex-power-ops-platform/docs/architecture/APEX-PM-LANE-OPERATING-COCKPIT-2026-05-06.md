...
|------|-------|---------------|--------------------|-----------------------------|----------------------------|
| Olares developer residency and operator hardening | Active governing migration lane | The governing direction is explicit and the remaining publication-boundary retirement dependencies are now inventoried; the laptop remains client-only and the current Windows parent-root publication boundary is still transitional rather than permanent. | Select the first retirement target from the dependency inventory, starting with the host-native operator publication workflow. | Do not reintroduce laptop-first durable state, conflicting workspace methods, or silent publication-boundary exceptions without a separate packet. | `Olares host bootstrap status` or `bash apex-power-ops-platform/tools/ai/run-olares-host-bootstrap-status.sh` |
