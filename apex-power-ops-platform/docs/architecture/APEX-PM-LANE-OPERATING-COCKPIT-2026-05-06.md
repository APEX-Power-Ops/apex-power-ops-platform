...
|------|-------|---------------|--------------------|-----------------------------|----------------------------|
| Olares developer residency and operator hardening | Active governing migration lane | The build-guide stack is now locally refreshed to match Olares-first execution, GitHub-canonical publication, client-only laptop posture, premium-plan-first AI use, optional local models, and the admitted minimal MCP trio. Publication and host parity are still required before this slice is durable. | Execute the Packet 091 publication/resync gate so the refreshed guidance is published and `/home/olares/code/apex` returns to clean parity. | Do not reintroduce laptop-first durable state, mandatory Ollama assumptions, broader AI-service scope, or silent publication-boundary exceptions without a separate packet. | `Olares host bootstrap status` or `bash apex-power-ops-platform/tools/ai/run-olares-host-bootstrap-status.sh` |
