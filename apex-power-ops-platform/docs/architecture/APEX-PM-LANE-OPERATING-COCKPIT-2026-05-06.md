...
|------|-------|---------------|--------------------|-----------------------------|----------------------------|
| Olares developer residency and operator hardening | Active governing migration lane | The build-guide stack and visual architecture reference are now published in line with Olares-first execution, GitHub-canonical publication, client-only laptop posture, premium-plan-first AI use, optional local models, and the admitted minimal MCP trio. | Reopen only if adjacent authority, operator, or visual drift still creates concrete mismatch. | Do not reintroduce laptop-first durable state, mandatory Ollama assumptions, broader AI-service scope, or silent publication-boundary exceptions without a separate packet. | `Olares host bootstrap status` or `bash apex-power-ops-platform/tools/ai/run-olares-host-bootstrap-status.sh` |
