...
Use this packet to introduce the bounded `pm-schema-020f` real XER host parser verification and golden fixture admission singleton:
...
1. it introduces only the host verification and fixture-admission slice and does not widen into later UI driver work, handoffs, `knowledge`, or `archive`
...
1. re-evaluate the first dependency-safe post-`020f` packet in the remaining driver and parser-reconciliation chain
2. if the `020g`/`020e.*` substrate is still unpublished, queue the earliest dependency-safe packet there before returning to `pm-schema-ui-002e`