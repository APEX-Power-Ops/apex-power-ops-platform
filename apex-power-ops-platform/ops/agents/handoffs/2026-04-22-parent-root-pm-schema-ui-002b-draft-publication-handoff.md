...
Use this packet to introduce the bounded `pm-schema-ui-002b` read-only Gantt prototype implementation singleton:
...
1. it introduces only the next dependency-safe read-only Gantt tranche and does not widen into later baseline-overlay or schedule-driver slices
...
1. re-evaluate the remaining `pm-schema-ui-002*` implementation family in dependency-safe order starting from `pm-schema-ui-002c`
