...
| TASK-023 | Assess the intended services-zone stack for AI orchestration and supporting tools on Olares, including Ollama, Open WebUI, Dify, n8n, Qdrant, Syncthing, Restic, and optional Gitea, and classify each as current-state, design-intent, deferred, or not yet governed. Step 3 published the classification matrix but left this task open. Packet 004 restored SSH access, so live host inventory is no longer access-blocked. The next bounded move is Packet `2026-05-03-olares-phase-5-005-ssh-host-runtime-inventory.json`; until that packet runs, the full Docker, K3s or Helm, installed-app, ports, volumes, networks, and timer inventory remains uncollected; the named gating file `docs/architecture/GIT-HOSTING-AND-GITEA-TRANSITION-CHECKLIST-2026-04-23.md` does not exist on disk in this workspace snapshot; bare-systemd Restic posture must still be reconciled with the intended Market-app Restic posture in writing. |  |  |
...
| TASK-025 | Assess whether a future Olares One dev-workspace expansion should be treated as: (a) workstation-only migration, (b) AI-services-zone expansion, (c) Gitea or code-hosting mirror enhancement, or (d) a broader canonical-hosting transition, and explicitly keep those decisions split instead of collapsing them into one “move to Olares” bucket. Step 3 preserved the four-way split and classified per-path readiness; Packet 004 improved path (a) from access-blocked to technically SSH-reachable, but none of (a), (b), (c), or (d) is currently READY. Per-path missing evidence remains named in `ops/agents/handoffs/2026-05-03-olares-phase-5-step-3-expansion-decision-surface-handoff.md` and must be reconciled after host inventory. |  |  |
...
- **FILE-027**: `ops/agents/handoffs/2026-05-03-olares-phase-5-next-task-and-prompt-routing-handoff.md` - copy-paste routing surface now updated through Packet 004 completion; the next live move is execution of Packet `2026-05-03-olares-phase-5-005` over the restored mesh SSH path, with Packet 004B retained only as fallback if mesh SSH regresses.
...
- **FILE-035**: `ops/agents/packets/draft/2026-05-03-olares-phase-5-005-ssh-host-runtime-inventory.json` - authored next execution packet for read-only host runtime inventory over the restored mesh SSH path, including Docker, K3s or Helm, installed-app, private-lane timer, and host repo-clone evidence without host mutation.
