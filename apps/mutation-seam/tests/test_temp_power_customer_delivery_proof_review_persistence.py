...
from app.temp_power_customer_delivery_proof_review_persistence import (
    ...
    TEMP_POWER_CUSTOMER_DELIVERY_PROOF_REVIEW_ACTION_TYPE,
    TEMP_POWER_PROJECT_ID,
    TEMP_POWER_SOURCE_FINGERPRINT,
    build_temp_power_customer_delivery_proof_review_id,
    classify_temp_power_customer_delivery_proof_review_record,
...
MIGRATION_PATH = Path(__file__).resolve().parents[1] / "migrations" / "010_pm_lane_329_customer_delivery_proof_reviews.sql"
...
def _payload(**overrides) -> dict:
    payload = {
        ...
        "candidate_id": TEMP_POWER_CANDIDATE_ID,
        "source_fingerprint": TEMP_POWER_SOURCE_FINGERPRINT,
        "customer_preview_review_id": overrides.pop("customer_preview_review_id"),
...
def test_temp_power_customer_delivery_proof_review_route_persists_one_insert_only_review_and_readback(client):
    _seed_temp_power_scope()
    preview_review_id = _seed_customer_preview_review()
    before_counts = _domain_counts()

    missing = load_temp_power_customer_delivery_proof_review_status()
    assert missing["status"] == "no_customer_delivery_proof_review_record"
    assert missing["storage_route_registered"] is True

    response = client.post(
        "/api/v1/mutations/temp-power-customer-delivery-proof-reviews",
        json=_request(_payload(customer_preview_review_id=preview_review_id)),
        headers=_token(),
    )
    ...
    assert data["entity_type"] == "pm_customer_delivery_proof_review"
    assert data["action_type"] == TEMP_POWER_CUSTOMER_DELIVERY_PROOF_REVIEW_ACTION_TYPE
    ...
    assert data["new_state"]["delivery_proof_recorded"] is True
    assert data["new_state"]["proof_recorded_at_utc"] == data["new_state"]["pm_reviewed_at"]
    assert data["new_state"]["mutation_id"] == data["mutation_id"]
    assert data["new_state"]["audit_event_id"] == data["audit_event_id"]
    assert data["entity_id"] in store.temp_power_customer_delivery_proof_reviews
    assert len(store.temp_power_customer_delivery_proof_reviews) == 1
    assert len(store.audit_log) == 1
    assert store.audit_log[0]["entity_type"] == "pm_customer_delivery_proof_review"
    assert _domain_counts() == before_counts

    status = client.get("/api/v1/reads/temp-power-customer-delivery-proof-status", headers=_token()).json()
    ...
    assert status["delivery_proof_recorded"] is True


def test_temp_power_customer_delivery_proof_review_route_replays_identical_payload_without_second_insert(client):
    _seed_temp_power_scope()
    preview_review_id = _seed_customer_preview_review()
    request = _request(_payload(customer_preview_review_id=preview_review_id))

    first = client.post("/api/v1/mutations/temp-power-customer-delivery-proof-reviews", json=request, headers=_token())
    second = client.post("/api/v1/mutations/temp-power-customer-delivery-proof-reviews", json=request, headers=_token())
...
def test_temp_power_customer_delivery_proof_review_readback_classifies_stale_lineage_and_followup_records_without_audit_dependency(client):
    _seed_temp_power_scope()
    preview_review_id = _seed_customer_preview_review()
    response = client.post(
        "/api/v1/mutations/temp-power-customer-delivery-proof-reviews",
    ...
    stale_record = dict(store.temp_power_customer_delivery_proof_reviews[accepted["entity_id"]])
    stale_record["source_fingerprint"] = "stale-source-fingerprint"
    stale = classify_temp_power_customer_delivery_proof_review_record(stale_record)
    assert stale["status"] == "customer_delivery_proof_review_recorded_stale_source"
    assert stale["current_candidate_match"] is True
    assert stale["current_source_fingerprint_match"] is False

    lineage_mismatch_record = dict(store.temp_power_customer_delivery_proof_reviews[accepted["entity_id"]])
    lineage_mismatch_record["preview_artifact_lineage"] = ["preview://temp-power/2026-05-18/review-bundle-mismatch"]
    mismatch = classify_temp_power_customer_delivery_proof_review_record(lineage_mismatch_record)
    assert mismatch["status"] == "customer_delivery_proof_review_lineage_mismatch"
    assert mismatch["preview_review_lineage_match"] is False

    followup_record = dict(store.temp_power_customer_delivery_proof_reviews[accepted["entity_id"]])
    followup_record["pm_delivery_approval_status"] = "PENDING_PM_FOLLOWUP"
    followup = classify_temp_power_customer_delivery_proof_review_record(followup_record)
    assert followup["status"] == "customer_delivery_proof_pending_pm_followup"


def test_temp_power_customer_delivery_proof_review_route_rejects_blocked_fields_invalid_context_and_lineage(client):
    _seed_temp_power_scope()
    preview_review_id = _seed_customer_preview_review()

    blocked = client.post(
        "/api/v1/mutations/temp-power-customer-delivery-proof-reviews",
        json=_request(_payload(customer_preview_review_id=preview_review_id, finance_export_recorded=True)),
        headers=_token(),
    )
    ...
    stale_source = client.post(
        "/api/v1/mutations/temp-power-customer-delivery-proof-reviews",
        json=_request(_payload(customer_preview_review_id=preview_review_id, source_fingerprint="stale-source-fingerprint")),
        headers=_token(),
    )
    lineage_mismatch = client.post(
        "/api/v1/mutations/temp-power-customer-delivery-proof-reviews",
        json=_request(
            _payload(
                customer_preview_review_id=preview_review_id,
                preview_artifact_lineage=["preview://temp-power/2026-05-18/review-bundle-mismatch"],
            )
        ),
        headers=_token(),
    )
    ...
    assert lineage_mismatch.json()["status"] == "rejected"
    assert lineage_mismatch.json()["error"]["code"] == "INVALID_PAYLOAD"
    assert not hasattr(store, "temp_power_customer_delivery_proof_reviews") or len(store.temp_power_customer_delivery_proof_reviews) == 0


def test_temp_power_customer_delivery_proof_review_status_read_classifies_storage_unavailable(monkeypatch):
    class BrokenCustomerDeliveryProofReviewStore:
        def values(self):
            raise RuntimeError("customer delivery proof review table missing")

    monkeypatch.setattr(store, "temp_power_customer_delivery_proof_reviews", BrokenCustomerDeliveryProofReviewStore())

    status = load_temp_power_customer_delivery_proof_review_status()
...
def test_temp_power_customer_delivery_proof_review_migration_preserves_insert_only_boundary():
    sql = MIGRATION_PATH.read_text(encoding="utf-8")

    assert "CREATE TABLE IF NOT EXISTS seam.pm_customer_delivery_proof_reviews" in sql
    assert "customer_delivery_proof_review_id   TEXT PRIMARY KEY" in sql
    assert "mutation_id                         TEXT NOT NULL" in sql
    assert "audit_event_id                      TEXT NOT NULL" in sql
    assert "finance_export_recorded             BOOLEAN NOT NULL DEFAULT FALSE" in sql
    assert "source_writeback_recorded           BOOLEAN NOT NULL DEFAULT FALSE" in sql
    assert "customer_billing_delivery_recorded  BOOLEAN NOT NULL DEFAULT FALSE" in sql
    assert "FUNCTION seam.reject_pm_customer_delivery_proof_review_mutation()" in sql
    assert "BEFORE UPDATE ON seam.pm_customer_delivery_proof_reviews" in sql
    assert "BEFORE DELETE ON seam.pm_customer_delivery_proof_reviews" in sql
    assert "delivery_channel IN ('CONTROLLED_EMAIL', 'LATER_APPROVED_PORTAL')" in sql
    assert "ALTER TABLE seam.pm_customer_delivery_proof_reviews ENABLE ROW LEVEL SECURITY" in sql
    assert "REVOKE ALL ON TABLE seam.pm_customer_delivery_proof_reviews FROM anon" in sql
    assert "REVOKE ALL ON TABLE seam.pm_customer_delivery_proof_reviews FROM authenticated" in sql
