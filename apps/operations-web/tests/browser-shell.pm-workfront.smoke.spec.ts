...
test('pm workfront route renders read-only readiness queue from governed seam', async ({ page }) => {
  ...
  const cableSignals = page.getByRole('region', { name: /PM signals for Cable Assembly A/i })
  await expect(cableSignals).toBeVisible()
  await expect(cableSignals.getByText(/Needs PM disposition/i)).toBeVisible()
  await expect(cableSignals.getByText(/Stale blocker/i)).toBeVisible()
  await expect(cableSignals.getByText(/1 open issue/i)).toBeVisible()
  await expect(cableSignals.getByText(/Returned to lead/i)).toHaveCount(0)
  await expect(cableSignals.getByRole('link')).toHaveCount(0)
  await expect(cableSignals.getByRole('button')).toHaveCount(0)
  const panelSignals = page.getByRole('region', { name: /PM signals for Distribution Panel/i })
  await expect(panelSignals).toBeVisible()
  await expect(panelSignals.getByText(/Ready for PM review/i)).toBeVisible()
  await expect(panelSignals.getByText(/Submitted snapshot/i)).toBeVisible()
  await expect(panelSignals.getByText(/Needs PM disposition/i)).toHaveCount(0)
  await expect(panelSignals.getByRole('link')).toHaveCount(0)
  await expect(panelSignals.getByRole('button')).toHaveCount(0)
  const mainSignals = page.getByRole('region', { name: /PM signals for Main Switchgear/i })
  await expect(mainSignals).toBeVisible()
  await expect(mainSignals.getByText(/Owner unassigned/i)).toBeVisible()
  await expect(mainSignals.getByRole('link')).toHaveCount(0)
  await expect(mainSignals.getByRole('button')).toHaveCount(0)
  ...
  const returnedSignals = page.getByRole('region', { name: /PM signals for Cable Assembly A/i })
  await expect(returnedSignals.getByText(/Returned to lead/i)).toBeVisible()
  await expect(returnedSignals.getByText(/Stale blocker/i)).toHaveCount(0)
