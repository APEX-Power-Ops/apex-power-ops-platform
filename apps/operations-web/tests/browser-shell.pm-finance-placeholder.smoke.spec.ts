...
test('pm finance placeholder route renders downstream no-live planning guardrails', async ({ page }) => {
  ...
  await expect(page.getByText('FINANCE_HANDOFF_DRAFT')).toBeVisible()
  ...
  await expect(page.getByText(/internal finance handoff planning from external customer billing delivery/i)).toBeVisible()
