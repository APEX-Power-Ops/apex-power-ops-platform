...
test('pm import intake workbench renders consolidated read-only Project Miner gates', async ({ page }) => {
  ...
  await expect(sourceExceptionDetail.getByLabel('Exception review and PM decision detail')).toHaveCount(1)
  ...
  await expect(approvalPrepDetail.getByLabel('Admission and approval contract')).toHaveCount(1)
  await expect(approvalPrepDetail.getByLabel('Local review checklist')).toHaveCount(1)
