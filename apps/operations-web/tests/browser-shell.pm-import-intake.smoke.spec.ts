...
test('pm import intake workbench renders consolidated read-only Project Miner gates', async ({ page }) => {
  ...
  await expect(outputSelectorControls.locator('a')).toHaveCount(5)
  ...
  const outputSelectorDisclosureStateKeys = await page.evaluate(() => Object.keys(window.localStorage).filter((key) => key.startsWith('pm-import-intake-') && /collapse|disclosure|output-selector/i.test(key)))
  ...
  const guardrailsControls = guardrails.locator(':scope > div').first()
  ...
  await expect(guardrailsControls.getByText(/Keep approval persistence and project import blocked until a later packet explicitly admits those writes/i)).toBeVisible()
  ...
  const approvalDraftControls = approvalDraft.locator(':scope > div').first()
  await expect(approvalDraftControls).toBeVisible()
  await expect(approvalDraftControls.locator('label')).toHaveCount(3)
  ...
  const closeoutControls = closeoutIntake.locator(':scope > div').first()
  await expect(closeoutControls).toBeVisible()
  await expect(closeoutControls.locator('label')).toHaveCount(8)
  ...
  const fieldReadinessControls = fieldReadiness.locator(':scope > div').first()
  await expect(fieldReadinessControls).toBeVisible()
  await expect(fieldReadinessControls.locator('label')).toHaveCount(8)
  ...
  const fieldPrepQueueControls = fieldPrepQueue.locator(':scope > div').first()
  ...
  await expect(fieldPrepQueueControls).toBeHidden()
  ...
  const fieldPrepCoverageControls = fieldPrepCoverage.locator(':scope > div').first()
  ...
  await expect(fieldPrepCoverageControls).toBeHidden()
  ...
  const fieldPrepAgendaControls = fieldPrepAgenda.locator(':scope > div').first()
  ...
  await expect(fieldPrepAgendaControls).toBeHidden()
  ...
  const fieldQuestionsControls = fieldQuestions.locator(':scope > div').first()
  await expect(fieldQuestionsControls).toBeVisible()
  ...
  await expect(fieldQuestionsControls).toBeHidden()
  ...
  const fieldObservationControls = fieldObservations.locator(':scope > div').first()
  await expect(fieldObservationControls).toBeVisible()
  ...
  await expect(fieldObservationControls).toBeHidden()
  ...
  const readinessControls = readiness.locator(':scope > div').first()
