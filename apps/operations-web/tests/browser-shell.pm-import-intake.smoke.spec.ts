...
async function expectWorkbenchViewportScan(page: Page, viewport: { width: number, height: number }, label: string) {
  ...
  const scan = await page.evaluate(() => {
    ...
    const namedSelectors = [
      ...
      ['field-start-conversation-closeout-prompt-review', '#pm-field-start-conversation-closeout-prompt-review'],
      ['field-start-bring-back-summary-triage-strip', '#pm-field-start-bring-back-summary-triage-strip'],
      ['field-start-bring-back-detail-jump-rail', '#pm-field-start-bring-back-detail-jump-rail'],
      ['field-start-bring-back-lens-open-context-cue', '#pm-field-start-bring-back-lens-open-context-cue'],
      ['field-start-bring-back-review-order-hint', '#pm-field-start-bring-back-review-order-hint'],
      ['field-start-bring-back-cue-status-legend', '#pm-field-start-bring-back-cue-status-legend'],
      ['field-start-bring-back-review-queue', '#pm-field-start-bring-back-review-queue'],
      ['field-start-source-review-bring-back-lens', '#pm-field-start-source-review-bring-back-lens'],
...
test('pm import intake workbench renders consolidated read-only Project Miner gates', async ({ page }) => {
  ...
  await expect(dailyActionPanels.locator('section#pm-field-start-conversation-closeout-prompt-review[aria-label="Local field-start conversation closeout prompt review"]')).toHaveCount(1)
  await expect(dailyActionPanels.locator('section#pm-field-start-bring-back-summary-triage-strip[aria-label="Local field-start bring-back summary triage strip"]')).toHaveCount(1)
  await expect(dailyActionPanels.locator('section#pm-field-start-bring-back-detail-jump-rail[aria-label="Local field-start bring-back detail jump rail"]')).toHaveCount(1)
  await expect(dailyActionPanels.locator('#pm-field-start-bring-back-lens-open-context-cue[aria-label="Local field-start bring-back lens open-context cue"]')).toHaveCount(1)
  await expect(dailyActionPanels.locator('#pm-field-start-bring-back-review-order-hint[aria-label="Local field-start bring-back review order hint"]')).toHaveCount(1)
  await expect(dailyActionPanels.locator('#pm-field-start-bring-back-cue-status-legend[aria-label="Local field-start bring-back cue status legend"]')).toHaveCount(1)
  await expect(dailyActionPanels.locator('section#pm-field-start-bring-back-review-queue[aria-label="Local field-start bring-back review queue"]')).toHaveCount(1)
  await expect(dailyActionPanels.locator('section#pm-field-start-source-review-bring-back-lens[aria-label="Local field-start source review bring-back lens"]')).toHaveCount(1)
  ...
  await expect(outputSelector.getByRole('link', { name: /Field Authorization Assignment Draft/i })).toHaveAttribute('href', '/pm-review/field-authorization-placeholder')
  await expect(outputSelector.getByRole('link', { name: /Schedule Status Controls Draft/i })).toHaveAttribute('href', '/pm-review/schedule-status-placeholder')
  await expect(outputSelector.getByRole('link', { name: /Durable Field Record Draft/i })).toHaveAttribute('href', '/pm-review/durable-field-record-placeholder')
  await expect(outputSelector.getByRole('link', { name: /Production Tracking Draft/i })).toHaveAttribute('href', '/pm-review/production-tracking-placeholder')
  await expect(outputSelector.getByRole('link', { name: /Customer Reporting Draft/i })).toHaveAttribute('href', '/pm-review/customer-reporting-placeholder')
  await expect(outputSelector.getByRole('link', { name: /Financial Handoff Draft/i })).toHaveAttribute('href', '/pm-review/financial-handoff-placeholder')
  ...
  const guardrailGroups = guardrailsControls.locator('[aria-label="Current PM guardrail groups"]')
  await expect(guardrailGroups).toBeVisible()
  await expect(guardrailGroups.locator(':scope > section')).toHaveCount(2)
  const currentReviewActions = guardrailGroups.locator('[aria-label="Current Review Actions guardrail group"]')
  const blockedWriteGuardrails = guardrailGroups.locator('[aria-label="Blocked Write Guardrails guardrail group"]')
  await expect(currentReviewActions.getByRole('heading', { name: 'Current Review Actions', exact: true })).toBeVisible()
  await expect(blockedWriteGuardrails.getByRole('heading', { name: 'Blocked Write Guardrails', exact: true })).toBeVisible()
  await expect(currentReviewActions.locator('article')).toHaveCount(2)
  await expect(blockedWriteGuardrails.locator('article')).toHaveCount(1)
  await expect(guardrailsControls.locator('article')).toHaveCount(3)
  await expect(guardrailsControls.getByRole('heading', { name: 'Current PM Next Actions', exact: true })).toBeVisible()
  const currentOpenDataEntryDecision = guardrailsControls.getByLabel('Current open Project Data Entry decision')
  await expect(currentOpenDataEntryDecision).toBeVisible()
  await expect(currentOpenDataEntryDecision.getByRole('heading', { name: 'Current Open PM Decision', exact: true })).toBeVisible()
  await expect(currentOpenDataEntryDecision.getByText(/PROJECT_DATA_ENTRY_FORMULA_ERRORS remains open/i)).toBeVisible()
  await expect(currentOpenDataEntryDecision.getByText(/Return exactly one PM Lane 238 label from the Project Data Entry exact reply options card/i)).toBeVisible()
  await expect(currentOpenDataEntryDecision.getByText(/do not use explanation text, a paraphrase, or REQUEST_SOURCE_CORRECTION_NO_LIVE/i)).toBeVisible()
  await expect(currentOpenDataEntryDecision.getByText(/does not accept the warning, approve the candidate, create an approval row, import project rows, write source files, or call a hosted mutation/i)).toBeVisible()
  ...
  await expect(guardrailGroups).toBeHidden()
  ...
  await expect(projectDataEntryAllowedLabels.filter({ hasText: 'ACCEPT_DATA_ENTRY_WARNING_NON_BLOCKING_NO_LIVE' })).toBeVisible()
  await expect(projectDataEntryAllowedLabels.filter({ hasText: 'REQUEST_DATA_ENTRY_WORKBOOK_CORRECTION_NO_LIVE' })).toBeVisible()
  await expect(projectDataEntryAllowedLabels.filter({ hasText: 'HOLD_DATA_ENTRY_WARNING_NO_LIVE' })).toBeVisible()
  await expect(projectDataEntryAllowedLabels.filter({ hasText: 'PROVIDE_EXACT_LIVE_ADMISSION_LATER' })).toBeVisible()
  await expect(projectDataEntryDecisionGate.getByText(/Prior source correction is already applied: REQUEST_SOURCE_CORRECTION_NO_LIVE -> Ground Resistance Test Lot/i)).toBeVisible()
  ...
  const exactReplyOptions = exceptionDecisionDetail.getByLabel('Project Data Entry exact reply options')
  await expect(exactReplyOptions).toBeVisible()
  await expect(exactReplyOptions.getByText(/Copy exactly one line only/i)).toBeVisible()
  await expect(exactReplyOptions.getByText(/Do not include explanation text, a paraphrase, or REQUEST_SOURCE_CORRECTION_NO_LIVE/i)).toBeVisible()
  await expect(exactReplyOptions.locator('code')).toHaveCount(4)
  await expect(exactReplyOptions.getByText('ACCEPT_DATA_ENTRY_WARNING_NON_BLOCKING_NO_LIVE', { exact: true })).toBeVisible()
  await expect(exactReplyOptions.getByText('REQUEST_DATA_ENTRY_WORKBOOK_CORRECTION_NO_LIVE', { exact: true })).toBeVisible()
  await expect(exactReplyOptions.getByText('HOLD_DATA_ENTRY_WARNING_NO_LIVE', { exact: true })).toBeVisible()
  await expect(exactReplyOptions.getByText('PROVIDE_EXACT_LIVE_ADMISSION_LATER', { exact: true })).toBeVisible()
  const outcomeRoutes = exceptionDecisionDetail.getByLabel('Project Data Entry outcome routes')
  await expect(outcomeRoutes).toBeVisible()
  await expect(outcomeRoutes.getByText(/What each reply does next/i)).toBeVisible()
  await expect(outcomeRoutes.getByText(/Route preview only/i)).toBeVisible()
  await expect(outcomeRoutes.getByText(/record no-live warning acceptance context; keep live admission separate/i)).toBeVisible()
  await expect(outcomeRoutes.getByText(/open a no-live workbook-correction packet before warning acceptance or live admission/i)).toBeVisible()
  await expect(outcomeRoutes.getByText(/keep the warning unresolved and continue no-live readiness only/i)).toBeVisible()
  await expect(outcomeRoutes.getByText(/defer disposition until a later exact live-admission packet/i)).toBeVisible()
  const validReturnChecklist = exceptionDecisionDetail.getByLabel('Project Data Entry valid return checklist')
  await expect(validReturnChecklist).toBeVisible()
  await expect(validReturnChecklist.getByText(/Valid return checklist/i)).toBeVisible()
  await expect(validReturnChecklist.getByText(/Use this only to screen a future reply/i)).toBeVisible()
  await expect(validReturnChecklist.getByText(/accepted: exactly one PM Lane 238 Data Entry label/i)).toBeVisible()
  await expect(validReturnChecklist.getByText(/rejected: explanation text; paraphrase; REQUEST_SOURCE_CORRECTION_NO_LIVE; multiple labels; continuation instruction; live admission language without a later admitting packet/i)).toBeVisible()
  await expect(validReturnChecklist.getByText(/after valid label: record the label in a no-live decision packet and keep live admission separate/i)).toBeVisible()
  const safeContinuation = exceptionDecisionDetail.getByLabel('Project Data Entry safe no-live continuation')
  await expect(safeContinuation).toBeVisible()
  await expect(safeContinuation.getByText(/Safe no-live continuation/i)).toBeVisible()
  await expect(safeContinuation.getByText(/These moves may continue while the exact Project Data Entry label is pending/i)).toBeVisible()
  await expect(safeContinuation.getByText(/allowed no-live continuation: candidate\/readiness review, packet drafting, Desktop Codex read-only scout review, and source\/resource question preparation/i)).toBeVisible()
  await expect(safeContinuation.getByText(/requires exact PM label first: warning acceptance, workbook-correction action, live admission packet, approval POST, approval row, and project import/i)).toBeVisible()
  await expect(safeContinuation.getByText(/Desktop Codex boundary: review clarity and relay burden only; do not choose the PM label or mutate business state/i)).toBeVisible()
  ...
  await expect(fieldStartOperatorScript).toHaveAttribute('open', '')
  await expect(fieldStartOperatorScript.getByRole('heading', { name: /Local Field Start Operator Script/i })).toBeVisible()
  const fieldStartOperatorScriptControls = fieldStartOperatorScript.locator('[aria-label="Local field start operator script controls"]')
  await expect(fieldStartOperatorScriptControls).toBeVisible()
  await expect(fieldStartOperatorScriptControls.locator('a')).toHaveCount(5)
  await expect(fieldStartOperatorScript.getByText('browser-local', { exact: true })).toBeVisible()
  await expect(fieldStartOperatorScript.getByText(/Morning-of field-start conversation script derived from existing local prep state/i)).toBeVisible()
  await expect(fieldStartOperatorScript.getByText(/creates no localStorage key, export artifact, backend route, task, issue, field authorization, assignment, schedule\/status change, durable field record, production tracking row, hosted write claim, or production write/i)).toBeVisible()
  await fieldStartOperatorScript.locator(':scope > summary').click()
  await expect(fieldStartOperatorScript).not.toHaveAttribute('open', '')
  await expect(fieldStartOperatorScriptControls).toBeHidden()
  const fieldStartOperatorScriptDisclosureStateKeys = await page.evaluate(() => Object.keys(window.localStorage).filter((key) => key.startsWith('pm-import-intake-') && /collapse|disclosure|field-start-operator|operator-script/i.test(key)))
  expect(fieldStartOperatorScriptDisclosureStateKeys).toEqual([])
  await fieldStartOperatorScript.locator(':scope > summary').click()
  await expect(fieldStartOperatorScript).toHaveAttribute('open', '')
  await expect(fieldStartOperatorScriptControls).toBeVisible()
  await expect(fieldStartOperatorScript.getByRole('link', { name: /Say field-start posture/i })).toHaveAttribute('href', '#field-prep')
  await expect(fieldStartOperatorScript.getByRole('link', { name: /Check source and access questions/i })).toHaveAttribute('href', '#field-prep')
  await expect(fieldStartOperatorScript.getByRole('link', { name: /Walk queue, coverage, and agenda/i })).toHaveAttribute('href', '#field-prep')
  await expect(fieldStartOperatorScript.getByRole('link', { name: /Export context only/i })).toHaveAttribute('href', '#pm-output-selector')
  await expect(fieldStartOperatorScript.getByRole('link', { name: /Stop line before field authority/i })).toHaveAttribute('href', '#guardrails')
  await expect(fieldStartOperatorScript.getByText(/Miner Temp Power: field-start preflight is 0 ready, 1 needs review, 4 blocked; field start authority remains blocked until field authority and tracking packet/i)).toBeVisible()
  await expect(fieldStartOperatorScript.getByText(/Capture drawing\/source and access\/safety questions before field reliance/i)).toBeVisible()
  ...
  await expect(fieldStartStopLineReview).toHaveAttribute('open', '')
  await expect(fieldStartStopLineReview.getByRole('heading', { name: /Local Field Start Stop-Line Quick Review/i })).toBeVisible()
  const fieldStartStopLineReviewControls = fieldStartStopLineReview.locator('[aria-label="Local field start stop-line quick review controls"]')
  await expect(fieldStartStopLineReviewControls).toBeVisible()
  await expect(fieldStartStopLineReviewControls.locator('a')).toHaveCount(5)
  await expect(fieldStartStopLineReviewControls.getByRole('button')).toHaveCount(0)
  await expect(fieldStartStopLineReview.getByText('blocked writes', { exact: true })).toBeVisible()
  await expect(fieldStartStopLineReview.getByText(/Morning-of Temp Power stop-line check for the field-start conversation/i)).toBeVisible()
  await expect(fieldStartStopLineReview.getByText(/creates no approval, import, field, production, customer, or finance write/i)).toBeVisible()
  await fieldStartStopLineReview.locator(':scope > summary').click()
  await expect(fieldStartStopLineReview).not.toHaveAttribute('open', '')
  await expect(fieldStartStopLineReviewControls).toBeHidden()
  const fieldStartStopLineReviewDisclosureStateKeys = await page.evaluate(() => Object.keys(window.localStorage).filter((key) => key.startsWith('pm-import-intake-') && /collapse|disclosure|stop-line|quick-review|field-start-stop/i.test(key)))
  expect(fieldStartStopLineReviewDisclosureStateKeys).toEqual([])
  await fieldStartStopLineReview.locator(':scope > summary').click()
  await expect(fieldStartStopLineReview).toHaveAttribute('open', '')
  await expect(fieldStartStopLineReviewControls).toBeVisible()
  await expect(fieldStartStopLineReview.getByRole('link', { name: /Field authority stop line/i })).toHaveAttribute('href', '#field-prep')
  ...
  await expect(fieldStartStopLineReview.getByRole('link', { name: /Customer and finance stop line/i })).toHaveAttribute('href', '#guardrails')
  await expect(fieldStartStopLineReview.getByRole('link', { name: /Context-only use/i })).toHaveAttribute('href', '#approval-readiness')
  await expect(fieldStartStopLineReview.getByText(/Miner Temp Power: field start authority remains blocked until field authority and tracking packet; preflight is 0 ready, 1 needs review, 4 blocked/i)).toBeVisible()
  await expect(fieldStartStopLineReview.getByText(/Do not assign leads or crews, schedule work, alter status, or direct field execution from this workbench/i)).toBeVisible()
  await expect(fieldStartStopLineReview.getByText(/Do not create durable field records, production quantities, progress rows, completion evidence, or customer-facing production truth/i)).toBeVisible()
  await expect(fieldStartStopLineReview.getByText(/Do not create customer reports, billing exports, payroll exports, invoices, accounting postings, or external finance-system syncs/i)).toBeVisible()
  await expect(fieldStartStopLineReview.getByText(/Use the operator script and local exports only as conversation context; \d+ blocked boundaries remain outside this browser-local review/i)).toBeVisible()
  const fieldStartCustomerSiteQuestions = page.locator('details#pm-field-start-customer-site-questions[aria-label="Local field start customer site questions quick review"]')
  await expect(fieldStartCustomerSiteQuestions).toHaveAttribute('open', '')
  await expect(fieldStartCustomerSiteQuestions.getByRole('heading', { name: /Local Field Start Customer\/Site Questions Quick Review/i })).toBeVisible()
  const fieldStartCustomerSiteQuestionsControls = fieldStartCustomerSiteQuestions.locator('[aria-label="Local field start customer site questions quick review controls"]')
  await expect(fieldStartCustomerSiteQuestionsControls).toBeVisible()
  await expect(fieldStartCustomerSiteQuestionsControls.locator('a')).toHaveCount(5)
  await expect(fieldStartCustomerSiteQuestionsControls.getByRole('button')).toHaveCount(0)
  await expect(fieldStartCustomerSiteQuestions.getByText('question review', { exact: true })).toBeVisible()
  await expect(fieldStartCustomerSiteQuestions.getByText(/Morning-of Temp Power customer\/site question review derived from existing field questions and observations/i)).toBeVisible()
  await expect(fieldStartCustomerSiteQuestions.getByText(/creates no localStorage key, export artifact, backend route, task, issue, assignment, schedule\/status change, customer commitment, customer report, field instruction, hosted write claim, or production write/i)).toBeVisible()
  await fieldStartCustomerSiteQuestions.locator(':scope > summary').click()
  await expect(fieldStartCustomerSiteQuestions).not.toHaveAttribute('open', '')
  await expect(fieldStartCustomerSiteQuestionsControls).toBeHidden()
  const fieldStartCustomerSiteQuestionsDisclosureStateKeys = await page.evaluate(() => Object.keys(window.localStorage).filter((key) => key.startsWith('pm-import-intake-') && /collapse|disclosure|customer-site|field-start-customer|site-questions|question-review/i.test(key)))
  expect(fieldStartCustomerSiteQuestionsDisclosureStateKeys).toEqual([])
  await fieldStartCustomerSiteQuestions.locator(':scope > summary').click()
  await expect(fieldStartCustomerSiteQuestions).toHaveAttribute('open', '')
  await expect(fieldStartCustomerSiteQuestionsControls).toBeVisible()
  await expect(fieldStartCustomerSiteQuestions.getByRole('link', { name: /Site access and safety questions/i })).toHaveAttribute('href', '#field-prep')
  ...
  await expect(fieldStartCustomerSiteQuestions.getByRole('link', { name: /Drawing and source questions/i })).toHaveAttribute('href', '#field-prep')
  await expect(fieldStartCustomerSiteQuestions.getByRole('link', { name: /PM follow-up and customer commitment boundary/i })).toHaveAttribute('href', '#guardrails')
  await expect(fieldStartCustomerSiteQuestions.getByText(/Miner Temp Power: capture site access, escort, safety, or LOTO questions before the field-start conversation; do not turn them into instructions or commitments here/i)).toBeVisible()
  await expect(fieldStartCustomerSiteQuestions.getByText(/Customer\/site constraint questions are still open; ask about access windows, shutdown constraints, escort requirements, and site contact questions as conversation context only/i)).toBeVisible()
  await expect(fieldStartCustomerSiteQuestions.getByText(/Material, equipment, and staging questions are still open; keep them as local prep questions until a later approved field packet exists/i)).toBeVisible()
  await expect(fieldStartCustomerSiteQuestions.getByText(/Drawing\/source questions are still open; review source truth before any future field-authority packet/i)).toBeVisible()
  await expect(fieldStartCustomerSiteQuestions.getByText(/No PM follow-up\/customer-site context is captured yet; do not turn questions into owner lists, assignments, customer commitments, customer reports, schedule\/status changes, or field direction from this workbench/i)).toBeVisible()
  const pmFollowupPromptReview = fieldStartCustomerSiteQuestions.locator('section#pm-field-start-pm-followup-prompt-review[aria-label="Local PM follow-up prompt review"]')
  await expect(pmFollowupPromptReview).toBeVisible()
  await expect(pmFollowupPromptReview.getByRole('heading', { name: /Local PM Follow-up Prompt Review/i })).toBeVisible()
  const pmFollowupPromptReviewControls = pmFollowupPromptReview.locator('[aria-label="Local PM follow-up prompt review controls"]')
  await expect(pmFollowupPromptReviewControls).toBeVisible()
  await expect(pmFollowupPromptReviewControls.locator('a')).toHaveCount(5)
  await expect(pmFollowupPromptReviewControls.getByRole('button')).toHaveCount(0)
  await expect(pmFollowupPromptReview.getByText('prompt review', { exact: true })).toBeVisible()
  await expect(pmFollowupPromptReview.getByText(/Copy\/paste prompt review for the next PM, lead, or customer question after the customer\/site check/i)).toBeVisible()
  await expect(pmFollowupPromptReview.getByText(/creates no localStorage key, export artifact, backend route, task, action item, owner, due date, customer commitment, hosted write claim, or production write/i)).toBeVisible()
  const pmFollowupPromptReviewStorageKeys = await page.evaluate(() => Object.keys(window.localStorage).filter((key) => key.startsWith('pm-import-intake-') && /follow.?up.?prompt|prompt.?review|customer.?commitment|owner|due.?date|action.?item/i.test(key)))
  expect(pmFollowupPromptReviewStorageKeys).toEqual([])
  await expect(pmFollowupPromptReview.getByRole('link', { name: /PM follow-up question prompt/i })).toHaveAttribute('href', '#field-prep')
  ...
  await expect(pmFollowupPromptReview.getByRole('link', { name: /Evidence\/source prompt/i })).toHaveAttribute('href', '#field-prep')
  await expect(pmFollowupPromptReview.getByRole('link', { name: /Next packet boundary prompt/i })).toHaveAttribute('href', '#guardrails')
  await expect(pmFollowupPromptReview.getByText(/Miner Temp Power: PM follow-up prompt is open; decide the next PM question before accountability, timing, field instruction, customer commitment, or status language is discussed/i)).toBeVisible()
  await expect(pmFollowupPromptReview.getByText(/Return to site access, escort, shutdown-window, and contact-path questions before relying on field assumptions/i)).toBeVisible()
  await expect(pmFollowupPromptReview.getByText(/Lead prompt is open; start with source, access, material, equipment, and staging unknowns before field reliance/i)).toBeVisible()
  await expect(pmFollowupPromptReview.getByText(/Evidence\/source prompt is open; identify what source record or site note should be checked before a later bounded packet/i)).toBeVisible()
  await expect(pmFollowupPromptReview.getByText(/If the follow-up needs accountability, timing, customer-facing language, field direction, report, schedule\/status update, or durable record, stop here and author a later bounded packet; this section records none/i)).toBeVisible()
  const conversationCloseoutPromptReview = fieldStartCustomerSiteQuestions.locator('section#pm-field-start-conversation-closeout-prompt-review[aria-label="Local field-start conversation closeout prompt review"]')
  await expect(conversationCloseoutPromptReview).toBeVisible()
  await expect(conversationCloseoutPromptReview.getByRole('heading', { name: /Local Field-Start Conversation Closeout Prompt Review/i })).toBeVisible()
  const conversationCloseoutPromptReviewControls = conversationCloseoutPromptReview.locator('[aria-label="Local field-start conversation closeout prompt review controls"]')
  await expect(conversationCloseoutPromptReviewControls).toBeVisible()
  await expect(conversationCloseoutPromptReviewControls.locator('a')).toHaveCount(5)
  await expect(conversationCloseoutPromptReviewControls.getByRole('button')).toHaveCount(0)
  await expect(conversationCloseoutPromptReview.getByText('closeout prompt', { exact: true })).toBeVisible()
  await expect(conversationCloseoutPromptReview.getByText(/Copy\/paste review context for what to bring back after the field-start conversation/i)).toBeVisible()
  await expect(conversationCloseoutPromptReview.getByText(/creates no meeting note, localStorage key, export artifact, backend route, task, action item, owner, due date, customer commitment, customer report, field instruction, hosted write claim, or production write/i)).toBeVisible()
  const conversationCloseoutPromptReviewStorageKeys = await page.evaluate(() => Object.keys(window.localStorage).filter((key) => key.startsWith('pm-import-intake-') && /conversation.?closeout|closeout.?prompt|meeting.?note|action.?item|owner|due.?date|customer.?commitment|customer.?report/i.test(key)))
  expect(conversationCloseoutPromptReviewStorageKeys).toEqual([])
  ...
  await expect(conversationCloseoutPromptReview.getByRole('link', { name: /Evidence\/source return prompt/i })).toHaveAttribute('href', '#field-prep')
  await expect(conversationCloseoutPromptReview.getByRole('link', { name: /Next packet closeout boundary/i })).toHaveAttribute('href', '#guardrails')
  await expect(conversationCloseoutPromptReview.getByText(/Miner Temp Power: after the field-start conversation, bring back a short summary of what changed and what stayed open; do not store the summary in this panel/i)).toBeVisible()
  await expect(conversationCloseoutPromptReview.getByText(/Customer\/site return remains open; ask what access, shutdown, escort, or contact answer still needs local review before any later bounded packet/i)).toBeVisible()
  await expect(conversationCloseoutPromptReview.getByText(/Lead\/resource return remains open; ask what material, equipment, staging, or lead clarification should be reviewed locally before field reliance/i)).toBeVisible()
  await expect(conversationCloseoutPromptReview.getByText(/Evidence\/source return remains open; identify the source record, drawing, workbook row, or site note to inspect before any later authority packet/i)).toBeVisible()
  await expect(conversationCloseoutPromptReview.getByText(/If the conversation reveals a needed next move, bring back only the packet question; do not turn it into work lists, accountability fields, timing fields, commitments, reports, or writes here/i)).toBeVisible()
  const bringBackSummaryTriageStrip = fieldStartCustomerSiteQuestions.locator('section#pm-field-start-bring-back-summary-triage-strip[aria-label="Local field-start bring-back summary triage strip"]')
  await expect(bringBackSummaryTriageStrip).toBeVisible()
  await expect(bringBackSummaryTriageStrip.getByRole('heading', { name: /Local Field Start Bring-Back Summary Triage Strip/i })).toBeVisible()
  const bringBackSummaryTriageStripControls = bringBackSummaryTriageStrip.locator('[aria-label="Local field-start bring-back summary triage strip controls"]')
  await expect(bringBackSummaryTriageStripControls).toBeVisible()
  await expect(bringBackSummaryTriageStripControls.locator('a')).toHaveCount(4)
  await expect(bringBackSummaryTriageStripControls.getByRole('button')).toHaveCount(0)
  await expect(bringBackSummaryTriageStrip.getByText('summary triage', { exact: true })).toBeVisible()
  await expect(bringBackSummaryTriageStrip.getByText(/Compact read-only triage strip for returned field-start conversation context/i)).toBeVisible()
  await expect(bringBackSummaryTriageStrip.getByText(/summarizes which bring-back areas currently have local context only before the detailed queue and lenses/i)).toBeVisible()
  await expect(bringBackSummaryTriageStrip.getByText(/creates no meeting note, localStorage key, export artifact, backend route, task, action item, owner, due date, assignment, schedule\/status write, customer commitment, customer report, field instruction, durable record, production tracking row, hosted write claim, or production write, and exposes no buttons/i)).toBeVisible()
  ...
  await expect(bringBackSummaryTriageStrip.getByRole('link', { name: /Lead\/resource clarification context/i })).toHaveAttribute('href', '#pm-field-start-lead-resource-clarification-bring-back-lens')
  await expect(bringBackSummaryTriageStrip.getByRole('link', { name: /Later bounded packet candidate context/i })).toHaveAttribute('href', '#pm-field-start-later-bounded-packet-candidate-bring-back-lens')
  await expect(bringBackSummaryTriageStrip.getByText(/Miner Temp Power: no returned source-review context is captured yet; if one returns, use the source lens before any later packet/i)).toBeVisible()
  await expect(bringBackSummaryTriageStrip.getByText(/No returned customer\/site clarification context is captured yet; if one returns, keep it local until a later bounded packet admits more/i)).toBeVisible()
  await expect(bringBackSummaryTriageStrip.getByText(/No returned lead\/resource clarification context is captured yet; if one returns, keep it as browser-local review context only/i)).toBeVisible()
  await expect(bringBackSummaryTriageStrip.getByText(/No later bounded packet candidate context is captured yet; if one returns, classify the packet question without creating work here/i)).toBeVisible()
  const bringBackDetailJumpRail = fieldStartCustomerSiteQuestions.locator('section#pm-field-start-bring-back-detail-jump-rail[aria-label="Local field-start bring-back detail jump rail"]')
  await expect(bringBackDetailJumpRail).toBeVisible()
  await expect(bringBackDetailJumpRail.getByRole('heading', { name: /Local Field Start Bring-Back Detail Jump Rail/i })).toBeVisible()
  const bringBackDetailJumpRailLinks = bringBackDetailJumpRail.locator('[aria-label="Local field-start bring-back detail jump rail links"]')
  await expect(bringBackDetailJumpRailLinks).toBeVisible()
  await expect(bringBackDetailJumpRailLinks.locator('a')).toHaveCount(4)
  await expect(bringBackDetailJumpRailLinks.getByRole('button')).toHaveCount(0)
  await expect(bringBackDetailJumpRail.getByText('detail jumps', { exact: true })).toBeVisible()
  await expect(bringBackDetailJumpRail.getByText(/Compact read-only jump rail from the summary triage strip to the exact bring-back detail lens/i)).toBeVisible()
  await expect(bringBackDetailJumpRail.getByText(/only navigates within this browser-local review panel/i)).toBeVisible()
  await expect(bringBackDetailJumpRail.getByText(/creates no meeting note, localStorage key, export artifact, backend route, task, action item, owner, due date, assignment, schedule\/status write, customer commitment, customer report, field instruction, durable record, production tracking row, hosted write claim, or production write, and exposes no buttons/i)).toBeVisible()
  ...
  await expect(bringBackDetailJumpRail.getByRole('link', { name: /Open lead\/resource clarification lens/i })).toHaveAttribute('href', '#pm-field-start-lead-resource-clarification-bring-back-lens')
  await expect(bringBackDetailJumpRail.getByRole('link', { name: /Open later packet candidate lens/i })).toHaveAttribute('href', '#pm-field-start-later-bounded-packet-candidate-bring-back-lens')
  await expect(bringBackDetailJumpRail.getByText(/Jump to the source review lens for drawing, workbook, site note, observer\/source, or work-area context only/i)).toBeVisible()
  await expect(bringBackDetailJumpRail.getByText(/Jump to the customer\/site clarification lens for access, shutdown, escort, contact, safety, or constraint review only/i)).toBeVisible()
  await expect(bringBackDetailJumpRail.getByText(/Jump to the lead\/resource clarification lens for lead, crew, material, equipment, staging, or resource review only/i)).toBeVisible()
  await expect(bringBackDetailJumpRail.getByText(/Jump to the later bounded packet candidate lens only to classify whether a later bounded packet is needed; do not create that packet here/i)).toBeVisible()
  const bringBackLensOpenContextCue = bringBackDetailJumpRail.locator('#pm-field-start-bring-back-lens-open-context-cue[aria-label="Local field-start bring-back lens open-context cue"]')
  await expect(bringBackLensOpenContextCue).toBeVisible()
  await expect(bringBackLensOpenContextCue.getByText('Open-context cue', { exact: true })).toBeVisible()
  await expect(bringBackLensOpenContextCue.getByText(/Populated detail lens context: none yet/i)).toBeVisible()
  await expect(bringBackLensOpenContextCue.getByText(/Keep returned field-start details local until a later bounded packet admits more/i)).toBeVisible()
  await expect(bringBackLensOpenContextCue.getByText(/browser-local open-context cue only/i)).toBeVisible()
  await expect(bringBackLensOpenContextCue.getByText(/creates no meeting note, localStorage key, task, action item, owner, due date, assignment, customer commitment, customer report, field instruction, durable field record, production tracking row, report, export artifact, storage key, backend route, button, hosted write claim, or write path/i)).toBeVisible()
  await expect(bringBackLensOpenContextCue.getByText('context cue', { exact: true })).toBeVisible()
  await expect(bringBackLensOpenContextCue.locator('a')).toHaveCount(0)
  ...
  await expect(bringBackCueStatusLegend.locator('a')).toHaveCount(0)
  await expect(bringBackCueStatusLegend.getByRole('button')).toHaveCount(0)
  const bringBackReviewQueue = fieldStartCustomerSiteQuestions.locator('section#pm-field-start-bring-back-review-queue[aria-label="Local field-start bring-back review queue"]')
  await expect(bringBackReviewQueue).toBeVisible()
  await expect(bringBackReviewQueue.getByRole('heading', { name: /Local Field Start Bring-Back Review Queue/i })).toBeVisible()
  const bringBackReviewQueueControls = bringBackReviewQueue.locator('[aria-label="Local field-start bring-back review queue controls"]')
  await expect(bringBackReviewQueueControls).toBeVisible()
  await expect(bringBackReviewQueueControls.locator('a')).toHaveCount(4)
  await expect(bringBackReviewQueueControls.getByRole('button')).toHaveCount(0)
  await expect(bringBackReviewQueue.getByText('review queue', { exact: true })).toBeVisible()
  await expect(bringBackReviewQueue.getByText(/Read-only bring-back queue for returned field-start conversation items/i)).toBeVisible()
  await expect(bringBackReviewQueue.getByText(/creates no meeting note, localStorage key, export artifact, backend route, task, action item, owner, due date, customer commitment, customer report, field instruction, hosted write claim, or production write/i)).toBeVisible()
  const bringBackReviewQueueStorageKeys = await page.evaluate(() => Object.keys(window.localStorage).filter((key) => key.startsWith('pm-import-intake-') && /bring.?back|review.?queue|returned.?field|bounded.?packet.?candidate|owner|due.?date|action.?item|customer.?commitment|customer.?report/i.test(key)))
  expect(bringBackReviewQueueStorageKeys).toEqual([])
  ...
  await expect(bringBackReviewQueue.getByRole('link', { name: /Lead\/resource clarification/i })).toHaveAttribute('href', '#field-prep')
  await expect(bringBackReviewQueue.getByRole('link', { name: /Later bounded packet candidate/i })).toHaveAttribute('href', '#guardrails')
  await expect(bringBackReviewQueue.getByText(/Miner Temp Power: if the conversation returns a drawing, workbook row, site note, observer\/source, or work-area question, classify it as local source review only/i)).toBeVisible()
  await expect(bringBackReviewQueue.getByText(/If the conversation returns access, shutdown, escort, contact, safety, or constraint answers, classify them as customer\/site clarification only/i)).toBeVisible()
  await expect(bringBackReviewQueue.getByText(/If the conversation returns lead, material, equipment, staging, or crew-readiness context, classify it as lead\/resource clarification only/i)).toBeVisible()
  await expect(bringBackReviewQueue.getByText(/If a returned item needs a move beyond local review, classify only the packet question; do not create work lists, timing fields, commitments, reports, records, or writes here/i)).toBeVisible()
  const sourceReviewBringBackLens = fieldStartCustomerSiteQuestions.locator('section#pm-field-start-source-review-bring-back-lens[aria-label="Local field-start source review bring-back lens"]')
  await expect(sourceReviewBringBackLens).toBeVisible()
  await expect(sourceReviewBringBackLens.getByRole('heading', { name: /Local Field Start Source Review Bring-Back Lens/i })).toBeVisible()
  const sourceReviewBringBackLensControls = sourceReviewBringBackLens.locator('[aria-label="Local field-start source review bring-back lens controls"]')
  await expect(sourceReviewBringBackLensControls).toBeVisible()
  await expect(sourceReviewBringBackLensControls.locator('a')).toHaveCount(5)
  await expect(sourceReviewBringBackLensControls.getByRole('button')).toHaveCount(0)
  await expect(sourceReviewBringBackLens.getByText('source lens', { exact: true })).toBeVisible()
  await expect(sourceReviewBringBackLens.getByText(/Read-only source review lens for returned field-start conversation items/i)).toBeVisible()
  await expect(sourceReviewBringBackLens.getByText(/helps classify drawings, workbook rows, site notes, observer\/source context, and work-area references before any later bounded packet/i)).toBeVisible()
  await expect(sourceReviewBringBackLens.getByText(/creates no meeting note, localStorage key, export artifact, backend route, task, action item, owner, due date, customer commitment, customer report, field instruction, durable record, hosted write claim, or production write/i)).toBeVisible()
  const sourceReviewBringBackLensStorageKeys = await page.evaluate(() => Object.keys(window.localStorage).filter((key) => key.startsWith('pm-import-intake-') && /source.?review.?bring.?back|source.?lens|drawing.?workbook|site.?note|observer.?source|work.?area|owner|due.?date|action.?item|customer.?commitment|customer.?report/i.test(key)))
  ...
  await expect(sourceReviewBringBackLens.getByRole('link', { name: /Work-area reference check/i })).toHaveAttribute('href', '#field-prep')
  await expect(sourceReviewBringBackLens.getByRole('link', { name: /Source review packet boundary/i })).toHaveAttribute('href', '#guardrails')
  await expect(sourceReviewBringBackLens.getByText(/Miner Temp Power: drawing, workbook, or source fingerprint context is available; review it locally before relying on a returned field-start source item. Fingerprint: stat-fingerprint-abc123/i)).toBeVisible()
  await expect(sourceReviewBringBackLens.getByText(/If the return is a site note, access observation, safety observation, or material\/equipment note, classify it for local source review before field reliance/i)).toBeVisible()
  await expect(sourceReviewBringBackLens.getByText(/If the return depends on who said it or when it was discussed, keep that question outside this lens and classify it for source review only/i)).toBeVisible()
  await expect(sourceReviewBringBackLens.getByText(/If the return names an area, workpackage, apparatus, or location reference, check it against source context before any later packet/i)).toBeVisible()
  await expect(sourceReviewBringBackLens.getByText(/If a returned source item needs more than local review, classify only the packet question; do not create records, tasks, timing fields, commitments, reports, or writes here/i)).toBeVisible()
  const customerSiteClarificationBringBackLens = fieldStartCustomerSiteQuestions.locator('section#pm-field-start-customer-site-clarification-bring-back-lens[aria-label="Local field-start customer/site clarification bring-back lens"]')
  await expect(customerSiteClarificationBringBackLens).toBeVisible()
  await expect(customerSiteClarificationBringBackLens.getByRole('heading', { name: /Local Field Start Customer\/Site Clarification Bring-Back Lens/i })).toBeVisible()
  const customerSiteClarificationBringBackLensControls = customerSiteClarificationBringBackLens.locator('[aria-label="Local field-start customer/site clarification bring-back lens controls"]')
  await expect(customerSiteClarificationBringBackLensControls).toBeVisible()
  await expect(customerSiteClarificationBringBackLensControls.locator('a')).toHaveCount(5)
  await expect(customerSiteClarificationBringBackLensControls.getByRole('button')).toHaveCount(0)
  await expect(customerSiteClarificationBringBackLens.getByText('clarification lens', { exact: true })).toBeVisible()
  await expect(customerSiteClarificationBringBackLens.getByText(/Read-only customer\/site clarification lens for returned field-start conversation items/i)).toBeVisible()
  await expect(customerSiteClarificationBringBackLens.getByText(/helps classify access, shutdown, escort, contact, safety, and constraint answers as browser-local review context only/i)).toBeVisible()
  await expect(customerSiteClarificationBringBackLens.getByText(/creates no meeting note, localStorage key, export artifact, backend route, task, action item, owner, due date, customer commitment, customer report, field instruction, durable record, hosted write claim, or production write/i)).toBeVisible()
  const customerSiteClarificationBringBackLensStorageKeys = await page.evaluate(() => Object.keys(window.localStorage).filter((key) => key.startsWith('pm-import-intake-') && /customer.?site.?clarification|clarification.?lens|access.?shutdown|escort.?contact|safety.?loto|constraint.?answer|promise.?stop|owner|due.?date|action.?item|customer.?commitment|customer.?report|field.?instruction|durable.?record/i.test(key)))
  ...
  await expect(customerSiteClarificationBringBackLens.getByRole('link', { name: /Constraint answer boundary/i })).toHaveAttribute('href', '#guardrails')
  await expect(customerSiteClarificationBringBackLens.getByRole('link', { name: /Customer\/site promise stop line/i })).toHaveAttribute('href', '#guardrails')
  await expect(customerSiteClarificationBringBackLens.getByText(/Miner Temp Power: if the return names access, shutdown, escort, gate, badge, or site-entry details, keep it as customer\/site clarification only/i)).toBeVisible()
  await expect(customerSiteClarificationBringBackLens.getByText(/If the return depends on a customer contact, site escort, observer, or conversation timing, keep that context local and do not assign ownership here/i)).toBeVisible()
  await expect(customerSiteClarificationBringBackLens.getByText(/If the return names safety, LOTO, PPE, escort, or access restrictions, classify it for local customer\/site clarification before field reliance/i)).toBeVisible()
  await expect(customerSiteClarificationBringBackLens.getByText(/If the return names shutdown windows, outage constraints, access hours, timing assumptions, or customer-facing language, classify only the packet question/i)).toBeVisible()
  await expect(customerSiteClarificationBringBackLens.getByText(/if a returned customer\/site answer needs accountability, timing, customer-facing language, field direction, report, schedule\/status update, durable record, or write authority, stop and author a later bounded packet; do not create promises here/i)).toBeVisible()
  const leadResourceClarificationBringBackLens = fieldStartCustomerSiteQuestions.locator('section#pm-field-start-lead-resource-clarification-bring-back-lens[aria-label="Local field-start lead/resource clarification bring-back lens"]')
  await expect(leadResourceClarificationBringBackLens).toBeVisible()
  await expect(leadResourceClarificationBringBackLens.getByRole('heading', { name: /Local Field Start Lead\/Resource Clarification Bring-Back Lens/i })).toBeVisible()
  const leadResourceClarificationBringBackLensControls = leadResourceClarificationBringBackLens.locator('[aria-label="Local field-start lead/resource clarification bring-back lens controls"]')
  await expect(leadResourceClarificationBringBackLensControls).toBeVisible()
  await expect(leadResourceClarificationBringBackLensControls.locator('a')).toHaveCount(5)
  await expect(leadResourceClarificationBringBackLensControls.getByRole('button')).toHaveCount(0)
  await expect(leadResourceClarificationBringBackLens.getByText('lead/resource lens', { exact: true })).toBeVisible()
  await expect(leadResourceClarificationBringBackLens.getByText(/Read-only lead\/resource clarification lens for returned field-start conversation items/i)).toBeVisible()
  await expect(leadResourceClarificationBringBackLens.getByText(/helps classify lead, crew-readiness, material, equipment, staging, and resource-limit details as browser-local review context only/i)).toBeVisible()
  await expect(leadResourceClarificationBringBackLens.getByText(/creates no meeting note, localStorage key, export artifact, backend route, task, action item, owner, due date, assignment, schedule\/status write, customer commitment, customer report, field instruction, durable record, production tracking row, hosted write claim, or production write, and exposes no buttons/i)).toBeVisible()
  const leadResourceClarificationBringBackLensStorageKeys = await page.evaluate(() => Object.keys(window.localStorage).filter((key) => key.startsWith('pm-import-intake-') && /lead.?resource.?clarification|lead.?resource.?lens|lead.?conversation|crew.?readiness|material.?equipment|staging.?resource|resource.?authority|task|action.?item|owner|due.?date|assignment|schedule.?status|durable.?record|production.?tracking|report|export|storage.?key|backend.?route|button/i.test(key)))
  ...
  await expect(leadResourceClarificationBringBackLens.getByText(/If the return names material, rental, equipment, tooling, delivery, or receiving details, classify it as lead\/resource clarification only before any later packet/i)).toBeVisible()
  await expect(leadResourceClarificationBringBackLens.getByText(/If the return names laydown, staging, access-for-equipment, work area, or resource limits, keep it as browser-local review context only/i)).toBeVisible()
  await expect(leadResourceClarificationBringBackLens.getByText(/if a returned lead\/resource answer needs a task, action item, owner, due date, assignment, timing field, schedule\/status write, durable record, production tracking, report, export, backend route, storage key, button, or write authority, stop and author a later bounded packet; do not create it here/i)).toBeVisible()
  const laterBoundedPacketCandidateBringBackLens = fieldStartCustomerSiteQuestions.locator('section#pm-field-start-later-bounded-packet-candidate-bring-back-lens[aria-label="Local field-start later bounded packet candidate bring-back lens"]')
  await expect(laterBoundedPacketCandidateBringBackLens).toBeVisible()
  await expect(laterBoundedPacketCandidateBringBackLens.getByRole('heading', { name: /Local Field Start Later Bounded Packet Candidate Bring-Back Lens/i })).toBeVisible()
  const laterBoundedPacketCandidateBringBackLensControls = laterBoundedPacketCandidateBringBackLens.locator('[aria-label="Local field-start later bounded packet candidate bring-back lens controls"]')
  await expect(laterBoundedPacketCandidateBringBackLensControls).toBeVisible()
  await expect(laterBoundedPacketCandidateBringBackLensControls.locator('a')).toHaveCount(5)
  await expect(laterBoundedPacketCandidateBringBackLensControls.getByRole('button')).toHaveCount(0)
  await expect(laterBoundedPacketCandidateBringBackLens.getByText('packet candidate lens', { exact: true })).toBeVisible()
  await expect(laterBoundedPacketCandidateBringBackLens.getByText(/Read-only later bounded packet candidate lens for returned field-start conversation items/i)).toBeVisible()
  await expect(laterBoundedPacketCandidateBringBackLens.getByText(/future packet for authority, evidence, owner\/timing, customer-facing language, or write-path admission/i)).toBeVisible()
  await expect(laterBoundedPacketCandidateBringBackLens.getByText(/creates no meeting note, localStorage key, export artifact, backend route, task, action item, owner, due date, assignment, schedule\/status write, customer commitment, customer report, field instruction, durable record, production tracking row, hosted write claim, or production write, and exposes no buttons/i)).toBeVisible()
  const futureBoundaryReminder = laterBoundedPacketCandidateBringBackLens.locator('#pm-field-start-later-bounded-packet-future-boundary-reminder[aria-label="PM Lane 203 future packet boundary reminder"]')
  ...
  const sourceResourceQuestionPrepCue = fieldQuestions.getByLabel('No-live source and resource question preparation cue')
  await expect(sourceResourceQuestionPrepCue).toBeVisible()
  await expect(sourceResourceQuestionPrepCue.getByText(/No-live source and resource question prep/i)).toBeVisible()
  await expect(sourceResourceQuestionPrepCue.getByText(/Equipment inventory context: 343 rows available for question shaping only/i)).toBeVisible()
  await expect(sourceResourceQuestionPrepCue.getByText(/Technician capability context: 50 rows available for coverage questions only/i)).toBeVisible()
  await expect(sourceResourceQuestionPrepCue.getByText(/allowed question prep: clarify source lineage, crew\/tooling\/lift\/rental\/equipment logistics, capability coverage, and customer\/site constraints/i)).toBeVisible()
  await expect(sourceResourceQuestionPrepCue.getByText(/blocked until admitted: resource assignment, schedule\/status changes, procurement or rental commitments, customer commitments, warning acceptance, approval rows, and project import/i)).toBeVisible()
  await expect(sourceResourceQuestionPrepCue.getByText(/No live source access, source writeback, resource assignment, schedule\/status change, task creation, field authorization, approval, import, or business-state mutation is granted/i)).toBeVisible()
  ...
  const sourceResourceCueStateKeys = await page.evaluate(() => Object.keys(window.localStorage).filter((key) => key.startsWith('pm-import-intake-') && /source.?resource.?question.?prep|no.?live.?source.?resource|resource.?prep.?cue/i.test(key)))
  expect(sourceResourceCueStateKeys).toEqual([])
  await fieldQuestions.locator(':scope > summary').click()
  await expect(fieldQuestions).toHaveAttribute('open', '')
  await expect(fieldQuestionsBodyControls).toBeVisible()
  await expect(fieldQuestionsGroups).toBeVisible()
  ...
  await expect(fieldStartOperatorScript.getByText(/Field prep queue: 2 complete \/ 2 next \/ 1 blocked. Coverage: 2 covered, 0 partial, 3 open, 2 blocked. Agenda: 2 context, 3 ask, 1 confirm, 1 blocked/i)).toBeVisible()
  await expect(fieldStartOperatorScript.getByText(/Do not approve, import, authorize field work, assign crews, schedule\/status work, create durable field records, or start production tracking from this workbench. Field observations present: yes/i)).toBeVisible()
  await expect(fieldStartStopLineReview.getByText(/Miner Temp Power: field start authority remains blocked until field authority and tracking packet; preflight is 3 ready, 0 needs review, 2 blocked/i)).toBeVisible()
  await expect(fieldStartStopLineReview.getByText(/Use the operator script and local exports only as conversation context; \d+ blocked boundaries remain outside this browser-local review/i)).toBeVisible()
  await expect(fieldStartCustomerSiteQuestions.getByText(/Miner Temp Power: site access, escort, safety, or LOTO question context is captured locally; treat it as conversation only/i)).toBeVisible()
  await expect(fieldStartCustomerSiteQuestions.getByText(/Drawing\/source question context is captured locally for the PM and lead conversation/i)).toBeVisible()
  await expect(fieldStartCustomerSiteQuestions.getByText(/Customer\/site constraint questions are still open; ask about access windows, shutdown constraints, escort requirements, and site contact questions as conversation context only/i)).toBeVisible()
  await expect(fieldStartCustomerSiteQuestions.getByText(/Use PM follow-up notes and field observations only to frame questions; do not turn them into owner lists, assignments, customer commitments, customer reports, schedule\/status changes, or field direction/i)).toBeVisible()
  await expect(pmFollowupPromptReview.getByText(/Miner Temp Power: PM follow-up prompt is open; decide the next PM question before accountability, timing, field instruction, customer commitment, or status language is discussed/i)).toBeVisible()
  await expect(pmFollowupPromptReview.getByText(/Use the customer\/site question review to pick one return question; do not promise access, shutdown windows, dates, scope outcomes, or customer reporting/i)).toBeVisible()
  await expect(pmFollowupPromptReview.getByText(/Ask the lead which drawing, safety, material, equipment, or staging uncertainty needs clarification next; keep the answer as local conversation context/i)).toBeVisible()
  await expect(pmFollowupPromptReview.getByText(/Observation source context is available; ask what source record, site note, or workbook lineage should be reviewed next/i)).toBeVisible()
  await expect(pmFollowupPromptReview.getByText(/If the follow-up needs accountability, timing, customer-facing language, field direction, report, schedule\/status update, or durable record, stop here and author a later bounded packet; this section records none/i)).toBeVisible()
  await expect(conversationCloseoutPromptReview.getByText(/Miner Temp Power: conversation source context exists; bring back only the summary of what was clarified, what stayed open, and what needs later packet authority/i)).toBeVisible()
  await expect(conversationCloseoutPromptReview.getByText(/Bring back customer\/site clarifications as local review context only; do not turn access, shutdown, escort, or contact answers into promises or field direction here/i)).toBeVisible()
  await expect(conversationCloseoutPromptReview.getByText(/Lead\/resource return remains open; ask what material, equipment, staging, or lead clarification should be reviewed locally before field reliance/i)).toBeVisible()
  await expect(conversationCloseoutPromptReview.getByText(/Bring back the source record, drawing, workbook row, or site note that should be checked next; keep it as local source-review context/i)).toBeVisible()
  await expect(conversationCloseoutPromptReview.getByText(/If the conversation reveals a needed next move, bring back only the packet question; do not turn it into work lists, accountability fields, timing fields, commitments, reports, or writes here/i)).toBeVisible()
  await expect(bringBackSummaryTriageStrip.getByText(/Miner Temp Power: source-review context is present; open the source lens for returned drawing, workbook, site note, observer\/source, or work-area review only/i)).toBeVisible()
  await expect(bringBackSummaryTriageStrip.getByText(/Customer\/site clarification context is present; open the clarification lens for access, shutdown, escort, contact, safety, or constraint review only/i)).toBeVisible()
  await expect(bringBackSummaryTriageStrip.getByText(/No returned lead\/resource clarification context is captured yet; if one returns, keep it as browser-local review context only/i)).toBeVisible()
  await expect(bringBackSummaryTriageStrip.getByText(/No later bounded packet candidate context is captured yet; if one returns, classify the packet question without creating work here/i)).toBeVisible()
  await expect(bringBackReviewQueue.getByText(/Miner Temp Power: source-review context exists; route returned drawing, workbook row, site note, observer\/source, or work-area context to local source review only/i)).toBeVisible()
  await expect(bringBackReviewQueue.getByText(/Customer\/site clarification context exists; route returned access, shutdown, escort, contact, safety, or constraint answers to local review only/i)).toBeVisible()
  await expect(bringBackReviewQueue.getByText(/If the conversation returns lead, material, equipment, staging, or crew-readiness context, classify it as lead\/resource clarification only/i)).toBeVisible()
  await expect(bringBackReviewQueue.getByText(/If a returned item needs a move beyond local review, classify only the packet question; do not create work lists, timing fields, commitments, reports, records, or writes here/i)).toBeVisible()
  await expect(sourceReviewBringBackLens.getByText(/Miner Temp Power: drawing, workbook, or source fingerprint context is available; review it locally before relying on a returned field-start source item. Fingerprint: stat-fingerprint-abc123/i)).toBeVisible()
  ...
  await expect(sourceReviewBringBackLens.getByText(/If the return names an area, workpackage, apparatus, or location reference, check it against source context before any later packet/i)).toBeVisible()
  await expect(sourceReviewBringBackLens.getByText(/If a returned source item needs more than local review, classify only the packet question; do not create records, tasks, timing fields, commitments, reports, or writes here/i)).toBeVisible()
  await expect(customerSiteClarificationBringBackLens.getByText(/Miner Temp Power: access, shutdown, escort, safety, or site-entry clarification context is available; keep it as browser-local review context only before any field reliance/i)).toBeVisible()
  await expect(customerSiteClarificationBringBackLens.getByText(/Customer\/site or observer context exists; verify the escort, site contact, or conversation source before any later bounded packet/i)).toBeVisible()
  await expect(customerSiteClarificationBringBackLens.getByText(/Safety, LOTO, or access observation context exists; review it as customer\/site clarification only before field reliance/i)).toBeVisible()
  await expect(customerSiteClarificationBringBackLens.getByText(/If the return names shutdown windows, outage constraints, access hours, timing assumptions, or customer-facing language, classify only the packet question/i)).toBeVisible()
  await expect(customerSiteClarificationBringBackLens.getByText(/if a returned customer\/site answer needs accountability, timing, customer-facing language, field direction, report, schedule\/status update, durable record, or write authority, stop and author a later bounded packet; do not create promises here/i)).toBeVisible()
  ...
  await expect(leadResourceClarificationBringBackLens.getByText(/If the return names laydown, staging, access-for-equipment, work area, or resource limits, keep it as browser-local review context only/i)).toBeVisible()
  await expect(leadResourceClarificationBringBackLens.getByText(/if a returned lead\/resource answer needs a task, action item, owner, due date, assignment, timing field, schedule\/status write, durable record, production tracking, report, export, backend route, storage key, button, or write authority, stop and author a later bounded packet; do not create it here/i)).toBeVisible()
  await expect(laterBoundedPacketCandidateBringBackLens.getByText(/If the return needs a move beyond local review, classify only the future packet question; do not create work lists, timing fields, commitments, reports, records, or writes here/i)).toBeVisible()
  await expect(laterBoundedPacketCandidateBringBackLens.getByText(/If the return needs approval submission, import, field authorization, assignment, schedule\/status, durable record, production tracking, report, or billing authority, stop and author a later bounded packet/i)).toBeVisible()
  await expect(laterBoundedPacketCandidateBringBackLens.getByText(/If the return depends on a drawing, workbook, site note, observer, work area, or source record, classify the evidence needed for the future packet without storing it here/i)).toBeVisible()
  await expect(laterBoundedPacketCandidateBringBackLens.getByText(/If the return needs owner, due date, timing, customer-facing language, field direction, or accountability, classify only that future-packet need and keep this section read-only/i)).toBeVisible()
  await expect(laterBoundedPacketCandidateBringBackLens.getByText(/Miner Temp Power: if a returned item needs a task, action item, owner, due date, assignment, timing field, schedule\/status write, customer commitment, customer report, field instruction, durable record, production tracking, export, backend route, storage key, button, or write authority, stop and author a later bounded packet; do not create it here/i)).toBeVisible()
  ...
  expect(exceptionRegisterExport).toContain('- Warning code: PROJECT_DATA_ENTRY_FORMULA_ERRORS')
  expect(exceptionRegisterExport).toContain('- Next input needed: return exactly one PM Lane 238 label.')
  expect(exceptionRegisterExport).toContain('- Response format: return_exactly_one_pm_lane_238_label.')
  expect(exceptionRegisterExport).toContain('REQUEST_DATA_ENTRY_WORKBOOK_CORRECTION_NO_LIVE')
  expect(exceptionRegisterExport).toContain('- Outcome routes:')
  expect(exceptionRegisterExport).toContain('ACCEPT_DATA_ENTRY_WARNING_NON_BLOCKING_NO_LIVE: record no-live warning acceptance context; keep live admission separate')
  expect(exceptionRegisterExport).toContain('REQUEST_DATA_ENTRY_WORKBOOK_CORRECTION_NO_LIVE: open a no-live workbook-correction packet before warning acceptance or live admission')
  expect(exceptionRegisterExport).toContain('HOLD_DATA_ENTRY_WARNING_NO_LIVE: keep the warning unresolved and continue no-live readiness only')
  expect(exceptionRegisterExport).toContain('PROVIDE_EXACT_LIVE_ADMISSION_LATER: defer disposition until a later exact live-admission packet')
  expect(exceptionRegisterExport).toContain('- Valid return checklist:')
  expect(exceptionRegisterExport).toContain('accepted: exactly one PM Lane 238 Data Entry label')
  expect(exceptionRegisterExport).toContain('rejected: explanation text; paraphrase; REQUEST_SOURCE_CORRECTION_NO_LIVE; multiple labels; continuation instruction; live admission language without a later admitting packet')
  expect(exceptionRegisterExport).toContain('after valid label: record the label in a no-live decision packet and keep live admission separate')
  expect(exceptionRegisterExport).toContain('- Safe no-live continuation moves:')
  expect(exceptionRegisterExport).toContain('allowed no-live continuation: candidate/readiness review, packet drafting, Desktop Codex read-only scout review, and source/resource question preparation')
  expect(exceptionRegisterExport).toContain('requires exact PM label first: warning acceptance, workbook-correction action, live admission packet, approval POST, approval row, and project import')
  expect(exceptionRegisterExport).toContain('Desktop Codex boundary: review clarity and relay burden only; do not choose the PM label or mutate business state')
  expect(exceptionRegisterExport).toContain('- Source/resource question prep cue:')
  expect(exceptionRegisterExport).toContain('equipment inventory context: 343 rows available for question shaping only')
  expect(exceptionRegisterExport).toContain('technician capability context: 50 rows available for coverage questions only')
  expect(exceptionRegisterExport).toContain('allowed question prep: clarify source lineage, crew/tooling/lift/rental/equipment logistics, capability coverage, and customer/site constraints')
  expect(exceptionRegisterExport).toContain('blocked until admitted: resource assignment, schedule/status changes, procurement or rental commitments, customer commitments, warning acceptance, approval rows, and project import')
  expect(exceptionRegisterExport).toContain('Paraphrases or prior source-correction labels do not close this gate.')
  expect(exceptionRegisterExport).toContain('hosted-read currency')
  expect(exceptionRegisterExport).toContain('no approval POST, approval row, import write, source writeback, hosted call, or business-state mutation')
  ...
  expect(fieldBrief).toContain('No-live source and resource question preparation cue:')
  expect(fieldBrief).toContain('equipment inventory context: 343 rows available for question shaping only')
  expect(fieldBrief).toContain('technician capability context: 50 rows available for coverage questions only')
  expect(fieldBrief).toContain('This cue prepares PM/lead questions only. It does not grant live source access, source writeback, resource assignment, schedule/status change, task creation, field authorization, approval, import, or business-state mutation.')
  ...
  expect(fieldPrepPacket).toContain('No-live source and resource question preparation cue:')
  expect(fieldPrepPacket).toContain('allowed question prep: clarify source lineage, crew/tooling/lift/rental/equipment logistics, capability coverage, and customer/site constraints')
  expect(fieldPrepPacket).toContain('This cue prepares PM/lead questions only. It does not grant live source access, source writeback, resource assignment, schedule/status change, task creation, field authorization, approval, import, or business-state mutation.')
  ...
  expect(brief).toContain('## Project Data Entry Decision Gate')
  expect(brief).toContain('- Next input needed: return exactly one PM Lane 238 label.')
  expect(brief).toContain('- Response format: return_exactly_one_pm_lane_238_label.')
  expect(brief).toContain('ACCEPT_DATA_ENTRY_WARNING_NON_BLOCKING_NO_LIVE')
  expect(brief).toContain('Paraphrases or prior source-correction labels do not close this gate.')
  expect(brief).toContain('- Valid return checklist:')
  expect(brief).toContain('accepted: exactly one PM Lane 238 Data Entry label')
  expect(brief).toContain('rejected: explanation text; paraphrase; REQUEST_SOURCE_CORRECTION_NO_LIVE; multiple labels; continuation instruction; live admission language without a later admitting packet')
  expect(brief).toContain('after valid label: record the label in a no-live decision packet and keep live admission separate')
  expect(brief).toContain('- Safe no-live continuation moves:')
  expect(brief).toContain('allowed no-live continuation: candidate/readiness review, packet drafting, Desktop Codex read-only scout review, and source/resource question preparation')
  expect(brief).toContain('requires exact PM label first: warning acceptance, workbook-correction action, live admission packet, approval POST, approval row, and project import')
  expect(brief).toContain('Desktop Codex boundary: review clarity and relay burden only; do not choose the PM label or mutate business state')
  expect(brief).toContain('- Source/resource question prep cue:')
  expect(brief).toContain('equipment inventory context: 343 rows available for question shaping only')
  expect(brief).toContain('technician capability context: 50 rows available for coverage questions only')
  expect(brief).toContain('blocked until admitted: resource assignment, schedule/status changes, procurement or rental commitments, customer commitments, warning acceptance, approval rows, and project import')
  expect(brief).toContain('Desktop Codex review-only boundary')
  expect(brief).toContain('display/export context only')
  ...
  expect(brief).toContain('No-live source and resource question preparation cue:')
  expect(brief).toContain('equipment inventory context: 343 rows available for question shaping only')
  expect(brief).toContain('This cue prepares PM/lead questions only. It does not grant live source access, source writeback, resource assignment, schedule/status change, task creation, field authorization, approval, import, or business-state mutation.')
