...
test('pm import intake workbench renders consolidated read-only Project Miner gates', async ({ page }) => {
  ...
  await expect(outputActionRail.getByLabel('Field prep output actions').getByRole('button')).toHaveCount(13)
  ...
  await expect(operatingQueueItems.locator('article')).toHaveCount(7)
