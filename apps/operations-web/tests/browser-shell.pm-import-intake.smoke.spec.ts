...
test('pm import intake workbench renders consolidated read-only Project Miner gates', async ({ page }) => {
  ...
  await expect(validReturnChecklist.getByText(/rejected: explanation text; paraphrase; REQUEST_SOURCE_CORRECTION_NO_LIVE; multiple labels; live admission language without a later admitting packet/i)).toBeVisible()
  ...
  expect(exceptionRegisterExport).toContain('rejected: explanation text; paraphrase; REQUEST_SOURCE_CORRECTION_NO_LIVE; multiple labels; live admission language without a later admitting packet')
  ...
  expect(brief).toContain('rejected: explanation text; paraphrase; REQUEST_SOURCE_CORRECTION_NO_LIVE; multiple labels; live admission language without a later admitting packet')
