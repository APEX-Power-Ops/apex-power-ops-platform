...
- **B2.1 — ✅ DONE 2026-06-12 (STATE §216; apex `cff0368d`; decision-4 lean = page-integrated print-first, Excel export = a later slice).** Shipped as the lvbreakertcc Screen-2 "Field Sheet" print overlay: pure `lib/field-sheet.ts` over the SERVED `/calculate` + `/settings` (SC3 terminology) payloads — zero new backend, sheet↔page basis identity by construction. Element | Setting | Trust | Test × | Test Current | Expected | Min | Max | Method/Basis | writable Measured | writable Pass/Fail, with the SC3 captions + dial symbols, withheld †/est* honesty footnotes, and the NETA=mfr footer law line. Live-verified on the deployed page at sensor 3833. *(Original spec:)* **Tolerance-sheet export (the MVP minimum goal / north-star).** The Series B reference calculator IS the minimum bar for NETA field use: a **self-contained per-breaker sheet** that hands a tech everything they need — test currents, expected pickups, MIN/MAX tolerance bands, delay windows — so they do NOT go pull TCC curves and derive values on-site while testing. B2.1 produces that sheet, populated with **DB-authoritative** values, for a **confirmed** breaker/trip-unit (Element | Setting | Test current | Expected | MIN | MAX | Delay MIN/MAX | Method). Backend-driven export; a tech carries it. **Enabled by selection confirmation (SC1-SC4 — so it's the RIGHT sheet) + the `/plot-tcc` delay-field route fix (so the delay rows are correct).**
