...
from apex_calc_engine.services.calc_engine import log_log_intersect, merge_sst_curves
...
# ==============================================================
#   T = (rating / (amps × (1−α)))^n × K × band_mult
#   Coefficients read from DB (section=2 sensor params).

def test_b4_thermal_tuf():
    print("\n=== B4 Thermal-TUF (M5) — Sensor 7102 (DB) ===")
    fx = load_fixture('b4_thermal_tuf.json')
    ...
    try:
        with SessionLocal() as session:
            lc = ETULTDCalculator(session, fx['sensor_id'])

            # TUF uses curve_id=2 with _TUF_OPEN layout: alpha=0, k=1, n=2, min_t=3
            cid = fx['curve_id']
            alpha = lc._pc(cid, 0)
            K = lc._pc(cid, 1)
            n = lc._pc(cid, 2)
            min_t = lc._pc(cid, 3, 0.025)

            check("K loaded", K != 0, f"K={K}")
            check("n loaded", n != 0, f"n={n}")
            ...
            for grp in fx['golden_points_open']:
                ...
                for pt in grp['points']:
                    ...
                    check(f"BM={bm} I={I_mult} → {t:.6f} vs {exp:.6f}",
                          err < 0.5,
                          f"err={err:.4f}% (K={K}, n={n}, α={alpha})")
    except Exception as exc:
        pytest.skip(f"Live database unavailable for Thermal-TUF golden test: {exc}")
