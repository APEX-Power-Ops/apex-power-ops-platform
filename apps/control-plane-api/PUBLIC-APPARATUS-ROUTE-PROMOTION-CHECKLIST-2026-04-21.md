...
The hosted control-plane is otherwise healthy and correctly configured, but the public host still omits `/api/v1/neta/apparatus/{apparatus_id}/resources` from deployed OpenAPI and still returns framework `404 Not Found` for that route.
...
Choose one bounded promotion path:
...
On the current workstation, option 1 is the only executable path until the dispatch credential exists.
