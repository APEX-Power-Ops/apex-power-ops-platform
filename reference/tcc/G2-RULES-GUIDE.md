...
| **BG-5** | Only then move the ETU UI beyond manufacturer-axis disclosure | **✅ DELIVERED 2026-06-01 (end-to-end).** UX wiring shipped: `/lvbreakertcc` is live with family-dispatched ETU selection consuming `vw_breaker_sst_bridge` (`GET /etu/bridge-sensors`); the ETU breaker cascade is bridge-narrowed (`bridge_only` → only ETU-capable breakers, MCCB 640→130); the cross-filter is **bridge-aware bidirectional** (`bridge_xfilter`, opt-in: breaker↔trip each narrow the other by real compatibility — 130 trip styles→1, 1562 breakers→4); and the **co-equal dual-axis front-end** now consumes it (`99d0dc88`) — Breaker lane + Trip-Unit lane each narrow the other, the sensor is reachable from either end, live-verified (trip-first: Trip Style 155 → 44→2 breakers / 3 sensors; breaker-first: ICCB → sensor pool 17,831→444). The §97 manufacturer-only ceiling is now closed in the UX. The deployed *explorer* keeps manufacturer-axis only (opt-in preserved). The LV page is now **fully live — all 3 screens × all 3 families** (Screen 2 ETU live tolerance bands + NETA Test @, TMT bounded, EMT context-only; Screen 3 ETU+TMT live `/plot-tcc` curves, EMT bounded note). Remaining is only the **gated `route_delay_curve` delay-trust work** (promote PROVEN direct-band delays "verify"→"DB" + the InvEq G4-fixture validation — see G4 §4/§5). See handoff `2026-06-01-lvbreakertcc-live-wiring-closeout`. `[VERIFIED-LIVE 2026-06-01]` `[05]` §28 |
...
|---|---|---|---|
| **D-11** | **InvEq numerical parity** — `IEEEInverseTimeSolver` vs EasyPower native `CalcThermEq`/`CalcAnsiEqGF` on ~6,200 InvEq sensors (4,524 STD-side + 1,713 GF-side) | ~~Captured EasyPower point fixtures remain the gate~~ **CLOSED — all three native families bit-exact** | **✅ FULLY CLOSED 2026-06-10 (#120)** `[06]` — **PRECISELY CHARACTERIZED 2026-05-31, SPLIT + PATCHED 2026-06-01, FULLY CLOSED 2026-06-10** (G4 §3e/§5): corpus = **STD 100% Therm, GF 8,450 Therm + 100 Ansi ROWS / 23–25 sensors / 3 trip styles**. All three native families now reproduced BIT-EXACT against the executed `TccBase.dll` kernel: STD Therm (§107) + GF Therm plug-basis (§206, `field[13]`=plug, `gf_inveq_field13_native_parity.json`) + **GF ANSI (#120, `CalcAnsiEqGF` `A + B/(M-C) + D/(M-C)² + E/(M-C)³` floored Tmin; 18-tuple / 10,398-point `gf_inveq_ansi_native_parity.json`; standalone pickup-basis lane — `id_op_i_calc=8`→byICalc=0; un-excluded `gf_inveq_is_excluded_ansi`→`gf_inveq_is_ansi`; trust→db; served via the #119 availability gate accepting `in_out=1` ANSI bands)**. Tests: `test_gf_inveq_ansi_native_parity` + `test_gf_inveq_field13_native_parity` + `test_etu_delay_routing`. No residual. `[VERIFIED-LIVE 2026-06-10]` |
...
| **Status** | **✅ RECOVERED 2026-06-01 (D1, migration `006_brk_styles_sst_bridge.sql`).** The 4 cols re-carried (source-faithful NAME strings, NOT coerced to FK) on all 3 style tables via the proven `rank=id` mapping; BG-4 surface `tcc.vw_breaker_sst_bridge` built; the 325 day-one orphan MCCB styles repointed (0 orphans). `tmt_use_sst` true = 1704/515/3193. Match-rates ICCB 100 / MCCB 95.6 / PCB 97.5% (non-null) == Access live-join; `T8V-1600`→ABB PR332/P→5 sensors live. **Residual:** `source_id` now **POPULATED + NOT NULL + UNIQUE** (migration `007`, the stable alternate key); **D4** `TMT_*` helper cols still OPEN (co-located, not re-carried — now a trivial `source_id`-join); **BG-5** UX wiring **✅ DELIVERED 2026-06-01** (`/lvbreakertcc` live; `bridge_only` cascade narrowing + `bridge_xfilter` bidirectional cross-filter + `/etu/bridge-sensors`). `[VERIFIED-LIVE 2026-06-01]` |
