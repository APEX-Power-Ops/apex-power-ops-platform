...
| **BG-5** | Only then move the ETU UI beyond manufacturer-axis disclosure | **✅ SUBSTANTIALLY DELIVERED 2026-06-01.** UX wiring shipped: `/lvbreakertcc` is live with family-dispatched ETU selection consuming `vw_breaker_sst_bridge` (`GET /etu/bridge-sensors`); the ETU breaker cascade is bridge-narrowed (`bridge_only` → only ETU-capable breakers, MCCB 640→130); and the cross-filter is now **bridge-aware bidirectional** (`bridge_xfilter`, opt-in: breaker↔trip each narrow the other by real compatibility — 130 trip styles→1, 1562 breakers→4). The deployed *explorer* keeps manufacturer-axis only (opt-in preserved). Remaining: the front-end trip-unit axis (co-equal dual-axis), TMT/EMT Screen 2, Stage C curve. See handoff `2026-06-01-lvbreakertcc-live-wiring-closeout`. `[VERIFIED-LIVE 2026-06-01]` `[05]` §28 |
