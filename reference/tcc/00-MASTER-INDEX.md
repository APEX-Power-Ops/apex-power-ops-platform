...
- **InvEq numerical parity — SPLIT + HALF-CLOSED (G4 §3e/§5).** Corpus is STD 100% Therm / GF 8,450 Therm + **100 Ansi ROWS / 23 sensors / 3 trip styles** (count corrected 2026-06-01 — was "100 sensors"; the §98 "Federal Pioneer" attribution is now `[OPEN-VALIDATION]`). The managed IEEE/Therm solver ignores coeffs c4/c5 and **mis-evaluates the Ansi rows** (empty curve on `id_open` c2=0; wrong curve on the others — the C37.112 exponent sits in the ignored c5 slot). **(a) Ansi half — ✅ CLOSED 2026-06-01 (lean 1):** hard-excluded at the dispatch layer (`etu_delay_routing.py`; INV-7; 54/54 tests). **(b) Therm half — OPEN:** one bounded **local** Ghidra fn (`CalcThermEq` in `D:\EasyPower\EasyPower.exe`) moves the 31,070-row Therm corpus BOUNDED→PROVEN — bounded + locally runnable (the CalcThermEq packet).
