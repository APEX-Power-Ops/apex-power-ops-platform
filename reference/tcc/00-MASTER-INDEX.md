...
- **InvEq numerical parity — SPLIT + PATCHED (G4 §3e/§5).** Corpus is STD 100% Therm / GF 8,450 Therm + **100 Ansi ROWS / 23 sensors / 3 trip styles** (count corrected 2026-06-01 — was "100 sensors"; the §98 "Federal Pioneer" attribution is now `[OPEN-VALIDATION]`). `CalcThermEq` was recovered from `TccBase.dll` 2026-06-01 and **does use c4/c5** (`rIref`/`rM`); the managed Therm branch was patched to the native formula in `etu_curves.py`. `CalcAnsiEqGF` was also recovered, but the 100-row Ansi family remains **hard-excluded** until a family-aware ANSI solver path has captured EasyPower fixtures. Residual: captured EasyPower point parity for Therm + ANSI path decision.
