...
| `/etu/breaker-alt-trips` | breaker→alt trip (retrofit) | `tcc.breaker_alt_trip_bridge` + `vw_trip_unit_cascade` | **NEW 2026-06-08.** `?breaker_style_id&breaker_class` → the breaker's alternative/retrofit trips (e.g. WavePro→EntelliGuard TU) + per-trip L/S/I/G class; isolated read, does NOT widen the hot cascade/bridge queries (DURABLE 23) `[VERIFIED-LIVE 2026-06-08]` `[STATE §192–§194]` |
...
### A3e. Alt-trip bridge — retrofit / multi-trip breakers (✅ SHIPPED 2026-06-08, STATE §192–§194)

The native SST bridge (`TMT_SST_*`) is **single-valued** per breaker style — it names ONE compatible trip and cannot express a breaker that accepts an **alternative / retrofit** trip (a field upgrade). `tcc.breaker_alt_trip_bridge` (migration `017`) maps a breaker **`(class, style_id)`** → an alternative `trip_style_id` (+ `sensor_id`), `[VENDOR-DOC]`-cited per row, and is **UNION-ed into `tcc.vw_breaker_sst_bridge`** (branch 2 exposes the alt trip's `sensor_id` + `trip_style_id`). Every bridge consumer (`/etu/bridge-sensors`, `/cascade?bridge_xfilter`, `/etu/breaker-cascade?bridge_xfilter`) therefore sees native **+** alt trips uniformly — no change to the hot queries. `[VERIFIED-LIVE 2026-06-08]`

- **Canonical case — GE WavePro → EntelliGuard TU retrofit:** WavePro (MVT-PM) LVPCBs accept the EntelliGuard TU as a drop-in retrofit; each WavePro frame serves its native MVT-PM trip **plus** EntelliGuard TU `1073` (LSIG) + `1566` (LIG), rating-matched. `[VENDOR-DOC]` GE `DET167C` + WavePro Application Manual + ABB `1SDC200139` (MVT→EG-TU conversion kit) + `1SDC001083` (EG-TU promo) — manifest rows in `catalogs/CATALOG-INDEX.csv`. `[VERIFIED-LIVE 2026-06-08]`
- **Serving — isolated read endpoint** `GET /etu/breaker-alt-trips?breaker_style_id&breaker_class` returns the alt-trip relation for a breaker + the SQL-derived **L/S/I/G protection class** per alt trip (from `vw_trip_unit_cascade.has_*`), keyed by `trip_style_id`. The frontend tags both selection paths `(retrofit · LSIG/LIG)` and de-dups the model-name stutter. `[VERIFIED-LIVE 2026-06-08]`
- **Per-class id hazard applies:** `breaker_alt_trip_bridge` keys on the **`(class, style_id)` PAIR** — breaker/style ids collide across ICCB/MCCB/PCB; always class-qualify. `[G1]` `[A3d]`

> **General mechanism, not a one-off.** The alt-trip bridge is the durable home for ANY breaker → alternative / retrofit trip (or a future multi-trip breaker). The native `tmt_sst_*` stays the primary single-valued bridge; alternatives accrue in `breaker_alt_trip_bridge`. Data-quality note shipped with it: EasyPower models `1566` "Wavepro LI" **with** ground (derives LIG) — tagged data-faithfully; whether that GF is real or a library artifact is a pending operator data call. DURABLE 22 (class from `has_*`, trust model over name) · 23 (isolated endpoint + keyed lookup, don't widen the hot query) · 30 (scope a cross-filter axis's leaf options to its parent — the §200 EntelliGuard duplicate-LSIG fix). `[STATE §192–§194, §200]`
