...
> - count corrections applied below: `EMT` 1,177→**174**, `Relays` 8,395→**1,442**, MCCB `Use_SST=1` 1,704→**1,680**. Governed-load delta CLOSED 2026-06-01: `tcc.brk_mccb` now 640/640 vs Access after widening the breaker parent unique key and reloading the 41 AC/DC + ANSI/IEC twins.
...
> **SST-bridge data-quality finding — the D1 re-carry is bit-faithful; the mis-mappings are NATIVE to EasyPower's source `[VERIFIED-LIVE 2026-06-04]` (STATE §138–§141).** Re-examined the §104 D1 load against raw Access (`BreakerMCCBStyles`) after a field-facing bug (Eaton PDG6 2500 A frames offered 60–225 A sensors):
> - **`rank=id` is BIT-EXACT** — prod `brk_*_styles.id` ↔ the Access ID-ascending row position (verified two ways: prod 8773 ↔ `COUNT(ID≤98922)`=8773; 8507 ↔ 95752). The §104 re-carry did **not** scramble.
> - **Prod `tmt_sst_*` is BIT-FAITHFUL to Access** — the *real* (non-empty, non-null) triple counts equal prod exactly (pcb 2218 / mccb 1649 / iccb 515); prod correctly normalizes Access's **`""` empty-string** "SST-enabled, no target" rows to NULL (the **975 PCB blank rows above = the empty-string set**, faithfully represented — a false "940-row load gap" alarm was caught here via the empty-string-vs-NULL trap).
> - **The wrong targets are verbatim in EasyPower's library:** Eaton Power Defense PDG3/4 → `(PXR20/25, NRX-LSI(RF))` [800–4000 A], PDG5/6 "PXR 10" → `(PXR 10, PDG2-LSI)` [60–225 A] — EasyPower's own mis-attribution of the trip type on the newer frames. Scope (4382 served bridged styles): 4253 resolve to sensors, **129 dangling** (no `DatStyle`), **~277 rating-mismatch** (breaker `r_cont_current` outside the SST sensor envelope). The bridge view (`tcc.vw_breaker_sst_bridge`) historically surfaced **all** sensors of the target style with no rating narrowing because prod dropped Access `r_cont_current`.
> - **Fix (APPLIED + LIVE):** a cited `[VENDOR-DOC]` SST-correction (Eaton TD012064/065/067/068EN + PXPM) re-pointed each defective PD frame to its rating-correct PXR-LSI style — **all 31 Eaton PD rows corrected AND applied to governed prod** (PDG5 migr `009`; PDG3→PDG3-LSI, PDG6→PDG6-LSIGM migr `010`; via `apply_migration`, §144). End-to-end: PDG3→125–600 A · PDG5→800–1600 A · PDG6→1600–2500 A. Curve model + map at `services/neta/pxr_curves.py` + `EATON-POWER-DEFENSE-PXR.md` (#73 verified the PD-LSI curves are already faithful, §142). `[VENDOR-DOC]` `[STATE §138–§146]`
> - **`r_cont_current` rating-narrow (RESOLVED §145, migr `011`/`012`, LIVE):** re-carried `r_cont_current` onto `brk_{mccb,pcb,iccb}_styles` (5412 bridged rows, `source_id`=rank=id join, id-weighted Access-vs-prod checksum EXACT) + appended it to `vw_breaker_sst_bridge`; `/etu/bridge-sensors` now **rating-narrows** to the sensor matching the breaker (`narrow_bridge_sensors_by_rating`, exact-match-else-full-set — safe). Coverage: 3054 narrow to 1 + 699 subset + 143 single = **~89% of 4382**; 357 imperfect + 129 dangling fall back safely. `[VERIFIED-LIVE 2026-06-04]`
> - **Remaining (#74):** the 230 out-of-range non-Eaton rating-mismatches + 129 dangling triples — need per-mfr authoritative docs. ETAP decoded catalog (§146) corroborates curve SHAPE but is gated to **internal cross-val only** (OTI proprietary — persist nothing into the product). `[STATE §146]`
...
>
> **Per-class inst-gate column-name split (added 2026-06-27 `[VERIFIED-LIVE]` `[03]`):** the engine reads `TMT_Thermal` on **ICCB** and `TMT_ThermalMagnetic` on **MCCB** - both bind to the SAME model property `TMTThermalMagnetic` (`TMTUseInstantaneous = (TMTThermalMagnetic == 0)` gates whether the TMT frame exposes an instantaneous-settings list). So `TMT_Thermal` carries the identical `0 = With Adjustable Instantaneous / 1 = Without` legend; on ICCB `TMT_ThermalMagnetic` is vestigial (all 0) and vice-versa. `[DLL DevLibBreakerStyle.GetBreakerStyleListSql]`
...
- **Inst-override blocks** `InstOvr*` + `NInstOvr*` (16/15 cols each, 3 tables) - **`N` = Non-Instantaneous** (breaker operated with instantaneous defeated / short-time-only), NOT an In-vs-Ir basis. **Resolved 2026-06-27** `[NATIVE-BOUNDED]`: DvlEng confirms the **input layout only** (recoverable); the enum legends + curve math are native in `TccBase` (symbol-stripped). DvlEng reads the `InstOvr*`/`NInstOvr*` field indexes `[DLL DvlEng/-Module-.cs:11961]`, loads base scalars `[:12155]` + NInst scalars `[:12164]`, and **falls back NInst -> Inst when the N columns are absent** `[:12172]` (= why `NInstOvrAmps == InstOvrAmps` in the common case live; the rows that differ carry real N data); four override curve classes **0-3** read via `ReadBreakerOvrCurve` `[:12278]` `[:15426]`. `CdvlInstOvr` is an empty 208-byte native container `[DLL DvlEng/CdvlInstOvr.cs:4]` (copy/clear preserves the four curve arrays `[:20875]`) - a layout/container, NOT the applicator. **Layout:** scalars 0-52 (alternating base/N), selector/calc ~64-84 + 96-116, entered-at floats 88/92/120/124, curve arrays 128/148/168/188. `[VERIFIED-LIVE 2026-06-27]` `[DEFERRED behavior]`
- **Non-instantaneous interrupt ratings** `r_int_ninst_*` / `r_iec_ninst_*` (**PCB-only**; ICCB/MCCB carry only `inst`/`series`) - interrupting/withstand rating with instantaneous defeated. Live: PCB `r_int_ninst < r_int_inst` in **585/1431**, equal in **842**, never higher (a physical rating distinction, not a setting basis). Getter `CTccCurveBase.GetIntKaNonInst` `[DLL TccBase/-Module-.cs:24034]`; DvlEng carries the `r_*_ninst_*` strings (vocabulary real) but not the getter. `[VERIFIED-LIVE 2026-06-27]`
...
> **TMT-frame lineage correction (F-79-03, 2026-06-28):** the `Breaker_TMTFrameSizes → tcc_tmt_frames` row above is **not a pure 1:1 Access load** — the governed `tcc.tmt_frames` is a **transformation**. Verified live (governed `access_raw` vs prod `tcc.tmt_*`, cross-engine): tcc carries **no Access-frame ID** (cols `id`/`breaker_style_id`/`breaker_class`/`size`; `id` is a re-sequenced surrogate, `size` ≠ Access `FrameDesc`), so there is **no honest Access-frame ↔ tcc-frame correspondence** — a direct `Breaker_TMTFrameSizes.ID = tcc.tmt_frames.id` anti-join is **FALSE evidence and must not be used**. On the clean style hop (`tmt_frames.breaker_style_id → brk_*_styles.source_id`): MCCB frames are Access-faithful (30,809/30,809 matched, corr 0.97), but ~10,412 tcc **PCB** frames are **derived** (style-matcher behavior, `FindMatchingBreakerStyles`) onto styles no Access frame references, while ~9,629 MCCB-only Access frames have no tcc frame. So the `tmt_frames` −169 row-count delta (§5) is the **residual of a frame→style/class transformation**, not a 169-row truncation. The exact matcher rule is not fully characterized here. Evidence: `reference/tcc/_access-evidence/F-79-03-evidence-packet.md` (+ reproducible `…-frame-style-localization.sql`). `[VERIFIED-LIVE 2026-06-28]`
...
|---|---|---|---|---|---|---|
| **D1** | `tmt_use_sst`, `tmt_sst_mfr`, `tmt_sst_type`, `tmt_sst_style` | `BreakerICCBStyles` / `BreakerMCCBStyles` / `BreakerPCBStyles` | `tcc.brk_iccb_styles` / `brk_mccb_styles` / `brk_pcb_styles` | This is the **SST-1 breaker→ETU bridge** — the columns that make "T8-1600 → ABB PR332/P → exact sensor set" possible. Without them the deployed cross-filter was **manufacturer-axis only** (offered 117 ABB trips instead of the 1 correct trip). | **Name-vs-id loader assumption:** `tmt_sst_mfr` is a manufacturer **NAME** string, but the loader expected a numeric `Manufacturers.ID` FK, so the bridge join silently dropped and the 4 columns were not carried. | **✅ CLOSED 2026-06-01 (migration `006_brk_styles_sst_bridge.sql`).** All 3 style tables now carry `tmt_use_sst`(bool) + `tmt_sst_{mfr,type,style}`(text, source-faithful NAME strings — NOT coerced to FK) + `source_id` (= Access `Breaker*Styles.ID`, **populated + NOT NULL + UNIQUE**, migration `007`; the stable alternate key for any future D4/D5 re-carry). Re-carried from Access via the **PROVEN `rank=id`** mapping (Supabase `brk_*_styles.id` == Access `Breaker*Styles` row position by `ID` asc; 46/46 spot-matches + per-class md5). `tmt_use_sst` true = 1704 mccb / 515 iccb / 3193 pcb (= Access `TMT_Use_SST<>0`; incl. 22×255 byte-wrapped True + 2×2; 725 NULL→false). **D4 (`TMT_*` helper cols) co-located and STILL OPEN** — only the 4 SST cols were re-carried this pass. Bridge surface = `tcc.vw_breaker_sst_bridge` (G0 §3); match-rates ICCB 100 / MCCB 95.6 / PCB 97.5% (non-null) == Access live-join. `[VERIFIED-LIVE 2026-06-01]` |
...
| **D4** | TMT helper cols `TMT_TCCNumber`, `TMT_Notes`, `TMT_TripPlug`, `TMT_BreakerType`, `TMT_ThermalMagnetic`, `TMT_Thermal` (ICCB/MCCB only) | `BreakerICCBStyles` / `BreakerMCCBStyles` | `tcc.brk_iccb_styles` / `brk_mccb_styles` | These describe the **thermal-magnetic alternative** used when `TMT_Use_SST=0` and decode the TMT breaker sub-type (`0=Thermal Magnetic / 1=Motor Circuit Protector`, etc.). Their absence weakens TMT-breaker characterization. | Carried along with the D1 drop (same `TMT_*` column block) / not part of the migrated `brk_*_styles` projection. | **CONFIRMED DROPPED + CHARACTERIZED 2026-06-27.** Live-confirmed absent (028 hazard `d4_tmt_helper_columns_absent_from_projection` + structural probe). Decodes (provenance per field): `TMT_BreakerType` 0=TM/1=MCP, `TMT_ThermalMagnetic` 0=with-adj-inst/1=without, `TMT_TripPlug` 0=Trip/1=Plug (these three `[DVL-DB §3.1]`); `TMT_Thermal` = the ICCB-class spelling of `TMT_ThermalMagnetic` (DB-undescribed; `[DLL §3.1 split]`); `TMT_TCCNumber` = free-text vendor-doc ref (Text, ~99% empty) + `TMT_Notes` = human memo (both DB-undescribed; `[VERIFIED-LIVE]`). **The engine consumes only `TMT_Use_SST` + the inst-gate flag** (`TMT_Thermal`/`ThermalMagnetic`) via `GetBreakerStyleListSql`; `BreakerType`/`TripPlug`/`TCCNumber`/`Notes` are metadata not read by the serving cascade. Re-carry rides `source_id` (D1 precedent). `[VERIFIED-LIVE 2026-06-27]` `[03]` |
| **D5** | `InstOvr*` + `NInstOvr*` inst-override blocks (≈16 cols each); `BrkTimes*50/60`; LV `r_int_*` / `r_iec_*` rating columns | `BreakerICCB/MCCB/PCBStyles` | `tcc.brk_*_styles` | Frame-limited instantaneous override (amps/tolerances/open-clear delay+radius) + mechanism timing + interrupt ratings. Needed for full breaker-override fidelity + ANSI/IEC rating display. | Undescribed columns outside the minimal migrated projection (`FindMatchingBreakerStyles` selects only `Mfr_Name, Type, Style`). | **DEFERRED (serving) + `[NATIVE-BOUNDED]` (data) 2026-06-27.** `N`/`ninst` = Non-Instantaneous (§3.4, resolved). DvlEng confirms the input surface is real, but the override application + enum legends + curve math are native (`CdvlInstOvr` 208-byte shell + `TccBase` curve recalc) = recoverable input LAYOUT only. **Raw re-carry IS possible** (source columns, or a side table for `InstOvr*`/`NInstOvr*`/`BrkTimes*`/`r_int_*`/`r_iec_*`/`Breaker_OvrCurves`, tagged `native_bounded`), keyed on **`(breaker_class, source_id)`** for a shared side table or per-class tables (§2B: `source_id` collides across classes), WITHOUT wiring to serving (G0 §4). `[NATIVE-BOUNDED]` `[VERIFIED-LIVE 2026-06-27]` |
...
> **Governed-load deltas (NOT dropped columns — row-count shortfalls vs Access source):** a handful of `tcc.*` tables carry fewer rows than their Access source. These are *load-completeness* notes distinct from the *dropped-column* (schema-projection) gaps D1–D6.
> - **`tcc.brk_mccb` = 640 vs Access `BreakerMCCB` = 640 — CLOSED 2026-06-01 (lean 2; the earlier "RI orphan drops" guess was WRONG).** The governed table had carried `UNIQUE (manufacturer_id, name)`, but the Access natural key is the **4-tuple `(Mfr_ID, Type, cStandard, Acdc)`** (0 dups there). 43 `(Mfr_ID, Type)` pairs have a twin differing **only by `Acdc` (AC↔DC) and/or `cStandard` (ANSI↔IEC)**; the too-narrow index collapsed them. **41, not 43:** 2 survived because their `name` differs only by letter-case (`Multi 9`/`MULTI 9`, `ComPact [IEC]`/`COMPACT [IEC]`) — Access groups case-insensitively, Postgres unique is case-sensitive. **Not a manufacturer-resolution (D1) drop:** `manufacturer_id` is preserved 1:1, all FKs resolve, and `brk_iccb` (29→29) + `brk_pcb` (157→157) loaded fully. Remediation: migration `infra/database/migrations/tcc/005_brk_loadpath_unique_key_reload.sql` widened `brk_{mccb,iccb,pcb}` to `UNIQUE NULLS NOT DISTINCT (manufacturer_id, name, standard, ac_dc_code)`, normalized existing parent tuples (MCCB 121 rows, PCB 10 rows), and inserted the 41 missing MCCB twins. Final live counts: `brk_mccb` 640/640, `brk_iccb` 29/29, `brk_pcb` 157/157; each Access 4-tuple present exactly once. G2 §4.6. `[VERIFIED-LIVE 2026-06-01]`
> - `tcc.tmt_thermal_adj` 14,620 vs 21,790; EMT child tables a few rows under Access (these remain RI-orphan/derived deltas, not re-characterized this pass). `[VERIFIED-LIVE 2026-05-31]`
> - **`tcc.brk_mccb_styles` 325 orphan rows (41 dead parent surrogates) — CLOSED 2026-06-01 (D1, migration `006`).** Root cause: the original load preserved a row-ordered surrogate (`rank=id`) but the too-narrow `(manufacturer_id,name)` parent key dropped 41 AC/DC & ANSI/IEC twins, leaving their styles pointing at parent ids that were never inserted. Migration 005 re-inserted the twins at **new** ids (gaps not back-filled), so the 325 styles stayed orphaned until D1 repointed them (frame-guarded, via Access `BreakerID → BreakerMCCB` 4-tuple → the 005 twin). The child FK `tcc_brk_mccb_styles_breaker_id_fkey` (convalidated, ON DELETE CASCADE) had carried these violations because they predate it / the load bypassed enforcement; data now satisfies it. 0 orphans live. `[VERIFIED-LIVE 2026-06-01]`
...
**Counterexample — ABB Ekip Dip is discrete, NOT envelope-only (`[VERIFIED-LIVE 2026-06-02]`).** Filtered precisely to `Ekip Dip` (vs the broader `Ekip` families — `Ekip Touch` is the adjustable/LCD unit and IS envelope-only), the family is **already stored as discrete dial rows** and served correctly — no catalog needed: LS/I → 16 LTPU taps, LSI/LSIG → 30, LVPCB/XT7 → 25 (e.g. XT2 LSI sensor 30067 LTPU `0.4,0.42,**0.46**…1.0` with **0.44 skipped**; STPU/INST `…3.5,4.5,5.5…` with **4/5/6 skipped**; LTD `3,12,36,60`). Those **irregular gaps are the DIP-switch signature** (switch-weight sums, not a smooth ladder) — they look like data errors but are faithful, so the endpoint correctly serves them verbatim. Ekip Dip pickups are `*_calc` frame/plug-based → labelled **"× In"** (the calc-driven units fix above handles this automatically; pre-fix they wrongly showed "× Ir"). The system correctly **distinguishes** discrete (serve as-is) from envelope-only (catalog), confirming the §7 design. **Edge:** two `Ekip DIP [IEC]` styles — `XT6 LS/I` + `XT7 LS/I` (~6 sensors) — have **zero** setting rows (element-absent / not loaded; a data-*completeness* gap distinct from envelope-only fabrication; would render "Not available"). `[VENDOR-DOC]` cross-check of the exact Ekip Dip DIP tap set is an optional belt-and-suspenders follow-on.
