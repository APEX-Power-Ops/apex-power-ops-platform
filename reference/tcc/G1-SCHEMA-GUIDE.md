...
> - **The wrong targets are verbatim in EasyPower's library:** Eaton Power Defense PDG3/4 → `(PXR20/25, NRX-LSI(RF))` [800–4000 A], PDG5/6 "PXR 10" → `(PXR 10, PDG2-LSI)` [60–225 A] — EasyPower's own mis-attribution of the trip type on the newer frames. Scope (4382 served bridged styles): 4253 resolve to sensors, **129 dangling** (no `DatStyle`), **~277 rating-mismatch** (breaker `r_cont_current` outside the SST sensor envelope). The bridge view (`tcc.vw_breaker_sst_bridge`) also surfaces **all** sensors of the target style with **no rating narrowing** — prod dropped Access `r_cont_current` (the rating EasyPower uses to pick the sensor within a style).
> - **Fix:** a cited `[VENDOR-DOC]` SST-correction (Eaton TD012064/065/067/068EN + PXPM) re-pointing each defective PD frame to its rating-correct PXR-LSI style — **all 31 Eaton PD rows corrected** (PDG5 migr `009`; PDG3→PDG3-LSI, PDG6→PDG6-LSIGM migr `010`); curve model + map at `services/neta/pxr_curves.py` + `EATON-POWER-DEFENSE-PXR.md`. `[VENDOR-DOC]` `[STATE §138–§141]`
