...
> - **`tcc.brk_mccb` = 599 vs Access `BreakerMCCB` = 640 (−41) — ROOT-CAUSED 2026-06-01 (lean 2; the earlier "RI orphan drops" guess was WRONG).** The governed table carries `UNIQUE (manufacturer_id, name)`, but the Access natural key is the **4-tuple `(Mfr_ID, Type, cStandard, Acdc)`** (0 dups there). 43 `(Mfr_ID, Type)` pairs have a twin differing **only by `Acdc` (AC↔DC) and/or `cStandard` (ANSI↔IEC)**; the too-narrow index collapsed them. **41, not 43:** 2 survived because their `name` differs only by letter-case (`Multi 9`/`MULTI 9`, `ComPact [IEC]`/`COMPACT [IEC]`) — Access groups case-insensitively, Postgres unique is case-sensitive. **Not a manufacturer-resolution (D1) drop:** `manufacturer_id` is preserved 1:1, all FKs resolve, and `brk_iccb` (29→29) + `brk_pcb` (157→157) loaded fully (no `(Mfr_ID,Type)` collisions). Fix = widen the unique key to `(manufacturer_id, name, standard, ac_dc_code)` + reload the dropped twins; **deferred to the breaker load-path packet** (bundled with the D1 SST-bridge reload). G2 §4.6. `[VERIFIED-LIVE 2026-06-01]`
