...
| **2 — Template seed / field-coverage matrix** | Parse the incumbent's test forms (start transformer + circuit-breaker families) into `form_templates.field_schema` per the `02` §3 control model; prove every field maps. Cross-reference the `apex-resa` NETA procedures/test items. First real ATS sheet (LV power circuit breaker). | 1, `02` | TODO — **next** |
