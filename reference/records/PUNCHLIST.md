...
| **2b — Datasheet templates (filter + curate)** | Author `form_templates.field_schema` by **referencing + filtering** the Chip-2a NETA items + the common-datasheet practical fields (the legacy LV power-CB form witnesses the real sheet). The NETA-vs-common-datasheet divergence surfaces as a query (`neta_test_items` LEFT JOIN field refs) for operator ruling — not a copy. First real ATS sheet = LV power circuit breaker. | 2a, `02` | TODO — **next** |
