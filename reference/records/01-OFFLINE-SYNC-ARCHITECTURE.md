...
> Read this before changing any `records.*` table, sync rule, or the field PWA.

- **Status:** RULED — direction decided 2026-06-12 (operator: Jason Swenson)
- **Owner:** APEX NETA Records lane
- **Home:** `reference/records/` (beside the migrations it governs)
...
Every `records.*` table is classified into exactly one sync direction. This is the
single most important rule in the lane:

### Reference data — **server-authoritative, flows DOWN only**
`asset_classes`, `assets`, `form_templates`, `pm_programs`, `pm_schedules`
...
`form_submissions`, `form_field_values`, `pm_events`
...
- Acceptance windows (`form_field_values.min_acceptable/max_acceptable/expected`) ride
...
- **Instrument import** — pulling readings directly off field test sets
  ...
  later lane. The `form_field_values.value_kind` shape already leaves room for it.
...
- **Hard cross-schema FK activation** (`records` → `org`/`work`) — currently soft UUID
