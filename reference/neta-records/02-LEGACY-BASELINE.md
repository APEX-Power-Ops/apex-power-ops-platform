...
`neta.datasheet_templates.field_schema` (JSONB) so the catalog is data, not code.
...
|---|---|---|
| R1 | As-Found / As-Left is a **sheet-level** classification | `as_found_as_left_enum` + `datasheets.as_found_as_left` (a maintenance visit = two sheets) |
...
| R3 | Readings split by type incl. embedded graphs | `result_value_kind = numeric|boolean|text|selection|graph` |
| R4 | A human-readable job number is the cross-system key | `datasheets.job_number` (+ index), alongside soft `project_ref` |
| R5 | Test status is richer than pass/fail | `datasheets.test_status_label` beside `overall_assessment` |
...
dedupe/round-trip. The detailed legacy field-to-`neta` mapping is derived **at that
