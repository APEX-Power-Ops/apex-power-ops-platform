...
- **Status:** Chips 0–5 BUILT on `ops_test` (SSoT + identity + quote model + recognition ledger + progress billing + intake envelope; dev-only; **MERGED to main `39c968d3` via PR #28 (2026-06-22)**) · **nothing applied to prod** (purely additive; ops migrations apply only to `ops_test`/`ops_dev`). Chip 5 **post-audit-hardened 2026-06-21**: an operator post-completion audit found 5 defects + the fix work exposed 1 latent `approve_run` bug — all fixed + regression-tested; a 2nd external (Codex) audit then found 6 more (finance leak via guard errors, patch_review races, content_type/FK/scope-name-leak boundary gaps) -- all fixed + tested (tip `2792be7c`).
