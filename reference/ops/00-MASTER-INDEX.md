...
- **Status:** Chips 0–5 BUILT on `ops_test` (SSoT + identity + quote model + recognition ledger + progress billing + intake envelope; dev-only; operator-gated merge to main) · **nothing applied to prod**. Chip 5 **post-audit-hardened 2026-06-21**: an operator post-completion audit found 5 defects + the fix work exposed 1 latent `approve_run` bug — all fixed + regression-tested (tip `89fafd83`).
