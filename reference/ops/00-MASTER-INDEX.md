...
- **Status:** Chips 0–5 BUILT on `ops_test` (SSoT + identity + quote model + recognition ledger + progress billing + intake envelope; dev-only; operator-gated merge to main) · **nothing applied to prod**
...
- **Chip 5 — Intake envelope / lifecycle / UI (extractor pre-existing). DONE** — `00ddc4d2`, dev-only `ops_test`, operator-gated merge. mig 007 envelope tables + `ops-intake` parse/approve package + host-gated control-plane API (5 routes, finance-redacted) + `operations-web` pm-review page (upload → review tree → approve, no dollars); 10/10 mig + 38 pkg + 14 API + UI smoke green. Decisions captured in D-OPS-12.
