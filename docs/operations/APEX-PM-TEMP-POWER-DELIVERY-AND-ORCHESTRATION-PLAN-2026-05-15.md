...
1. Local PM intake workbench usability is active and local-current through PM Lane 140. PM Lane 136 adds the repo-local backend approval-persistence implementation, PM Lane 137 surfaces read-only approval status, and PM Lane 140 reconciles the workbench around hosted readiness without adding UI write authority. It admits only the narrow approval-record persistence contract and keeps browser approval submission, first approval-row creation, project import, workpackage, task, apparatus, assignment, schedule, status, and production tracking writes blocked.
