...
2. Hosted PM intake parity is accepted green for the PM intake path and the broader deployed mutation-seam read surface. PM Lane 041A operations-web promotion is green, PM Lane 041B Render PM-intake read parity is green, PM Lane 041C cleared the prior Supabase pooler DSN issue for DB-backed approval/schedule reads, PM Lane 138 applies the approval-persistence hosted schema gate, and PM Lane 139 tightens the hosted smoke/closeout evidence contract.
3. Approval/import authority is narrowly advanced for approval-record persistence only. The dedicated table migration, insert-only adapter, PM-only mutation route, idempotent replay, audit linkage, and read-only status route are implemented locally; PM Lane 138 applies the hosted approval table/schema gate with zero approval rows; and PM Lane 139 proves the hosted smokes check approval-status GET plus approval POST OpenAPI registration. Frontend approval controls and project import mutation remain blocked until later packets explicitly open those paths.
