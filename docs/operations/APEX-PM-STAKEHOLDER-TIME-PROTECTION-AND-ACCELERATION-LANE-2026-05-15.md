...
1. Local PM intake workbench usability - active and local-current through PM Lane 142 dispatch. The safe local ergonomics run has reduced Jason's daily scan burden on `/pm-review/import-intake`; PM Lane 140 reconciles the workbench with the hosted approval-readiness truth, PM Lane 141 defines the browser approval submission packet, and PM Lane 142 authors the first-row execution gate prompt without adding browser write authority.
