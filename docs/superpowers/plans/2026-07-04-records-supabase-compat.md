...
**Revision:** rev 2 - folds the operator plan-audit (2026-07-04): P1 corrected ownership-transfer choreography (transfer AS the current owner, not while `SET ROLE`d into the target; 046 + 048 cross-role); P1 removed `apply_migration` (host `psql` only, no history rows); P2 per-probe unique scratch roles + teardown; P2 the applier-privilege probe is a scratch-write needing its own GO. Phase 0 is a single operator-gated run; Phase 2 does not fan out until Task 2.0 clears Gate A, Gate B, and the choreography. | rev 3 - folds the second operator plan-audit: reverse/down ownership probes (`B -> A` and `A -> postgres`, incl. probing whether managed Supabase permits `grant postgres to <custom>`); corrected temp schema `CREATE` to the NEW/receiving owner (not `postgres`); Task 0.7 self-proof that `supabase_probe.py` matches the manual Phase-0 matrix. | rev 4 - Phase-4-prep EXECUTED early (commits `22cc22a2` + `bb2695c3`): retired the conditional-`-1` in Task 0.1 / 3.2 (all up-migrations source-wrapped -> plain `psql -f`); marked Phase-4-prep done; reconciled the gate9 serving-design spec's '198'. | **rev 5 (2026-07-06) - Task-2.0 decisions folded (commit `e0c80da8`): trusted-applier Gate-A exemption + `records_reclaim_owner` down/up state machine. See the REV 5 block immediately below, which SUPERSEDES conflicting text.** | **rev 5.1 (2026-07-06): `045` up is ALSO reclaim-owner-aware; a uniform-owner PRE-CHECK (fail-loud on mixed) gates both 045-up and 046-up; `records_reclaim_owner` gets first-class posture asserts (D-B / D-D).**

## REV 5 (2026-07-06) - TASK-2.0 DECISIONS FOLDED (AUTHORITATIVE; overrides conflicting text below)

Phase 0 discovery + IRP + the Gate-A ESCALATION finding (`infra/database/migrations/records/PHASE0-FINDINGS.md` -> "ADDENDUM v2" + "TASK 2.0 DECISIONS", commit `e0c80da8`) resolved the gates; the operator ruled. These parameters SUPERSEDE any conflicting instruction below - especially the OLD "046 asserts NO usable postgres path into records_owner" and "046_down reclaims ownership to postgres."

### D-A. Gate A = TRUSTED-APPLIER EXEMPTION (invariant 8 re-scoped)
The managed `postgres` creator edge is SELF-ESCALATABLE (postgres self-grants SET+INHERIT via its admin option, then `SET ROLE` succeeds), so NO in-migration assert can isolate `postgres` from `records_owner`. DECISION: the `postgres`/operator applier is custody-controlled and EXEMPT from invariant 8. **Invariant 8 (RESTATED):** no NON-ADMIN role holds a USABLE (`set_option` OR `inherit_option`) membership path into `records_owner`/`records_fn_owner`. The non-admin targets that MUST stay isolated are `records_api`, `records_intake_writer`, `records_auditor`, and any Data-API/serving role (`service_role`, `authenticated`, `anon`). The trusted `postgres` applier is explicitly OUT OF SCOPE; `service_role`/`authenticated`/`anon` remain IN scope.
- Every zero-membership assert (045 [1], 046 [4], 047 final) becomes: flag a `pg_auth_members` row whose group is `records_owner`/`records_fn_owner`, `(set_option OR inherit_option)` is true, AND the member is a NON-admin role (`member <> 'postgres'::regrole`, and not another platform-admin role). It does NOT flag `postgres`, and does NOT flag admin-only (`set=false`/`inherit=false`) edges.
- Roles are created IN-migration by `postgres` (NOT pre-provisioned). `supabase_probe.py` runs `--gate-a-policy=trusted-applier`.

### D-B. records_reclaim_owner + the down/up ownership STATE MACHINE (PIN before any SQL)
`grant postgres to <custom>` is REJECTED (42501), so `046_down` cannot reclaim to `postgres`. DECISION: a dedicated **`records_reclaim_owner`** (NOLOGIN, NOSUPERUSER, NOBYPASSRLS, non-admin) is the down-direction parking owner. Records objects therefore have TWO at-rest owners: `postgres` (fresh from 001-044, before any 046 up) OR `records_reclaim_owner` (after a 046_down).
- **Role lifecycle:** `046` (up) creates `records_owner` AND ensures `records_reclaim_owner` exists (idempotent `create role ... if not exists` via a DO block). `046_down` transfers objects to `records_reclaim_owner`, then drops `records_owner` (now owns nothing) but KEEPS `records_reclaim_owner`. So `records_reclaim_owner` persists across down/up cycles; `records_owner` is per-up-created / per-down-dropped.
- **046 UP branches on the current owner** (detect via `relowner::regrole` on a records base table):
  - FROM `postgres`: `grant records_owner to postgres with set true, inherit false, admin false`; `grant create on schema records to records_owner`; transfer objects AS `postgres` (schema LAST); owner-only ops under `set role records_owner`; `revoke records_owner from postgres`.
  - FROM `records_reclaim_owner` (a prior down): `grant records_owner to records_reclaim_owner with set true`; under `set role records_reclaim_owner` (the current + schema owner) `grant create on schema records to records_owner` then `alter ... owner to records_owner` (all objects, schema LAST); `reset role`; owner-only ops under `set role records_owner`; `revoke records_owner from records_reclaim_owner`.
- **046 DOWN: records_owner -> records_reclaim_owner** (cross-role reverse, proven): ensure `records_reclaim_owner` exists; `grant records_reclaim_owner to records_owner with set true` (transient); under `set role records_owner` `grant create on schema records to records_reclaim_owner` (as the schema owner) then `alter ... owner to records_reclaim_owner` (all objects, schema LAST); `reset role`; `revoke records_reclaim_owner from records_owner`; then drop `records_owner` + reverse its role-attr work. KEEP `records_reclaim_owner`.
- **045 DOWN runs AFTER 046_down** (downs reverse: 049_down..046_down..045_down), so records objects are then owned by `records_reclaim_owner`. 045_down's object-level DDL (drop policies, `disable`/`no force row level security`) needs owner authority: `grant records_reclaim_owner to postgres with set true` (transient); `set role records_reclaim_owner`; drop policies + disable/no-force RLS; `reset role`; `revoke records_reclaim_owner from postgres`. Then the app-role reversal (drop/alter `records_api`/`records_intake_writer`) runs as `postgres`.
- **Cycle end state:** a full `049_down..045_down` leaves the records objects owned by `records_reclaim_owner` (app/owner/audit roles dropped; RLS/policies/triggers/audit removed; the 001-044 records tables themselves are NEVER dropped by 045-049 downs = data-preserving). A subsequent full `045..049` re-up enters 046 from the `records_reclaim_owner` branch above.
- **Uniform-owner PRE-CHECK (rev 5.1; shared by BOTH `045` up and `046` up):** before either migration branches on the at-rest owner, assert that EVERY records object - the schema (`nspowner`), ALL classes (`relowner`: tables/views/sequences) and ALL routines (`proowner`) in schema `records` - is owned by EXACTLY ONE role, and that role is `postgres` OR `records_reclaim_owner`. If ownership is MIXED or any other owner appears, `raise exception` (fail loud). A "detect via one base table" check is INSUFFICIENT - it would false-green a partial/botched transfer. The single detected owner is what both migrations branch on.
- **`045` UP is ALSO owner-aware (rev 5.1):** on a re-up after a down cycle the records objects are owned by `records_reclaim_owner` (046 has not run yet - up order is 045 then 046), so `045`'s object-level owner-requiring DDL - `enable row level security`, `create policy`, the SELECT/INSERT grant matrix to the app roles, `alter view ... security_invoker`, and any PUBLIC/schema/table/routine grant/revoke that requires ownership - MUST run AS the at-rest owner: directly as `postgres` when postgres-owned (fresh), or under `set role records_reclaim_owner` (via a transient `grant records_reclaim_owner to postgres with set`, revoked after) when reclaim-owned. The role creation + role-attr work (create `records_api`/`records_intake_writer`) always runs as `postgres`. This is the up-mirror of the rev-5 `045_down` reclaim-awareness.

### D-C. Executor premise = non-super managed `postgres` (documented, unchanged)
Non-super `postgres` applies migrations as the trusted admin/applier; serving connects AS non-bypass direct roles (`records_api`). Do NOT revert to a superuser-authored model. D4 (postgres is `BYPASSRLS`): Phase 3 adds a DML negative control proving records RLS BLOCKS a non-bypass role.

### D-D. records_reclaim_owner is a first-class POSTURE-SURFACE role (rev 5.1)
`records_reclaim_owner` is a PERSISTENT NOLOGIN owner-like role, NOT a hidden test helper - it carries explicit asserts wherever it appears:
- **Role flags:** `NOLOGIN`, `NOSUPERUSER`, `NOBYPASSRLS`, `NOREPLICATION`, `NOCREATEDB`, `NOCREATEROLE`.
- **No usable memberships:** no `set_option`/`inherit_option` membership to OR from any NON-admin role (`records_api`/`records_intake_writer`/`records_auditor`/`service_role`/`authenticated`/`anon`), in either direction - the same invariant-8 standard as the other owner roles (D-A). The transient `postgres`->`records_reclaim_owner` `WITH SET` grant used during a transfer is `postgres`-side (exempt) and MUST be revoked before any terminal assert.
- **Ownership:** it owns records objects ONLY in the down/at-rest state (post-`046_down`, pre-`046_up`); after a SUCCESSFUL `046` up it owns NOTHING (records objects are `records_owner`/`records_fn_owner`-owned). Assert both: `046` up = reclaim owns nothing at the end; `046_down` = reclaim owns EXACTLY the records object set, role flags correct.
...
- **Ownership-transfer rule (corrected).** `ALTER ... OWNER TO <new_owner>` ALWAYS runs AS the object's CURRENT owner, holding `WITH SET` membership in `<new_owner>` - NEVER while `SET ROLE`d into `<new_owner>` (that role does not yet own the object, so the transfer is rejected). Owner-only follow-ups (`FORCE ROW LEVEL SECURITY`, definer/RLS work) run under `SET ROLE <new_owner>` AFTER the transfer, because `INHERIT FALSE` membership does not by itself confer the owner's authority. For a cross-role transfer (048: a `records_owner`-owned object -> `records_fn_owner`), the executor is the current owner `records_owner`, which must hold `WITH SET` membership in `records_fn_owner`. Per the PG16+ rule, it is the NEW/receiving owner (NOT `postgres`) that must hold `CREATE` on the object's schema to receive ownership; grant it temporarily if it lacks it. The `_down` reversals transfer ownership BACK the same way, AS the then-current owner. **REV 5 OVERRIDE:** `046_down` reclaims to **`records_reclaim_owner`**, NOT `postgres` (`grant postgres to <custom>` was PROVEN REJECTED 42501); `048_down` = `records_fn_owner` -> `records_owner` (unchanged). After `046_down`, `045_down` operates under `set role records_reclaim_owner`, and `046` UP may start from EITHER `postgres` or `records_reclaim_owner`. See REV 5 D-B for the full state machine.
...
### Task 0.4: Probe the ownership-transfer choreography (red/green; forward + cross-role + reverse)
...
- [ ] **Step 3 (GREEN - forward, current-owner pattern MUST work)** - The NEW/receiving owner (NOT `postgres`) must hold `CREATE` on the object's schema to receive ownership (PG16+ rule): `grant create on schema probe_s to probe_own_a` if it lacks it (record whether it was needed). Then, as `postgres` (the current owner, holding the `WITH SET` grant in `probe_own_a`): `alter table probe_s.t owner to probe_own_a` (and the view/sequence/function; transfer `alter schema probe_s owner to probe_own_a` LAST). EXPECT success. Then the owner-only op under the bracket: `set role probe_own_a; alter table probe_s.t force row level security; reset role`. EXPECT success (validates the A3 `INHERIT FALSE` + `SET ROLE` design).
- [ ] **Step 4 (cross-role forward, for 048)** - `grant probe_own_b to probe_own_a with set true` (as postgres); `grant create on schema probe_s to probe_own_b` if needed; `set role probe_own_a` (now the current owner of `probe_s.t`); `alter table probe_s.t owner to probe_own_b` - transfer AS the current owner `probe_own_a`, which holds the `WITH SET` grant in `probe_own_b`; `reset role`. EXPECT success. This proves 048's `records_owner -> records_fn_owner` path.
- [ ] **Step 5 (reverse transfers, for the `_down` migrations)** - Prove the down-direction transfers `046_down`/`048_down` need. (a) `B -> A` (048_down): `set role probe_own_b` (the current owner, holding `WITH SET` in `probe_own_a` from Step 4); `alter table probe_s.t owner to probe_own_a`; `reset role`; EXPECT success. (b) `A -> postgres` (046_down): the current owner `probe_own_a` must be able to SET ROLE to `postgres` - so **probe whether managed Supabase PERMITS `grant postgres to probe_own_a with set`**. If permitted: `set role probe_own_a; alter table probe_s.t owner to postgres; reset role; revoke postgres from probe_own_a`; EXPECT success. **If `grant postgres to <custom>` is REJECTED, FLAG it** - the reclaim-to-`postgres` path is not cleanly achievable; route to a Task-2.0 down-parity escalation (alternate down design: drop-and-recreate, or a dedicated reclaim owner). Record the SQLSTATE either way.
- [ ] **Step 6** - `revoke probe_own_a from postgres`, `revoke probe_own_b from probe_own_a`, and revoke any `create on schema` grants; confirm ownership state + assert membership residue per Gate A.
- [ ] **Step 7 (teardown/finally)** - drop scratch objects + schema; `drop role probe_own_a`; `drop role probe_own_b`; assert clean. Record the FULL working choreography (forward + cross-role + both reverse directions; exact grant flags; which role needed schema `CREATE`; the RED wrong-pattern SQLSTATE; whether `grant postgres to <custom>` is permitted) to `PHASE0-FINDINGS.md`.
  Expected: a proven, copy-ready choreography for every transfer 045-049 up+down performs, or an explicit down-parity escalation if the reclaim-to-`postgres` path is blocked.
...
- [ ] **Step 1b (self-proof)** - Run `supabase_probe.py` against the SAME branch and assert its emitted pass/fail matrix EQUALS the manually-recorded Task 0.2-0.6 results (an agreement gate). Any divergence FAILS Phase 0 and must be reconciled - the reusable probe must be proven faithful to the manual findings before it is trusted as the Phase-4 prod precondition.
...
- [ ] **Step 1** - Read `PHASE0-FINDINGS.md`. Resolve, in writing (append to the findings file): (a) A2 per-attribute KEEP/DROP+assert list; (b) `GATE_A` verdict -> if `unavoidable-edge`, **STOP and escalate to the operator** (choose pre-provisioned-role path or explicit acceptance) before any 046 edit; (c) `GATE_B` verdict -> if `reject`, **STOP and escalate**; (d) the exact ownership choreography from Task 0.4 (grant flags, which role needs schema `CREATE`, SET-ROLE bracket, cross-role A->B, AND both reverse directions B->A and A->postgres), confirmed red/green proven - if Task 0.4 found no `postgres`-reachable forward path, OR the reclaim-to-`postgres` reverse requires a `grant postgres to <custom>` that managed Supabase rejects, **STOP and escalate** (the latter for a down-parity design decision); (e) any `supabase_admin`-owner flag from 0.1.
...
- [ ] **Step 3: implement** - in `045`: remove `nosuperuser` from lines 21-22 (certain); for the residual `nobypassrls/noreplication/nocreatedb/nocreaterole` apply the A2 KEEP/DROP+assert decision from Task 2.0 (if DROP: add a post-`alter` `do $$ begin if (select rolbypassrls or rolreplication or rolcreatedb or rolcreaterole from pg_roles where rolname='records_api') then raise exception '045: records_api forbidden attr'; end if; end $$;` block - exact per attribute per findings). Policy binding stays `TO records_api, records_intake_writer` per `GATE_B`; if `GATE_B=keep`, add a comment reconciling the stale header. The membership assert refines per REV 5 D-A (postgres-exempt; non-admin-role targets). **REV 5.1 D-B: `045` UP is owner-aware** - after the uniform-owner pre-check, run 045's object-level owner-requiring DDL (RLS enable, policy create, the grant matrix, `security_invoker` views, owner-required PUBLIC/schema grants) AS the at-rest owner: as `postgres` when postgres-owned (fresh), or under `set role records_reclaim_owner` on a re-up; role creation/attrs always as `postgres`. Mirror the reverse in `045_down` **- REV 5 D-B: because `045_down` runs AFTER `046_down`, the records objects are then owned by `records_reclaim_owner`, so `045_down`'s object-level DDL (drop policies, `disable`/`no force row level security`) runs under `set role records_reclaim_owner` (transient `grant records_reclaim_owner to postgres with set`); the app-role reversal runs as `postgres`.**
...
- [ ] **Step 1: test** - via `--apply-as-non-superuser`, adapted `046` SUCCEEDS; post-apply assert every `records.*` object + the schema are owned by `records_owner`; FORCE RLS on all base tables; `records_owner` is NOLOGIN/non-super/non-bypass; and invariant 8 **(REV 5 D-A re-scope: no NON-ADMIN role - `records_api`/`records_intake_writer`/`records_auditor`/`service_role`/`authenticated`/`anon` - holds a usable `SET`/`INHERIT` path into `records_owner`; the trusted `postgres` applier is EXEMPT)**.
- [ ] **Step 2** - Run; expect FAIL (unadapted 046 alter-roles + owner-to fail under non-super).
- [ ] **Step 3: implement** - in `046`: (i) remove `nosuperuser` from the `create/alter role records_owner` (33/37) + A2 residual decision + forbidden-attr assert; (ii) BEFORE the owner-to loop: `grant records_owner to postgres with set true, inherit false, admin false` (+ `grant create on schema records to records_owner` if Task 0.4 found the RECEIVING owner needs schema `CREATE` - it is `records_owner`, not `postgres`, that needs it); (iii) run the `owner to records_owner` loop AS `postgres` (the current owner, holding the `WITH SET` grant from (ii)) - do NOT `set role` for the transfer itself; transfer the schema owner LAST (or use the temp schema `CREATE` from (ii)) so the object transfers keep schema `CREATE`; THEN wrap ONLY the owner-only follow-ups (the `force row level security` loop and any other owner-only DDL) in `set role records_owner; ... reset role;` (per the corrected ownership-transfer rule - `INHERIT FALSE` membership does not by itself confer owner authority); (iv) `revoke records_owner from postgres` AFTER all owner-only DDL and BEFORE `046`'s terminal `raise exception '... LOGIN role(s) are members of records_owner'` (`:104-107`) - splice exactly there so the self-check sees zero LOGIN members; (v) add the effective-membership + temp-authority residue asserts. Mirror in `046_down` **per REV 5 D-B**: reassign ownership to **`records_reclaim_owner`** (NOT `postgres`) - `set role records_owner`, `grant records_reclaim_owner to records_owner with set` (transient) + `grant create on schema records to records_reclaim_owner` (as the schema owner), transfer all objects (schema LAST), reset/revoke; then drop `records_owner`, KEEP `records_reclaim_owner`. ALSO: `046` UP must ensure `records_reclaim_owner` EXISTS (idempotent) and BRANCH its transfer on the current owner (`postgres` fresh, OR `records_reclaim_owner` from a prior down) - REV 5 D-B. The terminal membership assert (iv) refines per REV 5 D-A (postgres-exempt, non-admin-role targets, usable-membership only).
...
- [ ] **Step 3: implement** - per P1-c, 048 re-establishes ITS OWN temp authority (046 revoked its own): `grant records_owner to postgres with set true...` AND `grant records_fn_owner to postgres with set true...`, AND (for the cross-role transfer) `grant records_fn_owner to records_owner with set true` so the creating owner can transfer to the fn-owner (+ `grant create on schema records to records_fn_owner` if Task 0.4 found the receiving owner needs schema `CREATE`). Then `set role records_owner`; CREATE `audit_log`/objects in the schema (they are now owned by `records_owner`); `alter table audit_log owner to records_fn_owner` - transfer AS the current owner `records_owner`, which holds the `WITH SET` grant in `records_fn_owner` (per the corrected ownership-transfer rule, NOT while set-role'd into `records_fn_owner`); `reset role`. Do the SECURITY DEFINER / FORCE RLS owner-only work under `set role records_fn_owner` (the new owner) AFTER the transfer; `reset role`. Revoke ALL temp grants (`records_owner`+`records_fn_owner` from `postgres`, and `records_fn_owner` from `records_owner`, plus any `create on schema` grants) BEFORE any terminal assert; assert residue clean. Mirror in `048_down`: the reverse transfer of audit objects `records_fn_owner` -> `records_owner` runs AS the current owner `records_fn_owner` (`set role records_fn_owner`, holding `WITH SET` in `records_owner`; per Task 0.4 Step 5(a)); reset/revoke before any terminal assert.
- [ ] **Step 4** - Run; expect PASS. Commit.
...
- [ ] **Step 2** - Run down->up cycles for 045-049 under the non-super mode; expect clean reversal + re-apply. **REV 5 - the `records_reclaim_owner` state machine MUST be tested (Task-2.0 rec 2):** (a) post-`046_down`, all `records.*` objects + the schema are owned by `records_reclaim_owner` (NOT `postgres`, NOT `records_owner`); (b) `045_down` SUCCEEDS after `046_down` (operating on reclaim-owner-owned objects under `set role records_reclaim_owner`); (c) `046` UP SUCCEEDS from the `records_reclaim_owner` at-rest state (the second up-branch); (d) a FULL `049_down..045_down` then `045..049` cycle passes and is data-preserving (records rows intact; `records_reclaim_owner` persists, `records_owner` re-created) - and the re-up `045`/`046` succeed FROM the `records_reclaim_owner` at-rest owner (the owner-aware up path); (e) `records_reclaim_owner` POSTURE (REV 5.1 D-D) holds throughout: NOLOGIN/non-super/non-bypass/no-repl/no-createdb/no-createrole, no usable membership to/from any non-admin role, owns records objects ONLY in the down state (owns nothing after a successful up); (f) the uniform-owner pre-check FAILS LOUD on a synthetically-mixed ownership state (negative test). Commit any harness tweaks.
  Expected: acceptance (2) + (9-local).

### Task 3.2: Branch green - adapted 001-049 + down->up on a real Supabase branch

- [ ] **Step 1** - `create_branch`; apply adapted `001-049` via value-silent host `psql` over the branch DSN (plain `psql -v ON_ERROR_STOP=1 -f` per file - all up-migrations source-wrapped, no `-1`; NO `apply_migration`); expect green end-to-end (the authoritative proof). Then run the FULL `049_down..045_down` (reverse order) -> re-apply `045..049` cycle on the branch; expect clean AND the REV 5 `records_reclaim_owner`-state invariants (Task 3.1 Step 2 a-d) to hold on the real branch (post-046_down owner = `records_reclaim_owner`; 045_down + 046-up-from-reclaim succeed; records data preserved across the cycle).
...
**5. Rev-3 fold check:** Task 0.4 now proves both reverse directions (B->A for 048_down, A->postgres for 046_down) and probes whether managed Supabase permits `grant postgres to <custom>` (the reclaim-to-postgres prerequisite), with a down-parity escalation routed to Task 2.0. Temp schema `CREATE` is correctly granted to the NEW/receiving owner (records_owner / records_fn_owner), not `postgres`, in Tasks 0.4/2.2/2.4. Task 0.7 Step 1b self-proves `supabase_probe.py` against the manual matrix before it becomes the Phase-4 precondition.
