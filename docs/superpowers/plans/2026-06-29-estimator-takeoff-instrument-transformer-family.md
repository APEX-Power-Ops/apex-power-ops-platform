...
**Post-build Codex-IRP follow-up (P2-b), shipped AFTER the 5-task build (commit on top of T5):** the build's `parsePackaging` (T3) never emitted `packaging:'individual'`, so the `matchInstrumentTransformer` individual branch (T2) was unreachable and an explicit "Individual" token (the firm's refs literally name "- Individual") was lost. Fix: add `individual_token` to `ItxPackagingEvidence` (signature/types.ts) and a `/\bindividual\b/i` branch to `parsePackaging` (after the bare-set branch, before the `unknown` fallback) returning `{ packaging:'individual', packagingEvidence:'individual_token' }`. Pinned by: normalize-itx ("explicit Individual token -> individual + individual_token"), itx-map ("individual packaging evidence -> the individual non-set default"), itx-pipeline ("CCVT Individual -> scope_pending with the individual provisional default"). The P2-a duplicate-tag evidence-merge in `pickAuthoritative` is DEFERRED to V2/SME (shared quantify semantics; fail-closed today).
