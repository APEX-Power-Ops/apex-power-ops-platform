...
7. **Down ordering (S7, DEV-7-amended):** FIRST ALTER each fn OWNER TO postgres, THEN DROP OWNED BY (revokes current-DB grants - posture restores), THEN **guarded/drop-if-safe DROP ROLE** - skip any password-bearing login role (`pg_authid.rolpassword IS NOT NULL`) with a NOTICE so a routine down/reset never deletes ops_intake_writer/ops_api after their SCRAM passwords are set out-of-band (DEV-7).
...
11. **Acceptance grep (S8) is part of done:** zero OPS_DEV_DSN references in the infra migration tests, both routers, mount gate, both named route-test files, smoke-estimator-native.mjs; no OPS_DEV_DSN fallback in packages/ops-intake/tests/conftest.py.
