...
- VALUE-SILENT TESTS: any assertion that checks the sanitized env MUST reference only plain locals (bools / key-name lists) computed BEFORE the `assert` -- never `env`, `set(env)`, `env[...]`, or `env.get(...)` INSIDE an `assert`. pytest expands any env-bearing expression in a failing assert and prints the full env dict, which against pre-fix code holds real ambient secret VALUES. (Learned live 2026-07-04: a red-proof dumped two real dev DB passwords into the transcript.) This governs the test code in Tasks 1 and 3; the committed value-silent form (commit ffe7a2de) supersedes any illustrative `assert ... not in env` snippet shown below.
