...
```bash
ssh olares-mesh 'cd /home/olares/code/apex/apex-estimator-renderer/apps/control-plane-api && set -a; . ../../infra/.env; set +a; export OPS_DEV_DSN="host=127.0.0.1 port=5432 dbname=ops_dev user=postgres password=$DEV_PG_PASSWORD sslmode=disable"; export DATABASE_URL="postgresql://postgres:$DEV_PG_PASSWORD@127.0.0.1:5432/ops_dev?sslmode=disable"; PYTHONPATH=/home/olares/code/apex/apex-estimator-renderer/packages/ops-intake/src nohup /home/olares/code/apex/apex-power-ops-platform/.venv/bin/python -m uvicorn main:app --host 127.0.0.1 --port 8010 > /tmp/cpapi_estsmoke.log 2>&1 & UVPID=$!; sleep 6; export PATH=$HOME/.local/bin:/home/olares/.nvm/versions/node/v20.20.2/bin:$PATH; cd .. && node operations-web/scripts/smoke-estimator-native.mjs; echo SMOKE_EXIT=$?; kill $UVPID 2>/dev/null || true'
```
<!-- Note: uvicorn is started with the MAIN worktree's `.venv/bin/python` (the renderer `.venv` lacks uvicorn) and `PYTHONPATH` pointing to the lane's `ops-intake/src`. `DATABASE_URL` is required by `config.py` as a base DB URL. `pkill -f "uvicorn main:app"` is intentionally avoided — it matches the launching shell and kills the ssh session; `kill $UVPID` is used instead. -->
