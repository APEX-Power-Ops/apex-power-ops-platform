...
## Convergence close (2026-06-27/28 UTC) -- dry-run, rounds 4-6, Tasks 8-10

### Host-clone DRY-RUN + Codex round 4 -> Task 8
Generated the real artifacts and ran the host-clone dry-run off `tcc_breaker_baseline_20260625`
(clone `tcc_breaker_d4d5_gen_20260627`): DDL+data first apply COMMIT, idempotent double-apply, partition
687/13533/2=14222, all guards. Committed dry-run artifacts + README (`4c00d2a0`). Codex round 4 then
flagged 3 genuine edge cases -> **Task 8 (`30b2b2c5`, opus PASS):** P2b the 030 extra-row guard now
anti-joins the FULL class domain {ICCB,MCCB,PCB} (no staged-class filter); P2a `generate-d4d5` default
`--out-dir` anchored to the repo root (cwd-independent); P3 snapshot selection fail-closed (require
`--snapshot-id` when a run has != 1 snapshot). `4c00d2a0` thereby became a validated CHECKPOINT, not final.

### Task 9 (`cc2c5308`, opus PASS) -- byte-reproducible generation
Controller regenerate exposed non-determinism: `generate()` stamped a wall-clock `generated_at` whenever
the caller passed None (the CLI does), so every regeneration produced a different SHA. FIX: drop the
wall-clock default -> `generated_at` omitted unless explicitly passed (build_report + emitter already
omit-when-None); provenance carried by run_id/source_sha256/snapshot_id/checksums. Two runs now byte-identical.

### Custody fix (`ba1a4802`) -- autocrlf was corrupting committed bytes
After committing the regenerated set (`00cdefa7`), the committed blob SHAs DIVERGED from the host-validated
SHAs: `core.autocrlf=true` (no .gitattributes) stripped embedded CR bytes from 030/report and would have
shipped corrupted verbatim data. FIX: a scoped `.gitattributes` marking the three artifacts `-text` (no
CRLF<->LF conversion either direction) + renormalize; committed blob sha256 then equals the validated bytes.

### Codex round 5 -> Task 10 (`7f02cb70`, opus PASS) -- CR-doubling data corruption
The confirming Codex pass caught a BLOCKING prod-bound bug the in-tx value-parity guard could NOT (it is
self-referential -- target vs the same stage literal): `generate()` wrote artifacts via `Path.write_text`
with no `newline=`, so Windows text mode doubled every source `\r\n` into `\r\r\n` (36832 of them) inside
`tmt_notes`. FIX: `write_text` -> `write_bytes(...encode("utf-8"))` for all three artifacts; LF file
endings + verbatim source newlines. NOTE: 4 MCCB rows (ID 93192/93212/93252/93282) GENUINELY store one
`\r\r\n` each in Access `TMT_Notes` (verified by direct governed query), so the correct 029 carries
EXACTLY 4 `\r\r\n` -- the write path adds zero. New live test asserts the file `\r\r\n` count does not
exceed the in-memory string's (write path introduces none).

### Re-validation + NON-SELF-REFERENTIAL source cross-check
Regenerated byte-faithful (029 `27334c75`, 030 `e384648c`, report `0dd3b334`; 029 has exactly 4 `\r\r\n`).
Re-validated on a FRESH clone `tcc_breaker_d4d5_gen_20260628b`: DDL+data apply COMMIT, idempotent
double-apply, full verify checklist PASS, P2b guard actively FIRED on a seeded extra row (RAISE + rollback).
**Crucially, to close the self-referential gap Codex identified: the applied `tmt_notes` md5 on the clone
was compared to the governed `access_raw."BreakerMCCBStyles"."TMT_Notes"` md5 for all 4 genuine-CRCRLF rows
-- ALL MATCH exactly (byte-faithful round-trip against the ORIGINAL source).** Committed `bd8c0162`; HEAD
blob SHAs == validated SHAs.

### Codex round 6 (`bd8c0162`) -- CLEAN = convergence
`review-run --review-head bd8c0162 --base-ref 57b51c42`: "No actionable correctness issues were found in
the diff. The changes appear internally consistent with the added tests and generated artifact
documentation." STOP per the convergence-bounding rule. NOT merged, NOT applied to prod -- both remain
separate operator gos. Cross-engine value this slice: Codex caught the all-null-D4 source-fidelity blocker
(R1), the `\set` false-green (R2), the provenance mislabel (R3), the 3 edge cases (R4), and the CR-doubling
data corruption (R5) -- none of which the per-task opus reviews surfaced.

3 disposable host clones (gen_20260627/28/28b) await manual DROP (hook-blocked). 2 non-blocking Task-10
Minors (test report re-derivation drift; redundant imports) deferred to the final whole-branch review.
