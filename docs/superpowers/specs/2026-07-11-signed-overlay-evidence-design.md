# Signed Evidence Overlays — Design (rev 5)
...
>
> **rev 2** folded the ratified round-1 cross-engine IRP: the checker is an **evidence-readiness** gate (not authorization); recency floor + temporal-coherence bound; fail-closed defaults; overlay/receipt binding. **rev 3 (2026-07-11)** folds the round-2 re-audit: reconciles the one residual HIGH (rev-2 pinned the derived window to `E == base_observed_at` by keeping SP009's `<= observed_at` *and* adding OV017) via a **deliberate SP009 gate change** (§3), and settles `max_consumer_evidence_age_hours` as a required CLI flag (§3). **rev 4 (2026-07-11)** makes that SP009 relaxation **provenance-conditional** (in-memory `derived_window_object_ids`; original bound retained for non-overlay relations and fail-closed when the marker is absent, §3), adds a **base-census canonical-window precheck** (`OV021`, §3/§5.8), and replaces the receipt `production_eligible` field with `evidence_ready` + `execution_authorized: false` (§5.10). IRP record (all rounds): `evidence/irp-cross-engine-overlay-35755d69.md`.
...
Until these are resolved, the `check_disposition` preapply gate **withholds evidence readiness** for a destructive/promote/compat/archive/harden action and rejects the raw census: `SP009` (zero-width `consumer_evidence.observation_window`), `SP010` (`required_observations` must be freshly `observed`), `SP022`/`SP013` (a resolved consumer conclusion requires EVERY consumer dimension `observed` — any non-observed state, incl. `not_applicable`/`query_failed`/`stale`, is rejected; the single exception is `external_clients=not_applicable` on an ACCEPTED delete, governed by the SP027 floor's exposure-false waiver — gate-corrections #102/#103; count agreement), `SP027` (delete floor). Today `check_disposition` verifies the *snapshot* signature (SP026) but has no way to accept later-collected evidence. Overlays are that mechanism — without weakening any existing gate and without touching the signed census.
...
- **At the semantic gate (`SP009` on the effective view):** ordering `S < E` and duration `(E−S)/3600 >= minimum_consumer_window_hours`, plus `SP027`'s DELETE-only `>= 720h`. **Deliberate, provenance-CONDITIONAL gate change** (round-2 residual HIGH: keeping `e <= observed_at` alongside `OV017` `S <= base_observed_at <= E` pins `E == base_observed_at`, so the gate could never go green). The checker maintains an in-memory `derived_window_object_ids` set — the relations whose consumer window it derived-and-wrote at merge — passed to SP009 as context and **never serialized**. SP009 branches per relation:
  - **overlay-derived** (`object_id ∈ derived_window_object_ids`): `S < E` + duration only; the `e <= observed_at` upper bound is dropped (future/recency/anchor already enforced at merge by `OV009`/`OV016`/`OV017`).
  - **non-overlay / census-supplied** (`object_id ∉` the set): the **original** `s < e <= observed_at` + duration, unchanged.
  - **no provenance context** (set unavailable): fail closed with the **original** SP009 behavior.
  The upper bound is thus **not removed globally** — only for windows the checker itself derived. **Base-census canonical-window precheck (`OV021`):** before any merge, reject a base census whose consumer window is not the canonical zero-width `{observed_at, observed_at}` the collector emits — signature attests integrity, not collector-emission, so a signed-but-hand-crafted non-zero base window must be caught before it can ride the relaxed path. The duration floor (rev-1 dropped it) is retained throughout.
...
8. **Build the effective view** — first, as an **UNCONDITIONAL preapply precheck (`OV021`, runs even with zero `--overlay` inputs):** every relation's base `consumer_evidence.observation_window` must be the canonical zero-width `{observed_at, observed_at}` (string-equal to `observed_at`; signature attests integrity, not collector-emission — a hand-crafted signed census cannot self-resolve its own window). Then `copy.deepcopy` the parsed census; set each resolved dimension value; for **every cluster-source relation** (all source relations of the manifest's decisions — `semantic_check` runs SP009 on every `src_rel`, so each must carry a derived non-zero window, **`retain` included**) derive-and-write the consumer window per the §3 predicate as **ISO-8601 strings** (`OV011`/`OV016`/`OV017`/`OV018`), recording that `object_id` in the in-memory `derived_window_object_ids` provenance set (passed to SP009, never serialized). `OV018` fires for any cluster-source relation with zero observed contributors; non-cluster relations keep their zero-width census default (still fail `SP009` closed via the **original** bound) and are not over-rejected. **`OV022` (delete-floor temporal coherence):** where the `SP027` delete floor relies on the `external_clients` `not_applicable` waiver (which requires `in_data_api_exposed_schema` observed `false`), the contributing `in_data_api_exposed_schema` overlay's `observation_window` must **cover the derived consumer window** (`started_at <= S` and `ended_at >= E`), proving the relation was unexposed *throughout* the evidence interval — not merely at a later instant. The parsed base census object is never mutated.
...
10. **Receipt (evidence readiness, NOT authorization).** Records, per overlay: `path`, raw-bytes `sha256`, `.sig` `sha256`, `signer`, `dimension`, covered object-id count, `captured_at`, `producing_repo_sha`, `source_hash`, and (for operator_declaration) `operator_identity`/`attestation_ref` as provenance; plus `disposition_schema_sha256`/`overlay_schema_sha256`, the recency policy `max_consumer_evidence_age_hours` (so the apply-runner revalidates the identical bound), and `effective_view: {in_memory_only: true}`, alongside the SP026 signer/`snapshot_signature_sha256`/gate fields. The receipt carries **no write-GO and is not an authorization token**; a GREEN receipt attests evidence readiness only (§2A). The SP026/SP028 `production_eligible` field is **replaced** by `evidence_ready: true` + `execution_authorized: false` (execution authorization is the future apply-runner control), so no receipt field can be read by an apply runner as write authorization.
...
| OV021 | base census consumer window is not the canonical zero-width `{observed_at, observed_at}` (unconditional pre-merge precheck; signature != collector-emission) |
| OV022 | delete-floor incoherence: the `in_data_api_exposed_schema` overlay window relied on for the `SP027` `external_clients` not_applicable waiver does not cover the derived consumer window (`started_at <= S`, `ended_at >= E`) |
...
- **Provenance-conditional SP009 + base canonical window + receipt (rev-4):** a signed base census carrying a **non-zero / future-ended** consumer window with **no overlays** still fails (`OV021`, **unconditional** pre-merge — assert it fires with zero `--overlay` inputs); a later overlay-derived window with `ended_at` after `base_observed_at` **passes** SP009; **removing the in-memory `derived_window_object_ids` marker restores the original SP009 rejection** (fail-closed on absent provenance); a base census with a non-canonical window fails **before merge**; an overlay-derived window outside `OV017`/`OV016` still fails; the receipt exposes `evidence_ready` + `execution_authorized: false` and never `production_eligible: true`.
- **Delete-floor coherence + all-action scope (rev-5):** a DELETE whose `external_clients` `not_applicable` waiver rests on an `in_data_api_exposed_schema` overlay whose window does **not** cover the derived consumer window → `OV022`; a `retain` (or any non-delete) cluster-source relation with no consumer overlay → `OV018` (window derived for **every** `src_rel` SP009 evaluates, not just resolved/delete); a `retain` relation **with** covering overlays reaches evidence-ready green.
