# Signed Evidence Overlays — Design (rev 2)
...
>
> **rev 2 (2026-07-11)** folds the ratified cross-engine IRP corrections on rev 1 (`35755d69`): the checker is an **evidence-readiness** gate (not an authorization gate); a corrected time model with a real recency floor + temporal-coherence bound; fail-closed defaults throughout; and full overlay-artifact/receipt binding. IRP record: `evidence/irp-cross-engine-overlay-35755d69.md`.
...
- `max_consumer_evidence_age_hours` is a **finite, positive** bound (new `cluster_manifest` field / CLI value), `_finite()`-guarded (invariant 7; mirrors the SP015 lesson).
...
- **At the semantic gate (`SP009` on the effective view, unchanged code):** ordering `S < E` and duration `(E−S)/3600 >= minimum_consumer_window_hours`, plus `SP027`'s DELETE-only `>= 720h`. Both existing SP009 clauses are retained (the rev-1 mistake of dropping the duration floor is reverted). Because the merge already rejects a stale/incoherent/zero-contributor window, `SP009` on the effective view can never see a cleared or out-of-range window.
...
5. **Freshness (finite-guarded).** `captured_at` within `--now` and an overlay max-staleness bound; not future (`OV010`, `_finite()`-guarded). Each `observation_window` well-formed, `started_at < ended_at`, `ended_at <= captured_at`, not future (`OV009`).
...
8. **Build the effective view** — `copy.deepcopy` the parsed census; set each resolved dimension value; for each relation derive-and-write the consumer window per the §3 predicate (`OV011`/`OV016`/`OV017`/`OV018`). The parsed base census object is never mutated.
9. **Run the existing semantic gate** (`semantic_check`, SP001–SP027) against the **effective** view — plus a re-schema-validation of the merged effective view **with the FormatChecker** so a merged datetime cannot reach an unguarded `parse_dt`. SP009 (ordering + duration) and SP027 (delete floor incl. 720h) are unchanged and now see a valid derived window. `observed_at` is never overwritten; the effective view is never serialized.
10. **Receipt (evidence readiness, NOT authorization).** Records, per overlay: `path`, raw-bytes `sha256`, `.sig` `sha256`, `signer`, `dimension`, covered object-id count, `captured_at`, `producing_repo_sha`, `source_hash`, and (for operator_declaration) `operator_identity`/`attestation_ref` as provenance; plus `disposition_schema_sha256`/`overlay_schema_sha256`, and `effective_view: {in_memory_only: true}`, alongside the SP026 signer/`snapshot_signature_sha256`/gate fields. The receipt carries **no write-GO and is not an authorization token**; a GREEN receipt attests evidence readiness only (§2A).
...
| OV010 | overlay `captured_at` stale (finite-guarded bound) or future |
...
| OV016 | consumer window stale: `now - E > max_consumer_evidence_age_hours` (recency floor) |
| OV017 | temporal incoherence: `base_observed_at` not within the derived window `[S, E]` |
| OV018 | zero contributing observed consumer overlays for a relation requiring a consumer window (fail-closed) |
...
- **Time model (adversarial):** overlay `captured_at` after `base_observed_at` **accepted**; **a fresh overlay carrying a decade-old window → `OV016`**; `database_deps`/`base_observed_at` outside the derived window → `OV017`; **empty contributor set → `OV018`** (fail-closed, not a silent pass); NaN/Inf `max_consumer_evidence_age_hours` → rejected (finite guard); future `captured_at` (`OV010`); `ended_at > captured_at` (`OV009`); empty intersection (`OV011`); a valid derived window with `ended_at` after `base_observed_at` **passes** SP009; SP009 duration floor still fires below `minimum_consumer_window_hours`.
