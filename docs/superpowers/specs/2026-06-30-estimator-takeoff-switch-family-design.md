...
Post-build review additions to the R1/SME gate (both fail-closed in V1, NOT defects): (f) **anchor coverage** - `Load Interrupter Switch` (a common NETA 7.5 MV switch) is not yet in `SWITCH_DEVICE`, so it routes to `unrecognized_apparatus_row` (surfaced as a question, never priced, never lost); add `load[\s-]?interrupter\s+switch` to the anchor set at the SME gate. (g) **contact-state vs construction** - `parseSwitchType` reads `Normally Open Disconnect` as `switchType:'open'` (the construction tier), conflating contact-state with construction; the SME confirms whether `normally open` should be distinguished from a construction `open` (low impact: scope_pending only, R1 provisional, operator picks the tier at Gate-2).
