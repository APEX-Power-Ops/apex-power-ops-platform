...
Status: DESIGN rev 1. D1 ratified = Option B (2026-07-03). Grounded against prod
...
validation harness, and a reviewed prod-variant apply packet. No prod mutation in
this lane.
...
bypassed entirely. Caveats folded into sections 3.1, 3.4, and 7.
...
- Supavisor session mode (port 5432) - fallback when IPv4-only networking is
  required or pooling/queuing is wanted; supports prepared statements.
...
current_user = the role for every statement.
...
### 3.5 Startup identity assertion

A serving process asserts, at startup or first request, that current_user is one
of the three sanctioned roles and is not a superuser, not BYPASSRLS, and not an
owner role - fail-closed if not. This is the runtime detector covering the
honest-scope gap (it cannot prove the absence of a superuser/service_role
credential elsewhere, only that THIS process holds a sanctioned role).
...
3. secret-audit Check 3 arming + startup assertion (section 6).
...
- records_api: can SELECT all 14 app-served tables; cannot INSERT/UPDATE/DELETE
  any records table; cannot read neta_table_source_links or audit_log.
- records_intake_writer: can SELECT the 14; can INSERT/UPDATE only the column-
  ...
  source_links or audit_log.
- records_auditor: can SELECT audit_log only; cannot read the 14 or source_links;
  cannot INSERT/UPDATE/DELETE audit_log.
- anon and authenticated: reach NOTHING in records - no schema USAGE, no table
  grant (the Data-API-exclusion negative, AC2).
- audit_log stays append-only: no role can UPDATE or DELETE it.
- Role cleanup: drop all created roles AFTER the DB drop (Gate 5 [drop-role]
...
url_driver_qualified, pg_env_vars). Armed only when RECORDS_SERVING_GLOBS points
at real serving config, introduced in the same change. Value-silent. Add a
red/green unit per rejected credential shape and per DSN form.
...
- AC3: Only the sanctioned role DSNs reach the allowed tables; each role reaches
  exactly its contract scope and no more.
- AC4: records_api reads exactly the 14 app-served tables and writes nothing.
- AC5: records_intake_writer reads the 14 and inserts/updates only the column-
  scoped writer matrix on the 6 write-path tables; no DELETE.
