...
- **The H3 breaker-pair conflict guard, guard-first + transfer-LOCAL (D3):** inside `assessTransferSwitch`, a transfer-anchored row carrying `FRAME_TRIP` (`###AF/###AT`), a single `###AF|###AT` token, a trip-function descriptor (LSIG), an unambiguous breaker hint (`MCB|MCCB|ACB|VCB|breaker|draw-out|GB|FB`), or a `TRANSFER_CONFLICT_NONBREAKER` token (`PDU|UPS|SPD|PQM|METER|BUS DUCT` = `NON_BREAKER` MINUS the transfer tokens) -> a `transfer_parent_conflict` question (signature null), NEVER a silent transfer line and NEVER suppressing a real breaker. Using the transfer-local predicate (not full `NON_BREAKER`) is what lets a plain `ATS`/`MTS`/`STS` row NOT self-conflict.
...
- **OPEN DECISION T1 (rating-annotated ATS - surfaced by the IRP; needs operator ruling at the spec-review gate):** the H3 guard fires `transfer_parent_conflict` for ANY transfer-anchored row carrying `AF/AT`. A real single ATS device commonly carries its own frame/withstand rating as `###AF/###AT`, so this systematically converts rated ATS devices into questions rather than scope_pending lines (fail-closed - never a misprice - but an under-recognition). Rev-2 keeps the ratified D3 conflict-on-`AF/AT` posture; the alternative (recommended by both reviewers) is to conflict ONLY on a full `AF/AT` PAIR co-occurring with a breaker HINT or a trip-function, and treat a lone `AF|AT` token as `ampRating` evidence -> scope_pending. See T1 in the Testing section.
...
- New token regexes (ASCII):
  ...
  - `TRANSFER_AMP` (PLAIN continuous amps only; `\bA\b` boundary excludes `AF`/`AT`): `/(?<!\d)(\d{2,6})\s*A\b/i` - evidence only.
...
- `assessTransferSwitch(x, voltageBasis?)` - H3 CONFLICT GUARD FIRST:
  - `if (TRANSFER_BREAKER_CONFLICT.test(x.raw) || FRAME_TRIP.test(x.raw) || TRANSFER_FRAME_TRIP.test(x.raw) || TRANSFER_TRIP_FN.test(x.raw) || TRANSFER_CONFLICT_NONBREAKER.test(x.raw))` -> `transfer_parent_conflict`, signature null. (Uses `TRANSFER_CONFLICT_NONBREAKER`, NOT `NON_BREAKER` - so a plain `ATS`/`MTS`/`STS` does not self-conflict, while a transfer row co-carrying `UPS`/`PDU` does. Emit a message distinguishing the frame/trip case from the co-located-non-breaker case.)
  - else build the signature: `automationClass`, `bypassIsolation`, `ampRating`, `voltageClass` (may be undefined - NOT gated). `assessmentCode:'transfer_recognized'`.
...
3. **H3 main-tie-main: two `###AF/###AT` breakers (labeled MAIN/TIE, NO transfer anchor) -> both stay BREAKERS (priced); a single transfer-anchored row carrying `AF/AT` -> `transfer_parent_conflict` (null signature), breaker NOT suppressed.** (Recognition posture for a rated single ATS = OPEN DECISION T1.)
4. **`STS-1` (clean, tagged) -> `transfer_catalog_gap`; `STS-1 800AF/800AT LSIG` -> `transfer_parent_conflict` (H3 guard-first).** STS never maps to an ATS ref (D5).
...
- **#3 (H3) two MAIN/TIE `###AF/###AT` breakers (no transfer anchor) -> both stay BREAKERS; a single transfer-anchored row + `AF/AT` -> `transfer_parent_conflict` (null signature), breaker NOT suppressed. Breaker golden byte-identical.**
- **#4 (D5) `STS-1` clean -> `transfer_catalog_gap`; `STS-1 800AF/800AT LSIG` -> `transfer_parent_conflict`; never an ATS ref.**
...
- **T1 (OPEN DECISION - operator to rule at the spec-review gate):** the recognition posture for a rating-annotated single ATS. Rev-2 default (ratified D3): `ATS-1 800AF/800AT` -> `transfer_parent_conflict` (fail-closed question). Alternative (IRP-recommended): conflict ONLY on a full `AF/AT` PAIR + a breaker HINT or trip-fn; a lone `AF|AT` on an ATS-anchored row -> `ampRating` evidence -> transfer scope_pending. The operator picks; the test pins the chosen behavior.
