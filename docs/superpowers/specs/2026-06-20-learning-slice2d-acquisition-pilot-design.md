...
2. `resource_viewed` then `resource_completed` on **SCADA** → `content_progress` completed;
   `competency_rollup` Level III `covered_ksas` 0→2 (`KSA-III-SC-002`, `KSA-III-SC-003`),
   `coverage_percent` 0→`round(100*2/169,1)=1.2`, `evidence_event_count` ≥1.
...
2. **Assert exact equality** (engine output == manifest) for each of the four read models, captured
   ...
   - `GET /competency?user_id=U` — `resolved_level='III'`, `level_source='target'`,
     `covered_ksas == {KSA-III-SC-002, KSA-III-SC-003}`, `coverage_percent == 1.2`,
     `evidence_event_count` == count of `resource_completed`+`assessment_completed` on distinct
     non-null content.
...
3. **Negative control:** a user/level with no content-linked evidence must show coverage 0/null; a
   Level-I resolution must remain null (0 KSAs).
...
non-`learning_test` DSN; the read-only guard test; the deterministic mini-graph fixture).
...
- **Committable:** this spec; the redacted runbook (`docs/learning/slice2d/runbook.md`); the redacted
  ...
  committed, reversible cohort-provisioning SQL script (synthetic handles; idempotent + paired
  teardown by `acquisition_run_id`).
