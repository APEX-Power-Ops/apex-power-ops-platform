...
- Returns a candidate ref-GROUP (the priced scope tiers for the recognized coolant/class) + a `defaultRef` (the firm's standard ATS tier for that coolant - surfaced DOWNSTREAM as `provisionalDefaultRef` with `r1Ratified: false` until the estimator confirms it) + a human `scopeQuestion` for Gate-2. It NEVER returns a single auto-priced ref.
...
- `buckets/types.ts`: `ApparatusDispositionStatus` (line 23) gains `scope_pending`. `DispositionReasonCode` (line 30) gains `transformer_scope_pending | transformer_catalog_gap | transformer_breaker_conflict | transformer_attrs_unparsed`. `ApparatusDisposition` gains optional `candidateRefs?: string[]` + `provisionalDefaultRef?: string` + `scopeQuestion?: string` (populated only for `scope_pending`; the default is surfaced as `provisionalDefaultRef` -- NOT an authoritative `defaultRef` -- because R1 is unratified). `ReconciliationReport` gains a `scopePending[]` array (lineKey / tag / qty / candidateRefs / provisionalDefaultRef / `r1Ratified: false` / scopeQuestion) so the CLI JSON carries the full Gate-2 contract. `OperatorQuestionCode` (line 9) gains the matching codes. `TakeoffFinding.code` (line 66, currently `VoltageAssertionCode` only) generalizes to also admit `'transformer_catalog_gap'` (severity `warning`).
- `emit/emit.ts`: `runTakeoff` (line 46) match step calls `match(sig)` (dispatch) instead of `matchBreaker` directly. A transformer line resolves to a `scope_pending` disposition (stamp `scope_pending` / `transformer_scope_pending`, carry `candidateRefs` + `provisionalDefaultRef` + `scopeQuestion`; the `ref` slot is left undefined -- no authoritative ref) + an `OperatorQuestion` (`transformer_scope_pending`) carrying the group; a `catalog_gap` -> `unmatched` + warning finding. `MatchedLine` build (line 107, reads `mountingBasis`) is breaker-only -> guarded by `kind === 'breaker'`. `emitEnvelope` (line 122): scope_pending + catalog_gap lines are NOT matched lines, so they never enter the priced envelope; the zero-matched throw (line 131) is unchanged (a transformer-only takeoff legitimately has 0 priced lines -> handled by the runner's partial_preview path, not an error).
...
- R1 (estimating-AUTHORITY call) - the two default tiers (dry + oil firm-standard ATS) are surfaced as PROVISIONAL, never authoritative: `R1_RATIFIED = false` in `transformer-map.data.ts`, the structured output names them `provisionalDefaultRef` + `r1Ratified: false`, and an `it.todo` ratification marker tracks the open item (NOT a hard-failing test). The fail-closed guarantee is BEHAVIORAL: transformers are NEVER auto-priced in V1 (a recognized transformer is `scope_pending` or `catalog_gap`, never a priced line), so the unratified default is only a Gate-2 SUGGESTION. When the estimator confirms the two tiers, flip `R1_RATIFIED = true`. Lean: mid dry `(TTR/WR/IR)`, mid oil `(TTR/WR/IR)`.
