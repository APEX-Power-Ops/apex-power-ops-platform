...
- Recognition: a new `TRANSFORMER_HINT = /\b(XFMR|transformer|dry.?type|pad.?mount|oil.?filled|kVA)\b/i` OR producer `candidateKind === 'transformer'`. `TX|XFMR|KVA` are REMOVED from `NON_BREAKER` (they are now transformer-recognized); the remaining NON_BREAKER tokens (`PDU|UPS|STS|ATS|MTS|SPD|PQM|METER|BUS DUCT`) are unchanged.
...
- `null` = no ref-group covers this transformer -> a `catalog_gap` (D1): surfaced as a question + a warning finding carrying the recognized attributes; NEVER fabricated.
...
- R1 - The two `defaultRef` constants (dry + oil firm-standard ATS tier) need operator/estimator confirmation. Marked `[CONFIRM]` in `transformer-map.data.ts`; default lean = the mid dry tier `(TTR/WR/IR)` and the oil mid tier `(TTR/WR/IR)`. Non-blocking for the design; pinned before the catalog task.
...
Bounded V1 slice, ~7-8 TDD tasks mirroring the breaker engine: (1) signature discriminated-union refactor + breaker-regression guard; (2) transformer recognition + NON_BREAKER carve-out + conflict guard; (3) attribute parsers (kVA/coolant/padMount/ltc) + assessTransformer; (4) transformer-map (group + default) + family dispatch; (5) quantify/specKey kind-narrow; (6) scope_pending disposition + catalog_gap finding threaded through runTakeoff/emit; (7) runner/reconcile scope_pending = unresolved + partial_preview; (8) real golden + scope-resolution priced proof. Cross-engine (Codex) IRP before merge; merge operator-gated.
