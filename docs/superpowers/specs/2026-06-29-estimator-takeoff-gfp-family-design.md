...
- `looksLikeGfp(x): boolean` - parent exclusion FIRST, then candidateKind, then token:
  ...
  - `if (x.candidateKind !== undefined && x.candidateKind !== 'relay') return false`   // defer to breaker/transformer producer signals; a relay row with dedicated GFP wording still becomes GFP (Rev 2: dedicated ground-fault relay -> GFP)
