...
- `access_meta.tables.status` is a closed enum of LOAD-PROCESS states only (`inventoried_only`/`extracting`/`loaded`/`checksummed`/`failed`) - never a fidelity judgment.
