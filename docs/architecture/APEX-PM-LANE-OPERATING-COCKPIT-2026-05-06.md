...
|------|-------|---------------|--------------------|-----------------------------|----------------------------|
| Olares developer residency and operator hardening | Active provenance-normalization lane | The build-guide stack, adjacent authority framework, workstation rerun checklist, AI first-slice runbook, and minimal-trio runtime decision are now published in line with Olares-first execution, GitHub-canonical publication, client-only laptop posture, premium-plan-first AI use, optional local models, and the admitted minimal MCP trio. The strongest remaining misrouting risk is the repo-root task surface still exposing current-looking parent-root bootstrap and scaffold tasks. | Execute the repo-root task-surface historical relabel and demotion slice selected by Packet 096. | Do not reintroduce laptop-first durable state, mandatory Ollama assumptions, broader AI-service scope, wrapper-level Codex admission, silent publication-boundary exceptions, or parent-root task labels that still read like current default operator actions. | `Olares host bootstrap status` or `bash apex-power-ops-platform/tools/ai/run-olares-host-bootstrap-status.sh` |
