...
|------|-------|---------------|--------------------|-----------------------------|----------------------------|
| Olares developer residency and operator hardening | Active governing migration lane | The governing direction is explicit and the publication-boundary retirement dependency inventory is now published on the authoritative host mirror; the laptop remains client-only and the current Windows parent-root publication boundary is still transitional rather than permanent. | Author and execute the host-native operator publication workflow packet as the first concrete retirement target. | Do not reintroduce laptop-first durable state, conflicting workspace methods, or silent publication-boundary exceptions without a separate packet. | `Olares host bootstrap status` or `bash apex-power-ops-platform/tools/ai/run-olares-host-bootstrap-status.sh` |
